...
/**
 * Customers Tab - Client wrapper for interactive list + inline Excel entry tab
 */
...
import { Users, Phone, MapPin, Pencil, Trash2, Plus, FileSpreadsheet, ArrowLeft } from 'lucide-react';
...
import { ExcelCustomerGrid } from '@/components/customer/ExcelCustomerGrid';
import { useDataOptional } from '@/lib/context/DataContext';
import type { Customer } from '@/types';

type ViewMode = 'list' | 'excel';
...
export function CustomersTab({
    ...
    return (
        <div className="min-w-0 space-y-4 overflow-x-hidden touch-manipulation lg:space-y-6">
            {/* Excel view occupies the full tab area */}
            {viewMode === 'excel' && (
                <div className="flex flex-col" style={{ minHeight: 'calc(100vh - 200px)' }}>
                    ...
                    <div className="mb-3 flex items-center gap-3">
                        <Button
                            ...
                            className="h-8 gap-1.5 rounded-xl text-xs font-semibold text-emerald-700 hover:bg-emerald-50"
                        >
                    ...
                    <div className="flex-1 overflow-hidden rounded-xl">
                        <ExcelCustomerGrid
                            ...
                            onSuccess={(newCustomers: any[]) => {
                                // Splice new customers into hub state surgically — no full re-fetch,
                                // no customers array replacement, no table flicker.
                                const ctx = dataCtx as any;
                                if (Array.isArray(newCustomers) && newCustomers.length > 0 && typeof ctx?.upsertCustomerInState === 'function') {
                                    newCustomers.forEach((c) => {
                                        if (c?.id) ctx.upsertCustomerInState(c);
                                    });
                                } else if (typeof ctx?.fetchCustomers === 'function') {
                                    // Fallback: no force so in-flight guard prevents a storm
                                    void ctx.fetchCustomers();
                                }
                                setViewMode('list');
                            }}
                        />
            ...
            <div className="hidden items-center justify-between lg:flex">
                ...
                <div className="flex gap-2">
                    ...
                    <Button
                        onClick={() => setViewMode('excel')}
                        variant="outline"
                        className="h-10 rounded-xl border-emerald-200 bg-emerald-50/50 px-4 font-bold text-emerald-700 hover:bg-emerald-100"
                    >
