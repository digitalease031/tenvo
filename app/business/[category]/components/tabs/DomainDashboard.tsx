...
export function DomainDashboard({
    ...
    const { isEasyMode: storedEasyMode, isRetailSimpleDashboard: storedRetailSimple, modeReady } = useAppMode();
    const { planCan } = usePermissions();
    // Free/Starter plans are locked to Easy mode — clamp regardless of localStorage value.
    const canUseAdvancedDashboard = planCan('advanced_dashboard');
    const isEasyMode = canUseAdvancedDashboard ? storedEasyMode : true;
    const isRetailSimpleDashboard = canUseAdvancedDashboard ? storedRetailSimple : storedRetailSimple;
    ...
    const periodMetrics = useMemo(() => {
        ...
        const currentExpensesFromList = expenses
            .filter(exp => inRange(getExpenseDate(exp), currentFrom, currentTo))
            .reduce((sum, exp) => sum + getExpenseAmount(exp), 0);

        // expenseBreakdown is loaded in the bootstrap (faster than lazy expenses list).
        // Use it as a fallback so the retail dashboard KPI shows a number immediately.
        const expenseBreakdownTotal = Array.isArray(expenseBreakdown)
            ? expenseBreakdown.reduce((sum, row) => sum + (Number(row?.value) || 0), 0)
            : 0;

        const currentExpenses = currentExpensesFromList > 0
            ? currentExpensesFromList
    ...
    }, [dateRange, invoices, expenses, expenseBreakdown, dashboardMetrics, isSalesLoading, unifiedComparison]);
