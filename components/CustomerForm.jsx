...
import { UsersIcon, Loader2, Sparkles, Building2, Smartphone, Wallet, Globe, X, Droplets, RefreshCw } from 'lucide-react';
import toast from 'react-hot-toast';
import { getDomainCustomerFields, normalizeKey, resolveDomainFieldKey } from '@/lib/utils/domainHelpers';
import { isWaterHisabRelevant, generateWaterCustomerId } from '@/lib/storefront/waterShopHisab';
                        ...
                        <div className="grid grid-cols-1 gap-x-6 gap-y-4 md:grid-cols-3">
                            ...
                            {isWaterRoute ? (
                                <div className="space-y-1.5 md:col-span-1">
                                    <Label className={cn(MOBILE_LABEL_CLASS, 'font-semibold text-emerald-800 flex items-center justify-between')}>
                                        <span>Account No (ID)</span>
                                        <button
                                            type="button"
                                            onClick={() => {
                                                const newId = generateWaterCustomerId();
                                                handleInputChange('domain_data', { ...formData.domain_data, accountno: newId });
                                            }}
                                            className="text-[10px] text-emerald-600 hover:text-emerald-800 flex items-center gap-1 font-normal"
                                            title="Auto-generate new Customer ID"
                                        >
                                            <RefreshCw className="h-3 w-3" /> Auto ID
                                        </button>
                                    </Label>
                                    <Input 
                                        value={formData.domain_data?.accountno || ''} 
                                        onChange={(e) => handleInputChange('domain_data', { ...formData.domain_data, accountno: e.target.value })} 
                                        placeholder="Auto W-XXXXXX" 
                                        className={cn(MOBILE_INPUT_CLASS, 'h-10 text-[14px] font-mono font-semibold text-emerald-800 bg-emerald-50/50 border-emerald-200')}
                                    />
                                </div>
                            ) : null}
                        ...
                        {isWaterRoute && domainFields.length > 0 && (
                            <div className="grid grid-cols-1 gap-4 rounded-xl border border-sky-200 bg-sky-50/50 p-4 md:grid-cols-2 mt-2">
                                ...
                                {domainFields.map(field => {
                                    const key = resolveDomainFieldKey(field, category) || normalizeKey(field);
                                    return (
                                        <DomainFieldRenderer
                                            key={field}
                                            field={key}
