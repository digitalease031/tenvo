              ...
              <div className="flex items-center justify-between gap-4 p-4 bg-slate-50/80 rounded-2xl border border-slate-100">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500 font-medium mt-1 leading-relaxed">
                    ...
                    {!planHasFeatureWithPackaging(planTier, 'advanced_dashboard', business?.settings) && (
                      <span className="ml-1 text-amber-600 font-semibold">Pro mode requires Professional plan.</span>
                    )}
                  </p>
                </div>
                <Switch
                  checked={isEasyMode}
                  disabled={!planHasFeatureWithPackaging(planTier, 'advanced_dashboard', business?.settings) && !isEasyMode}
                  onCheckedChange={(checked) => {
                    if (!checked && !planHasFeatureWithPackaging(planTier, 'advanced_dashboard', business?.settings)) {
                      return; // block switching to Pro on lower plans
                    }
                    setAppMode(checked ? 'easy' : 'advanced');
                  }}
                />
              </div>
              <div className="flex items-center justify-between gap-4 p-4 bg-slate-50/80 rounded-2xl border border-slate-100">
