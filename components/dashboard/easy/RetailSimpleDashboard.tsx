...
export function RetailSimpleDashboard(props: RetailSimpleDashboardProps) {
  ...
  const routeRelevant = milkRelevant || waterRelevant;
  ...
  const milkOrUnitsAction = routeRelevant ? 'route-hisab' : 'invoices';
    ...
    <div className="w-full min-w-0 space-y-4 overflow-x-hidden bg-gradient-to-b from-neutral-50 via-white to-neutral-50/80 p-0 lg:p-1">
      ...
      <section aria-label="Key metrics">
        ...
        <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-4">
          <RetailKpiBox
            label={waterRelevant ? 'Bottles Delivered' : milkRelevant ? 'Total Milk Sold' : 'Units Sold'}
          ...
          />
          <RetailKpiBox
            label={waterRelevant ? 'Route Orders' : 'Online Orders'}
            value={onlineOrders.toLocaleString()}
            hint={waterRelevant ? 'Storefront + route' : 'Storefront'}
            accent="bg-cyan-500"
            isLoading={salesLoading}
            onClick={canOpenOrders ? () => onQuickAction?.('orders') : undefined}
          />
          <RetailKpiBox
            label={waterRelevant ? 'Route Sales' : 'Online Sales'}
          ...
          <RetailKpiBox
            ...
            hint={waterRelevant ? `${displayOrders} route + invoice` : `${displayOrders} orders`}
          ...
          <RetailKpiBox
            ...
            isLoading={financeLoading}
            onClick={() => onQuickAction?.(waterRelevant ? 'route-hisab' : 'invoices')}
          />
          <RetailKpiBox
            label="Low stock"
            value={String(reminders.lowStock ?? 0)}
            hint={waterRelevant ? 'Bottles to reorder' : 'SKUs to review'}
            accent="bg-amber-500"
      ...
      {/* Shop / Route health — compact second band */}
      <section aria-label={waterRelevant ? 'Route health' : 'Shop health'}>
        <div className="mb-2 px-0.5">
          <h2 className="text-[11px] font-semibold uppercase tracking-widest text-neutral-400">
            {waterRelevant ? 'Route health' : milkRelevant ? 'Route health' : 'Shop health'}
          </h2>
        </div>
        <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        <RetailKpiBox
          ...
          hint={waterRelevant ? 'Unpaid route bills' : 'Open invoices'}
          accent="bg-rose-400"
          onClick={() => onQuickAction?.(waterRelevant ? 'route-hisab' : 'overdue')}
        />
        <RetailKpiBox
          label="Pending orders"
          value={String(reminders.pendingOrders ?? 0)}
          hint={waterRelevant ? 'Unconfirmed orders' : 'Needs attention'}
          accent="bg-orange-400"
          onClick={() => onQuickAction?.('pending-orders')}
        />
        <RetailKpiBox
          label={waterRelevant ? 'Bottle stock' : 'Stock value'}
          value={formatCurrencyCompact(inventoryValue)}
          hint={waterRelevant ? `${inStockUnits.toLocaleString()} units` : `${inStockUnits.toLocaleString()} units`}
          accent="bg-slate-600"
          onClick={() => onQuickAction?.('inventory')}
        />
        <RetailKpiBox
