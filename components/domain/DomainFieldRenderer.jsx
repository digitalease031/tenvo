...
import { getFieldLabel, getFieldInputType, isFieldRequired, getDomainKnowledge, normalizeKey, getSelectOptions, getDomainUnitPreview, resolveDomainFieldKey } from '@/lib/utils/domainHelpers';
...
export function DomainFieldRenderer({
  ...
  const getFieldIcon = () => {
    ...
    if (n.includes('model') || n.includes('serial') || n.includes('account') || n.includes('customer')) return <Hash className="w-3.5 h-3.5" />;
    ...
    if (readOnly) {
      return (
        <Input
          ...
          value={value || ''}
          readOnly
          placeholder={fieldConfig?.placeholder || 'Assigned on save'}
          className="font-mono border-gray-100 bg-gray-50/80 text-gray-700 cursor-default"
        />
      );
    }
    switch (inputType) {
      ...
      default: {
        const datalistId = fieldConfig?.datalist?.length ? `dl-${normalizeKey(field)}` : null;
        return (
          <div className="relative">
            {datalistId && (
              <datalist id={datalistId}>
                {fieldConfig.datalist.map(opt => <option key={opt} value={opt} />)}
              </datalist>
            )}
            <Input
              type="text"
