            ...
            const result = await pool.query(`
                ...
                all_invoices AS (
                    SELECT 
                        ...
                        i.payment_status,
                        GREATEST(
                            COALESCE(i.grand_total, 0) - COALESCE(ip.paid, 0),
                            0
                ...
                all_purchases AS (
                    SELECT 
                        ...
                        p.payment_status,
                        GREATEST(
                            COALESCE(p.total_amount, 0) - COALESCE(pa.paid, 0),
                            0
                        ) AS remaining_payable
                    FROM purchases p
                    LEFT JOIN (
                ...
                SELECT
                    ...
                    -- Receivables (Accounts Receivable outstanding balance truth)
                    (SELECT COALESCE(SUM(remaining_balance), 0) FROM all_invoices 
                    ...
                    (SELECT COALESCE(SUM(remaining_balance), 0) FROM all_invoices 
                     WHERE status = 'overdue' AND remaining_balance > 0.009) as overdue_amount,
                    (SELECT COUNT(*) FROM all_invoices 
                     WHERE status = 'overdue' AND remaining_balance > 0.009) as overdue_count,
                    ...
                    -- Payables (Accounts Payable outstanding balance truth)
                    (SELECT COALESCE(SUM(remaining_payable), 0) FROM all_purchases 
                     WHERE remaining_payable > 0.009
                       AND status != 'cancelled') as total_payables,
    ...
    try {
        ...
        const [
            ...
            categoryScoped
                ...
                : client.query(SALES_UNITS_PERIOD_SQL, [businessId, curFrom, curTo, channel]),
