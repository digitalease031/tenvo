...
export async function createInvoiceAction(params) {
    try {
        ...
        // Purge invoice list cache and dashboard cache for instant consistency
        await Promise.allSettled([
            purgeTenantCacheKey(invoiceData.business_id, 'invoice-list'),
            purgeTenantCacheKey(invoiceData.business_id, 'dashboard'),
        ]);
...
export async function getInvoicesAction(businessId, options = {}) {
    try {
        ...
        const statusKey = Array.isArray(statusIn) ? statusIn.slice().sort().join(',') : '';
        const parseIsoDayStr = (val) => {
            if (!val) return '';
            if (val instanceof Date) return val.toISOString().slice(0, 10);
            return String(val).slice(0, 10);
        };
        const dateFromKey = parseIsoDayStr(dateFrom);
        const dateToKey = parseIsoDayStr(dateTo);
        const cacheSuffix = `${offset}:${limit || 500}:${customerId || ''}:${includeItems ? '1' : '0'}:${dateFromKey}:${dateToKey}:${statusKey}`;
        ...
        const dateFilter = {};
        if (dateFrom) {
            const dFrom = dateFrom instanceof Date ? new Date(dateFrom) : new Date(typeof dateFrom === 'string' && dateFrom.length === 10 ? `${dateFrom}T00:00:00.000` : dateFrom);
            if (!isNaN(dFrom.getTime())) dateFilter.gte = dFrom;
        }
        if (dateTo) {
            let dTo = dateTo instanceof Date ? new Date(dateTo) : new Date(typeof dateTo === 'string' && dateTo.length === 10 ? `${dateTo}T23:59:59.999` : dateTo);
            if (!isNaN(dTo.getTime())) {
                if (dTo.getHours() === 0 && dTo.getMinutes() === 0 && dTo.getSeconds() === 0 && dTo.getMilliseconds() === 0) {
                    dTo.setHours(23, 59, 59, 999);
                }
                const todayLocalStr = new Date().toISOString().slice(0, 10);
                const dateToDayStr = dTo.toISOString().slice(0, 10);
                if (dateToDayStr >= todayLocalStr) {
                    const extendedTo = new Date(Math.max(dTo.getTime(), Date.now() + 86400000));
                    dTo = extendedTo;
                }
                dateFilter.lte = dTo;
            }
        ...
        // Cache list-level reads (header-only) for instant repeat loads
        if (!includeItems) {
...
export async function deleteInvoiceAction(businessId, invoiceId) {
    try {
        ...
        await Promise.allSettled([
            purgeTenantCacheKey(businessId, 'invoice-list'),
            purgeTenantCacheKey(businessId, 'dashboard'),
        ]);
...
export async function updateInvoiceAction(params) {
    try {
        ...
        await Promise.allSettled([
            purgeTenantCacheKey(invoice.business_id, 'invoice-list'),
            purgeTenantCacheKey(invoice.business_id, 'dashboard'),
        ]);
