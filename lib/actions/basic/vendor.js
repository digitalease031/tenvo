...
export async function getVendorsAction(businessId) {
    try {
        ...
        // Join purchases + payment allocations for live vendor outstanding balance (payables) truth
        ...
        try {
            const result = await client.query(
                `SELECT 
                    v.*,
                    COALESCE(
                        COALESCE(v.outstanding_balance, 0) + COALESCE(pur.unpaid_balance, 0),
                        0
                 ...
                 LEFT JOIN (
                     SELECT 
                         ...
                         SUM(
                             GREATEST(
                                 COALESCE(p.total_amount, 0) - COALESCE(alloc.paid, 0),
                                 0
                             )
                         ) AS unpaid_balance
                     FROM purchases p
                     LEFT JOIN (
                         ...
                         WHERE pa.business_id = $1 AND (pay.is_deleted = false OR pay.is_deleted IS NULL)
                         GROUP BY pa.purchase_id
                     ) alloc ON alloc.purchase_id = p.id
                     WHERE p.business_id = $1
                       AND (p.is_deleted = false OR p.is_deleted IS NULL)
                       AND COALESCE(LOWER(p.payment_status), '') <> 'paid'
                       AND COALESCE(LOWER(p.status), '') NOT IN ('cancelled', 'voided', 'draft')
        ...
        try {
            const result = await client.query(
                `SELECT 
                 ...
                 WHERE v.id = $2 AND v.business_id = $1 AND (v.is_deleted IS NULL OR v.is_deleted = false)
                 LIMIT 1`,
                [businessId, vendorId]
            );
