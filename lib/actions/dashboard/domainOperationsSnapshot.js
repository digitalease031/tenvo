...
export async function getDomainOperationsSnapshotAction(businessId, filter = {}) {
  ...
  try {
    ...
    let constructionOps = {
      ...
      qualityPassRate: 0,
      pendingSiteInspections: 0,
      activeSubcontractors: 0,
      subcontractorRetention: 0,
    };
    if (profile.showConstruction) {
      const projectsAgg = await safeQuery(
        client,
        `SELECT
      ...
      const ipcAgg = await safeQuery(
        client,
        `SELECT COUNT(*) AS pending_ipc_count
         FROM interim_payment_certificates
         WHERE business_id = $1::uuid
           AND status IN ('SUBMITTED', 'VERIFIED')`,
        [businessId]
      );
      const boqCount = await safeQuery(
        client,
        `SELECT COUNT(*) AS boq_item_count
         FROM bill_of_quantities_items
         WHERE business_id = $1::uuid`,
        [businessId]
      );
      const machineryHours = await safeQuery(
        client,
      ...
      const safetyAgg = await safeQuery(
        client,
        `SELECT 
      ...
      const qualityAgg = await safeQuery(
        client,
        `SELECT 
      ...
      const inspectionCount = await safeQuery(
        client,
        `SELECT COUNT(*) FILTER (WHERE compliance_status IN ('NON_COMPLIANT', 'CONDITIONAL')) AS pending_count
         FROM construction_site_inspections
         WHERE business_id = $1::uuid`,
        [businessId]
      );
      const subcontractorAgg = await safeQuery(
        client,
        `SELECT 
      ...
      const pa = projectsAgg?.[0] || {};
      const sa = safetyAgg?.[0] || {};
      const qa = qualityAgg?.[0] || {};
      const sca = subcontractorAgg?.[0] || {};
      const totalTests = parseInt(qa.total_tests || 0, 10);
      const passedTests = parseInt(qa.passed_count || 0, 10);
      
      constructionOps = {
        activeProjects: parseInt(pa.active_projects || 0, 10),
        totalProjects: parseInt(pa.total_projects || 0, 10),
        contractValue: parseFloat(pa.contract_value || 0),
        certifiedWork: parseFloat(pa.certified_work || 0),
        retentionHeld: parseFloat(pa.retention_held || 0),
        pendingIpcCount: parseInt(ipcAgg?.[0]?.pending_ipc_count || 0, 10),
        totalBoqItems: parseInt(boqCount?.[0]?.boq_item_count || 0, 10),
        machineryHoursThisMonth: parseFloat(machineryHours?.[0]?.hours_this_month || 0),
        openSafetyIncidents: parseInt(sa.open_count || 0, 10),
        criticalSafetyIncidents: parseInt(sa.critical_count || 0, 10),
        totalQualityTests: totalTests,
        failedQualityTests: parseInt(qa.failed_count || 0, 10),
        qualityPassRate: totalTests > 0 ? parseFloat(((passedTests / totalTests) * 100).toFixed(1)) : 0,
        pendingSiteInspections: parseInt(inspectionCount?.[0]?.pending_count || 0, 10),
        activeSubcontractors: parseInt(sca.active_count || 0, 10),
        subcontractorRetention: parseFloat(sca.retention_total || 0),
      };
    }
    ...
    if (profile.showConstruction) {
      // Real construction KPIs already computed above — no override needed
    }
