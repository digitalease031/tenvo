...
export async function getHubDashboardPeriodExtras(businessId, filter = {}) {
    ...
    try {
        ...
        const curKpiRes = await client.query(SALES_KPI_PERIOD_SQL, [businessId, from, to, 'all']);
        const prevKpiRes = await client.query(SALES_KPI_PERIOD_SQL, [businessId, prevFrom, prevTo, 'all']);
        const curStatusRes = await client.query(SALES_ORDER_STATUS_PERIOD_SQL, [businessId, from, to]);
        const prevStatusRes = await client.query(SALES_ORDER_STATUS_PERIOD_SQL, [businessId, prevFrom, prevTo]);
