...
import { applyDefaultFabricUnitConversions } from '@/lib/utils/fabricUnitConversions';
...
import { safeJsonParam } from '@/lib/utils/jsonUtils';
...
export async function upsertIntegratedProductAction(params) {
    ...
    try {
        ...
        // Auto-apply default fabric unit_conversions for textile verticals with base unit "meter".
        // Reads the business category from DB (one cheap query) to avoid callers needing to pass it.
        if (sanitizedData.unit === 'meter' || sanitizedData.unit === 'metre') {
            try {
                const bizCatRes = await client.query(
                    `SELECT category FROM businesses WHERE id = $1 LIMIT 1`,
                    [businessId]
                );
                const bizCategory = bizCatRes.rows[0]?.category || '';
                applyDefaultFabricUnitConversions(sanitizedData, bizCategory);
            } catch {
                // Non-critical — skip silently if DB query fails for any reason
            }
        }
        ...
        const jsonFields = new Set(['domain_data', 'images', 'unit_conversions', 'batches', 'serial_numbers', 'variants']);

        const formatFieldValue = (field, val) => {
            if (jsonFields.has(field)) {
                const fallback = field === 'images' || field === 'batches' || field === 'serial_numbers' || field === 'variants' ? [] : {};
                return safeJsonParam(val, fallback);
            }
            return val;
        };

        // 1. Upsert Product (Smart Auto-Match + Constraint Resilient)
        let effectiveIsUpdate = Boolean(isUpdate && productId);
        let targetProductId = productId;

        if (!effectiveIsUpdate && businessId) {
            // Check if an active product with matching SKU, barcode, or name already exists for this business
            const cleanSku = (sanitizedData.sku || '').trim();
            const cleanBarcode = (sanitizedData.barcode || '').trim();
            const cleanName = (sanitizedData.name || '').trim();

            if (cleanSku || cleanBarcode || cleanName) {
                const matchRes = await client.query(
                    `SELECT id FROM products
                     WHERE business_id = $1 AND COALESCE(is_deleted, false) = false
                       AND (
                         ($2::text != '' AND LOWER(TRIM(sku)) = LOWER($2::text)) OR
                         ($3::text != '' AND LOWER(TRIM(barcode)) = LOWER($3::text)) OR
                         ($4::text != '' AND LOWER(TRIM(name)) = LOWER($4::text))
                       )
                     ORDER BY 
                       CASE WHEN $2::text != '' AND LOWER(TRIM(sku)) = LOWER($2::text) THEN 1
                            WHEN $3::text != '' AND LOWER(TRIM(barcode)) = LOWER($3::text) THEN 2
                            ELSE 3 END
                     LIMIT 1`,
                    [businessId, cleanSku, cleanBarcode, cleanName]
                );
                if (matchRes.rows.length > 0) {
                    targetProductId = matchRes.rows[0].id;
                    effectiveIsUpdate = true;
                }
            }
        }

        if (effectiveIsUpdate && targetProductId) {
            await assertEntityBelongsToBusiness(client, 'product', targetProductId, businessId);
            finalProductId = targetProductId;

            const fields = Object.keys(sanitizedData).filter(f => f !== 'id' && f !== 'business_id');
            if (fields.length > 0) {
                const values = fields.map(f => formatFieldValue(f, sanitizedData[f]));
                ...
                `, [targetProductId, businessId, ...values]);
            } else {
                await client.query(
                    `UPDATE products SET updated_at = NOW() WHERE id = $1 AND business_id = $2`,
                    [targetProductId, businessId]
                );
            }
        } else {
            // Create logic with duplicate key conflict fallback
            ...
            try {
                ...
                const errStr = String(insertErr?.message || insertErr || '');
                const isDuplicateKey = insertErr?.code === '23505' || errStr.includes('products_business_');
                if (isDuplicateKey) {
                    const cleanName = (sanitizedData.name || '').trim();
                    const cleanSku = (sanitizedData.sku || '').trim();
                    const cleanBarcode = (sanitizedData.barcode || '').trim();
                    ...
                    if (matchRes.rows.length > 0) {
                        finalProductId = matchRes.rows[0].id;
                        const updateFields = Object.keys(sanitizedData).filter(f => f !== 'id' && f !== 'business_id');
                        if (updateFields.length > 0) {
                            const updateValues = updateFields.map(f => formatFieldValue(f, sanitizedData[f]));
                            const setClause = updateFields.map((f, i) => `${f} = $${i + 3}`).join(', ');
                            await client.query(`
                                UPDATE products 
                                SET ${setClause}, updated_at = NOW() 
                                WHERE id = $1 AND business_id = $2
                            `, [finalProductId, businessId, ...updateValues]);
                        }
                    } else if (errStr.includes('products_business_name_active_key')) {
                        throw new Error(`A product named "${sanitizedData.name || 'this'}" already exists in your inventory. Please update the existing product or use a unique name.`);
                    } else if (errStr.includes('products_business_sku_active_key')) {
                        throw new Error(`A product with SKU "${sanitizedData.sku}" already exists in your inventory. Please use a unique SKU.`);
                    } else if (errStr.includes('products_business_barcode_active_key')) {
                        throw new Error(`A product with barcode "${sanitizedData.barcode}" already exists in your inventory. Please use a unique barcode.`);
                    } else {
                        throw insertErr;
                    }
                } else {
        ...
        const product = await ProductService.getProduct(finalProductId, businessId, client);
