      ...
      .map((c) => {
        ...
        for (const p of hisabProducts) {
          ...
          if (line) {
            ...
            if (line.product_name_snapshot && line.product_name_snapshot.includes('(')) {
              const match = line.product_name_snapshot.match(/\(([^)]+)\)/);
              if (match?.[1]) {
                breadTypes[pid] = match[1];
              }
            }
    ...
    await prismaBase.$transaction(async (tx) => {
      for (const row of rows) {
        ...
        for (const [productId, rawQty] of Object.entries(qtyMap)) {
          ...
          const breadType = row.breadTypes?.[pid] || row.breadTypes?.[productId] || '';
          const nameSnapshot = breadType ? `${product.name} (${breadType})` : product.name;
          lineCreates.push({
            business_id: businessId,
