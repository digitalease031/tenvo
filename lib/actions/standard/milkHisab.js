...
export async function getMilkHisabDayAction({ businessId, category, deliveryDate }) {
  try {
    ...
    const rows = customers
      .map((c) => {
        ...
        for (const p of hisabProducts) {
          ...
          const customPrice = prefs.productPrices?.[pid]
            ? Number(prefs.productPrices[pid])
            : pid === String(milkProductId) && prefs.customMilkRate > 0
              ? prefs.customMilkRate
              : null;
          let unitPrice = customPrice ?? defaultPrice;
          if (line) {
            qty = Number(line.quantity) || 0;
            unitPrice = Number(line.unit_price_snapshot) || customPrice || defaultPrice;
          } else if (!stop && milkProductId && pid === String(milkProductId) && prefs.dailyMilkKg > 0) {
            qty = prefs.dailyMilkKg;
          }
          if (qty > 0 && Math.abs(unitPrice - defaultPrice) > 0.01) {
            qtyByProduct[pid] = `${qty}*${unitPrice}`;
          } else {
            qtyByProduct[pid] = qty || '';
          }
          dayAmount += qty * unitPrice;
        }
        return {
          customerId: cid,
          customerName: c.name,
    ...
    const delivered = rows.filter((r) =>
      Object.values(r.qtyByProduct || {}).some((q) => {
        const { qty } = parseQtyCalculatorInput(q);
        return qty > 0;
      })
    ).length;
    ...
    await prismaBase.$transaction(async (tx) => {
      for (const row of rows) {
        ...
        for (const [productId, rawQty] of Object.entries(qtyMap)) {
          ...
          const { qty, price } = parseQtyCalculatorInput(rawQty, product.price);
          ...
          lineCreates.push({
            ...
            quantity: qty,
            unit_price_snapshot: Number(price) || 0,
          });
        }
