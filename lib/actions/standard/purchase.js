...
async function resolvePrimaryWarehouseId(client, businessId) {
...
async function receivePurchaseLineStock({
    ...
    if (item.product_id && unitCost > 0) {
        await client.query(
            `UPDATE products
             SET cost_price = $1, updated_at = NOW()
             WHERE id = $2 AND business_id = $3 AND COALESCE(cost_price, 0) != $1`,
            [unitCost, item.product_id, businessId]
        );
    }
...
export async function getPurchaseByIdAction(businessId, purchaseId) {
    try {
        ...
        try {
            let headerResult;
            if (businessId) {
                await checkAuth(businessId, 'purchases.view', client);
                headerResult = await client.query(`
                    SELECT 
                        p.*,
                        v.name as vendor_name,
                        v.email as vendor_email,
                ...
                if (headerResult.rows.length > 0) {
                    const foundBid = headerResult.rows[0].business_id;
                    await checkAuth(foundBid, 'purchases.view', client);
                }
            }
            ...
            const effectiveBid = businessId || purchase.business_id;
            ...
            `, [purchaseId, effectiveBid]);

            purchase.items = itemsResult.rows;

            return actionSuccess(serializeDecimalsDeep({ purchase }));
        } finally {
            client.release();
        }
    } catch (error) {
...
export async function createPurchaseAction(purchaseData) {
    ...
    try {
        ...
        if (!header.warehouse_id) {
            header.warehouse_id = await resolvePrimaryWarehouseId(client, header.business_id);
        }
...
export async function updatePurchaseStatusAction(businessId, purchaseId, status) {
    ...
    try {
        ...
        if (newStatus === PURCHASE_STATUSES.RECEIVED && !purchase.warehouse_id) {
            const primaryWarehouseId = await resolvePrimaryWarehouseId(client, businessId);
            if (primaryWarehouseId) {
                purchase.warehouse_id = primaryWarehouseId;
                await client.query(
                    `UPDATE purchases SET warehouse_id = $1 WHERE id = $2 AND business_id = $3`,
                    [primaryWarehouseId, purchaseId, businessId]
                );
            } else {
                await client.query('ROLLBACK');
        ...
        return actionSuccess(serializeDecimalsDeep({ purchase: result.rows[0] }));
    } catch (error) {
        await client.query('ROLLBACK');
...
export async function createBulkPurchaseOrdersAction(businessId, orders) {
    ...
    try {
        ...
        for (const orderData of orders) {
            ...
            let targetWarehouseId = orderData.warehouse_id || null;
            if (targetWarehouseId) {
                await assertEntityBelongsToBusiness(client, 'warehouse', targetWarehouseId, businessId);
            } else {
                targetWarehouseId = await resolvePrimaryWarehouseId(client, businessId);
            }
