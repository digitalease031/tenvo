...
export async function getQuotationsAction(businessId, options = {}) {
    ...
    try {
        ...
        return actionSuccess({
            quotations: serializeDecimalsDeep(result.rows),
            limit,
            offset,
    ...
    try {
        ...
        if (items && items.length > 0) {
            for (const item of items) {
                ...
                const total = item.total_amount ? Number(item.total_amount) : (qty * price);

                await client.query(`
                    INSERT INTO quotation_items (
                        quotation_id, business_id, product_id,
                        quantity, unit_price, total_amount,
                        description, batch_number, serial_number
        ...
        return actionSuccess({ quotation: serializeDecimalsDeep(quotation) });
...
export async function getQuotationDetailAction(quotationId, businessId) {
    ...
    try {
        ...
        return actionSuccess({
            quotation: serializeDecimalsDeep({
                ...headerResult.rows[0],
...
export async function getSalesOrdersAction(businessId, options = {}) {
    ...
    try {
        ...
        return actionSuccess({
            salesOrders: serializeDecimalsDeep(result.rows),
            limit,
            offset,
            hasMore: result.rows.length >= limit,
        });
...
export async function createSalesOrderAction(orderData) {
    ...
    try {
        ...
        return actionSuccess({ salesOrder: serializeDecimalsDeep(salesOrder) });
...
export async function getSalesOrderDetailAction(orderId, businessId) {
    ...
    try {
        ...
        return actionSuccess({
            salesOrder: serializeDecimalsDeep({
                ...headerResult.rows[0],
                items: itemsResult.rows
            })
        });
...
export async function getChallansAction(businessId, options = {}) {
    ...
    try {
        ...
        return actionSuccess({
            challans: serializeDecimalsDeep(result.rows),
            limit,
            offset,
            hasMore: result.rows.length >= limit,
        });
...
export async function createChallanAction(challanData) {
    ...
    try {
        ...
        return actionSuccess({ challan: serializeDecimalsDeep(challan) });
