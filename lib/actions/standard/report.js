...
export async function getAccountingSummaryAction(businessId, startDate, endDate, options = {}) {
    ...
    try {
        ...
        try {
            ...
            let accountsReceivable = getNetForCode(ACCOUNT_CODES.ACCOUNTS_RECEIVABLE);
            if (accountsReceivable === 0) {
                const arRes = await client.query(`
                    SELECT COALESCE(SUM(calculate_invoice_balance(id)), 0) as total_ar
                    FROM invoices
                    WHERE business_id = $1 AND status IN ${PENDING_INVOICE_STATUS_SQL_IN} AND (is_deleted = false OR is_deleted IS NULL)
                `, [businessId]);
                accountsReceivable = parseFloat(arRes.rows[0]?.total_ar || 0);
            }

            let accountsPayable = getNetForCode(ACCOUNT_CODES.ACCOUNTS_PAYABLE);
            if (accountsPayable === 0) {
                const apRes = await client.query(`
                    SELECT COALESCE(SUM(p.total_amount - COALESCE(alloc.paid, 0)), 0) as total_ap
                    FROM purchases p
                    LEFT JOIN (
                        SELECT pa.purchase_id, SUM(pa.amount)::numeric AS paid
                        FROM payment_allocations pa
                        INNER JOIN payments pay ON pay.id = pa.payment_id AND pay.business_id = pa.business_id
                        WHERE pa.purchase_id IS NOT NULL AND pa.business_id = $1 AND (pay.is_deleted = false OR pay.is_deleted IS NULL)
                        GROUP BY pa.purchase_id
                    ) alloc ON alloc.purchase_id = p.id
                    WHERE p.business_id = $1 AND p.status IN ('draft', 'ordered', 'pending', 'approved', 'sent', 'received') AND COALESCE(LOWER(p.payment_status), '') <> 'paid' AND (p.is_deleted = false OR p.is_deleted IS NULL)
                `, [businessId]);
                accountsPayable = parseFloat(apRes.rows[0]?.total_ap || 0);
            }

            let inventoryValue = getNetForCode(ACCOUNT_CODES.INVENTORY_ASSET);
            if (inventoryValue === 0) {
                const invRes = await client.query(`
                    SELECT COALESCE(SUM(stock * cost_price), 0) as total_inv
                    FROM products
                    WHERE business_id = $1 AND stock > 0 AND (is_deleted = false OR is_deleted IS NULL)
                `, [businessId]);
                inventoryValue = parseFloat(invRes.rows[0]?.total_inv || 0);
            }

            const totalRevenue = getNetForType('income') + getNetForType('revenue');

            // Aggregate all COGS family accounts (5000, 5001, 5002)
            const cogsCodeSet = new Set(COGS_CODES);
            const totalCOGS = rows
                .filter(r => cogsCodeSet.has(r.code))
                .reduce((sum, match) => {
                    const debits = parseFloat(match.total_debit);
                    const credits = parseFloat(match.total_credit);
