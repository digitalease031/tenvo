        ...
        try {
            ...
            if (accountsPayable === 0) {
                const apRes = await client.query(`
                    SELECT COALESCE(SUM(total_amount - COALESCE(paid_amount, 0)), 0) as total_ap
                    FROM purchases
                    WHERE business_id = $1 AND status IN ('draft', 'ordered', 'pending', 'approved', 'sent') AND (is_deleted = false OR is_deleted IS NULL)
                `, [businessId]);
