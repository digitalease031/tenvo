...
/**
 * Get restaurant order history (completed & active with filters)
 */
export async function getOrderHistoryAction(businessId, filters = {}) {
    const client = await pool.connect();
    try {
        await checkAuth(businessId, client, 'restaurant.view_tables', 'restaurant_kds');
        ...
        if (filters.date) {
            query += ` AND DATE(ro.created_at) = $${idx}::date`;
            params.push(filters.date);
            idx++;
        }

        query += ` GROUP BY ro.id, rt.table_number, rt.section, c.name ORDER BY ro.created_at DESC LIMIT 100`;
