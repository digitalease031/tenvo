...
/**
 * If a water-delivery business has no active products yet (existing accounts registered
 * before the catalog seed was wired), seed the two core 19L hisab SKUs so the route
 * sheet has columns immediately. Fire-and-forget, idempotent — checks by SKU before insert.
 * @param {string} businessId
 * @param {import('@prisma/client').PrismaClient | object} [db]
 */
async function ensureWaterHisabCoreProducts(businessId, db = prismaBase) {
  try {
    for (const sku of WATER_HISAB_CORE_SKUS) {
      const existing = await db.products.findFirst({
        where: { business_id: businessId, sku: sku.sku, is_deleted: false },
        select: { id: true },
      });
      if (existing) continue;

      // Upsert the category first
      await db.product_categories.upsert({
        where: { business_id_name: { business_id: businessId, name: sku.category } },
        create: {
          business_id: businessId,
          name: sku.category,
          slug: sku.category.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, ''),
          is_active: true,
          sort_order: 0,
        },
      ...
      await db.products.create({
        data: {
          ...
          stock: sku.stock,
          sku: sku.sku,
          is_featured: Boolean(sku.is_featured),
          is_active: true,
          is_deleted: false,
          domain_data: sku.domain_data || {},
          slug: sku.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 80),
        },
      });
    }
  } catch (seedErr) {
    console.warn('[ensureWaterHisabCoreProducts] non-critical seed error:', seedErr?.message);
  }
}
...
export async function getWaterHisabDayAction({ businessId, category, deliveryDate }) {
  try {
    ...
    // NOTE: Core product seeding moved to registration flow and one-time migrations.
    // Legacy accounts registered before catalog seeding was wired should run a manual
    // migration or seed products via the inventory UI. This hot-path seed was removed
    // to improve first-load performance (saves ~200-300ms).
    let resolvedProducts = products;

    const hisabProducts = resolveWaterHisabProducts(resolvedProducts, business.settings || {});
    
    // Fire-and-forget: ensure customer IDs are populated without blocking the sheet load
    // Only run this once per day per business to reduce DB load
    const lastBackfillKey = `water-customer-id-backfill:${businessId}`;
    const shouldBackfill = () => {
      if (typeof window === 'undefined') return true; // Server-side: always attempt
      try {
        const last = localStorage.getItem(lastBackfillKey);
        if (!last) return true;
        const lastDate = new Date(last).toISOString().split('T')[0];
        const today = toWaterHisabDateKey(new Date());
    ...
    if (shouldBackfill()) {
      void ensureWaterCustomerIds(businessId, customers).then(() => {
    ...
    // Self-healing backfill for existing accounts registered before the waterHisab
    // settings seed was added. If settings.waterHisab is absent, write the defaults
    // once so subsequent reads are durable and the 19L DEL+REC columns appear consistently.
    if (!business.settings?.waterHisab) {
      void (async () => {
        try {
          const current = await prismaBase.businesses.findFirst({
            where: { id: businessId },
            select: { settings: true },
          });
          if (current && !current.settings?.waterHisab) {
            const next = {
          ...
          console.warn('[getWaterHisabDayAction] waterHisab settings backfill failed (non-critical):', backfillErr?.message);
        }
      })();
    }
...
export async function saveWaterHisabSheetSettingsAction({ 
  ...
  try {
    ...
    // When enabledSizeIds is not provided, preserve the existing stored value rather than
    // overwriting with the hardcoded default. This prevents partial saves from clobbering
    // multi-size configurations that the owner already set.
    const existingSettings = business.settings && typeof business.settings === 'object' ? business.settings : {};
    const existingWaterHisab = existingSettings.waterHisab && typeof existingSettings.waterHisab === 'object' ? existingSettings.waterHisab : {};
    const fallbackSizes = Array.isArray(existingWaterHisab.enabledSizeIds) && existingWaterHisab.enabledSizeIds.length
      ? existingWaterHisab.enabledSizeIds
      : [...WATER_HISAB_DEFAULT_ENABLED_SIZES];
...
export async function getWaterHisabPeriodSummaryAction({ businessId, category, period }) {
  try {
    ...
    // Optimized: Single SQL query with CTE aggregation instead of client-side loops
    const result = await pool.query(
      `
      WITH line_aggregates AS (
        SELECT
          s.customer_id,
          l.product_id,
          ...
          SUM(l.received_quantity) as total_received
        FROM water_delivery_stops s
        INNER JOIN water_delivery_lines l ON l.stop_id = s.id AND l.business_id = s.business_id
        WHERE s.business_id = $1
          AND s.is_deleted = false
          AND s.delivery_date >= $2::date
          AND s.delivery_date <= $3::date
          AND l.product_id IS NOT NULL
        GROUP BY s.customer_id, l.product_id, l.product_name_snapshot, 
                 l.unit_snapshot, l.unit_price_snapshot
      ),
      stop_aggregates AS (
        SELECT
          s.customer_id,
          ...
          s.house_no_snapshot,
          s.account_no_snapshot,
          s.town_code_snapshot,
          COUNT(DISTINCT s.id) as stop_count,
          SUM(s.cash_collected) as total_cash,
          SUM(s.special_discount) as total_discount
        FROM water_delivery_stops s
        WHERE s.business_id = $1
          AND s.is_deleted = false
          AND s.delivery_date >= $2::date
          AND s.delivery_date <= $3::date
        GROUP BY s.customer_id, s.customer_name_snapshot, s.house_no_snapshot, 
                 s.account_no_snapshot, s.town_code_snapshot
      ),
      product_aggregates AS (
        SELECT
          customer_id,
          SUM(total_quantity) as del_total,
          SUM(total_received) as rec_total,
          jsonb_object_agg(
            product_id::text,
            total_quantity
          ) as qty_by_product,
        ...
        WHERE total_quantity > 0 OR total_received > 0
      ...
      customer_prefs AS (
        ...
        WHERE c.business_id = $1
          ...
          AND c.id IN (SELECT customer_id FROM stop_aggregates)
      ),
      invoices_filtered AS (
        ...
        WHERE i.business_id = $1
          ...
          AND i.notes LIKE '%water_hisab_period%'
      )
      SELECT
        sa.customer_id,
        sa.customer_name_snapshot as customer_name,
        sa.house_no_snapshot as house_no,
        sa.account_no_snapshot as account_no,
        sa.town_code_snapshot as town_code,
        cp.domain_data,
        sa.stop_count,
        sa.total_cash as cash_collected,
        sa.total_discount as special_discount,
        COALESCE(pa.del_total, 0) as del_total,
        COALESCE(pa.rec_total, 0) as rec_total,
        COALESCE(pa.qty_by_product, '{}'::jsonb) as qty_by_product,
        COALESCE(pa.product_meta, '{}'::jsonb) as product_meta,
        i.id as invoice_id,
        i.invoice_number,
        i.payment_status as invoice_payment_status,
        i.notes as invoice_notes
      FROM stop_aggregates sa
      LEFT JOIN customer_prefs cp ON cp.customer_id = sa.customer_id
      LEFT JOIN product_aggregates pa ON pa.customer_id = sa.customer_id
      LEFT JOIN invoices_filtered i ON i.customer_id = sa.customer_id
        AND i.notes LIKE '%' || $4 || '%'
      ORDER BY sa.house_no_snapshot NULLS LAST, sa.customer_name_snapshot
      `,
      [businessId, startIso, endIso, periodKey]
    );

    const rows = result.rows.map((row) => {
      const prefs = readWaterCustomerPrefs({ domain_data: row.domain_data || {} });
      const qtyByProduct = row.qty_by_product || {};
      const productMeta = row.product_meta || {};
      
      // Calculate amount from product metadata
      let amount = 0;
      Object.entries(productMeta).forEach(([pid, meta]) => {
        const qty = Number(meta.qty) || 0;
        const price = Number(meta.price) || 0;
        amount += qty * price;
      });
      amount = Math.max(0, Math.round((amount - (Number(row.special_discount) || 0)) * 100) / 100);

      const billed = Boolean(row.invoice_id);
      const hisabPaymentStatus = readWaterHisabPeriodPayment(row.domain_data, periodKey);
      const paymentStatus = resolveWaterHisabRowPaymentStatus({
        invoicePaymentStatus: row.invoice_payment_status || null,
        hisabPaymentStatus,
        billed,
      });

      const qtySum = Object.values(qtyByProduct).reduce((a, b) => a + (Number(b) || 0), 0);
      const delTotal = Number(row.del_total) > 0 ? Number(row.del_total) : qtySum;
      const recTotal = Number(row.rec_total) || 0;
      ...
      return {
        ...
        delTotal,
        recTotal,
        cashCollected: Math.round((Number(row.cash_collected) || 0) * 100) / 100,
        specialDiscount: Math.round((Number(row.special_discount) || 0) * 100) / 100,
        qtyByProduct,
        productMeta,
        amount,
        ...
        hisabPaymentStatus,
        billed,
        billedPeriod: billed ? (String(row.invoice_notes || '').match(/\[water_hisab_period=([^\]]+)\]/)?.[1] || periodKey) : null,
        deliveryDays: prefs.deliveryDays || 'Daily',
        routeLabel: prefs.routeLabel || '',
      };
    }).filter((r) => r.amount > 0 || r.billed);

    // Enrich product columns from catalog (reuse cached products if available)
    ...
    const allProducts = await prismaBase.products.findMany({
      where: { 
        business_id: businessId, 
        is_deleted: false,
        id: { in: [...productIds] } // Only fetch products that are actually used
      },
      select: { id: true, name: true, unit: true, price: true, category: true, domain_data: true },
    });
...
export async function getWaterHisabCustomerDayBreakdownAction({
  ...
  try {
    ...
    // Add stops and products to breakdown so buildWaterPeriodPrintModel can use them
    breakdown.stops = stops;
    breakdown.products = columns.map((col) => ({
      id: col.id,
      name: col.name,
      unit: col.unit || 'pcs',
    ...
    breakdown.startIso = startIso;
    breakdown.endIso = endIso;
...
export async function getWaterHisabBulkDayBreakdownAction({ businessId, category, period }) {
  try {
    ...
    for (const [cid, custStops] of byCustomer) {
      ...
      // Add stops and products to breakdown for buildWaterPeriodPrintModel
      breakdown.stops = custStops;
      breakdown.products = columns.map((col) => ({
        id: col.id,
        name: col.name,
        unit: col.unit || 'pcs',
