    ...
    const result = await pool.query(
      ...
      WITH stop_aggregates AS (
        SELECT
          ...
          jsonb_object_agg(
            COALESCE(l.product_id::text, 'none'),
            COALESCE(SUM(l.quantity), 0)
          ) FILTER (WHERE l.product_id IS NOT NULL) as qty_by_product,
          jsonb_object_agg(
            l.product_id::text,
            jsonb_build_object(
              'name', l.product_name_snapshot,
              'unit', l.unit_snapshot,
              'price', l.unit_price_snapshot,
              'qty', SUM(l.quantity)
            )
          ) FILTER (WHERE l.product_id IS NOT NULL AND l.quantity > 0) as product_meta
        FROM water_delivery_stops s
        LEFT JOIN water_delivery_lines l ON l.stop_id = s.id AND l.business_id = s.business_id
