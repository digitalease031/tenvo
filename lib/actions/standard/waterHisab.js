...
export async function getWaterHisabDayAction({ businessId, category, deliveryDate }) {
  try {
    ...
    // If this business has no active products yet (existing account registered before catalog
    // seeding was wired), seed the core 19L SKUs synchronously so the route sheet shows
    // columns on this very first load — no refresh needed.
