...
export async function getWaterHisabDayAction({ businessId, category, deliveryDate }) {
  try {
    ...
    await ensureWaterCustomerIds(businessId, customers);
      ...
      for (const row of rows) {
        ...
        if (!hasActivity) {
          ...
          if (existing) {
            ...
            // Reverse today's bottle delta so clearing a stop does not leave inflated BAL.
        ...
        const existingStop = await tx.water_delivery_stops.findFirst({
          where: {
            business_id: businessId,
            customer_id: customerId,
            delivery_date: new Date(dateKey),
        ...
        // Idempotent: apply DEL/REC against opening (pre-today) balance, not stored BAL.
        // Customer ID (accountno) is system-owned — never overwrite from sheet edits.
        const nextHouse = String(row.houseNo || '').trim();
        const nextRoute = String(row.routeLabel || '').trim();
 ...
 */
export async function saveWaterHisabSheetSettingsAction({ businessId, category, enabledSizeIds }) {
  try {
    assertWaterHisab(category);
    ...
    return await actionSuccess({ enabledSizeIds: nextSizes });
  } catch (e) {
