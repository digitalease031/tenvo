...
export async function fetchStorefrontCategoriesOnClient(client, businessId) {
  ...
  try {
    ...
    try {
      ...
      if (firstErr?.message?.includes('Connection terminated') || firstErr?.code === '57P01' || firstErr?.code === '08006') {
        console.warn('[getCategories] Transient pooler disconnect, retrying category query...');
        result = await runQuery();
      } else {
        throw firstErr;
      }
