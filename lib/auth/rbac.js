...
import { isPlatformLevel, isPlatformOwner, isPartnerUser } from '@/lib/config/platform';

async function isDemoBusiness(identifier) {
    ...
    try {
        const query = isUuid
            ? `SELECT is_demo_requested, domain, settings FROM businesses WHERE id = $1`
            : `SELECT is_demo_requested, domain, settings FROM businesses WHERE domain = $1`;
        const res = await client.query(query, [str]);
        if (res.rows.length === 0) {
            return str.startsWith('demo-') || str.endsWith('-demo');
        }
        const row = res.rows[0];
        return Boolean(
            row.settings?.is_demo_store === true ||
            row.settings?.is_demo === true ||
            row.is_demo_requested === true ||
            row.domain?.startsWith('demo-') ||
            row.domain?.endsWith('-demo')
        );
    } catch {
        return str.startsWith('demo-') || str.endsWith('-demo');
    } finally {
 ...
 * Deduplicated per RSC request context via React cache().
 ...
 */
async function getUserBusinessRole(userId, identifier) {
    ...
    const client = await pool.connect();
    try {
        let actualBusinessId = str;
        if (!isUuid) {
            const bizRes = await client.query(`SELECT id FROM businesses WHERE domain = $1 LIMIT 1`, [str]);
            if (bizRes.rows.length === 0) return null;
            actualBusinessId = bizRes.rows[0].id;
        }

        const res = await client.query(
            `SELECT role FROM business_users 
       WHERE user_id = $1 AND business_id = $2::uuid AND status = 'active'`,
            [userId, actualBusinessId]
        );
...
async function buildAuthContext(businessId) {
    ...
    const partnerUser = isPartnerUser(session.user);
    ...
    if (platformAdmin) {
        ...
        if (!role && partnerUser && businessId) {
            isDemoAccess = await isDemoBusiness(businessId);
            if (isDemoAccess) {
                role = 'manager';
            }
        }
