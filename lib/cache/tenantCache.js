...
/**
 * Purge a specific tenant cache entry or all entries of a kind for a tenant.
 * @param {string} businessId
 * @param {TenantCacheKind} kind
 * @param {string} [suffix]
 * @returns {Promise<boolean>}
...
export async function purgeTenantCacheKey(businessId, kind, suffix = '') {
  ...
  try {
    ...
    // Delete known hot keys immediately
    const exactKey = tenantCacheKey(businessId, kind, '');
    await Promise.allSettled([
      redisDel(exactKey),
      redisDel(tenantCacheKey(businessId, kind, '0:500::0')),
      redisDel(tenantCacheKey(businessId, kind, '0:500::1')),
      redisDel(tenantCacheKey(businessId, kind, '0:500::0:0')),
      redisDel(tenantCacheKey(businessId, kind, '0:500::1:0')),
    ]);

    // Fire pattern scans asynchronously in background (don't block mutation response)
    const patternKey = tenantCacheKey(businessId, kind, '*');
    const broadPatternKey = `${exactKey}*`;
    void Promise.allSettled([
      redisDelByPattern(patternKey),
      redisDelByPattern(broadPatternKey),
    ]);
