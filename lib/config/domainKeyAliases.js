/**
 * Maps registration / demo slugs to canonical domainKnowledge keys.
 * Keeps hub URLs like `textile` wired to the correct vertical preset.
 */
export const DOMAIN_KEY_ALIASES = Object.freeze({
  textile: 'textile-wholesale',
  apparel: 'garments',
  clothing: 'garments',
  'clothing-store': 'garments',
  restaurant: 'restaurant-cafe',
  cafe: 'restaurant-cafe',
  'hardware-store': 'hardware-sanitary',
  hardware: 'hardware-sanitary',
  electronics: 'electronics-goods',
  'mobile-phone-shop': 'mobile',
  clinic: 'dental-clinic',
  salon: 'salon-spa',
  'beauty-salon': 'salon-spa',
  'spa-wellness': 'salon-spa',
  'medical-clinic': 'clinics-healthcare',
  spa: 'salon-spa',
  bakery: 'bakery-confectionery',
  milk: 'milk-shop',
  milkshop: 'milk-shop',
  'doodh-shop': 'milk-shop',
  'dairy-shop': 'milk-shop',
  water: 'water-delivery',
  'water-shop': 'water-delivery',
  'water-supply': 'water-delivery',
  'water-bottles': 'water-delivery',
  'bottled-water': 'water-delivery',
  'aqua-delivery': 'water-delivery',
  'mineral-water': 'water-delivery',
  'water-rider': 'water-delivery',
  grocery: 'supermarket',
  jewellery: 'gems-jewellery',
  jewelry: 'gems-jewellery',
  tiles: 'ceramics-tiles',
  ceramics: 'ceramics-tiles',
  'tiles-marble': 'ceramics-tiles',
  marble: 'ceramics-tiles',
  'marble-stone': 'ceramics-tiles',
  'natural-stone': 'ceramics-tiles',
  'tile-shop': 'ceramics-tiles',
  boutique: 'boutique-fashion',
  vet: 'veterinary-clinic',
  veterinary: 'veterinary-clinic',
  dealership: 'vehicle-dealership',
  vincar: 'vehicle-dealership',
  sehgal: 'vehicle-dealership',
  'sehgal-motorsports': 'vehicle-dealership',
  showroom: 'vehicle-dealership',
  'tenvo-vehicles': 'vehicle-dealership',
  'car-dealer': 'vehicle-dealership',
  'auto-dealer': 'vehicle-dealership',
  'car-marketplace': 'auto-marketplace',
  sgcarmart: 'auto-marketplace',
  'auto-marketplace': 'auto-marketplace',
  'fitness-gym': 'gym-fitness',
  gym: 'gym-fitness',
  marine: 'marine-parts',
  'marine-spare-parts': 'marine-parts',
  'ships-parts': 'marine-parts',
  'ship-spare-parts': 'marine-parts',
  'maritime-parts': 'marine-parts',
  'tenvo-marine': 'marine-parts',
  tyre: 'tyre-shop',
  tire: 'tyre-shop',
  tyres: 'tyre-shop',
  tires: 'tyre-shop',
  'tyre-store': 'tyre-shop',
  'tire-shop': 'tyre-shop',
  'tire-store': 'tyre-shop',
  footwear: 'leather-footwear',
  shoes: 'leather-footwear',
  sneakers: 'leather-footwear',
  'shoe-store': 'leather-footwear',
  'shoe-shop': 'leather-footwear',
  'sneaker-store': 'leather-footwear',
  construction: 'construction-contractor',
  contractor: 'construction-contractor',
  builder: 'construction-contractor',
  'civil-engineering': 'construction-contractor',
  infrastructure: 'construction-contractor',
  epc: 'construction-contractor',
  'construction-projects': 'construction-contractor',
  'public-works': 'construction-contractor',
  'infra-contractor': 'construction-contractor',
  'building-construction': 'construction-contractor',
  'road-construction': 'construction-contractor',
  'bridge-construction': 'construction-contractor',
  ev: 'ev-bikes',
  'ev-bike': 'ev-bikes',
  'ev-bikes': 'ev-bikes',
  'electric-bike': 'ev-bikes',
  'electric-bikes': 'ev-bikes',
  'electric-scooter': 'ev-bikes',
  'electric-scooters': 'ev-bikes',
  vlektra: 'ev-bikes',
  ramza: 'ev-bikes',
  'ramza-aima': 'ev-bikes',
  metroev: 'ev-bikes',
  'metro-ev': 'ev-bikes',
  'ev-dealership': 'ev-bikes',
  'ev-showroom': 'ev-bikes',
  'tenvo-ev': 'ev-bikes',
  'e-bike': 'ev-bikes',
  'e-bikes': 'ev-bikes',
  'e-scooter': 'ev-bikes',
  'e-scooters': 'ev-bikes',
  oil: 'lubricant-distribution',
  lubricant: 'lubricant-distribution',
  lubricants: 'lubricant-distribution',
  'engine-oil': 'lubricant-distribution',
  'pakistan-oil': 'lubricant-distribution',
  'pakistan-engine-oil': 'lubricant-distribution',
  'lubricant-distribution': 'lubricant-distribution',
  'oil-wholesaler': 'lubricant-distribution',
  'lubricant-wholesaler': 'lubricant-distribution',
});

/**
 * @param {string} category
 * @returns {string}
 */
export function resolveDomainKey(category) {
  const key = String(category || '').trim().toLowerCase();
  if (!key) return 'retail-shop';
  if (DOMAIN_KEY_ALIASES[key]) return DOMAIN_KEY_ALIASES[key];
  if (key.includes('construction') || key.includes('contractor') || key.includes('constructors')) {
    return 'construction-contractor';
  }
  if (key.includes('ev-') || key.includes('electric-bike') || key.includes('vlektra') || key.includes('metroev') || key.includes('ramza')) {
    return 'ev-bikes';
  }
  if (key.includes('lubricant') || key.includes('engine-oil') || key.includes('pakistan-oil')) {
    return 'lubricant-distribution';
  }
  return key;
}

/**
 * @param {string} category
 * @returns {boolean}
 */
export function isKnownDomainKey(category) {
  const resolved = resolveDomainKey(category);
  return resolved !== 'retail-shop' || category === 'retail-shop' || category === '';
}
