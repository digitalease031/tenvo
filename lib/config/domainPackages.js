/**
 * Domain-specific commercial packages — presets for vertical SKUs.
 *
 * Each package combines:
 * - supported vertical keys (`garments`, `textile-wholesale`, …)
 * - recommended SaaS plan tier + optional limit bumps
 * - optional `settings.packaging` custom feature overrides
 * - marketing metadata for `/solutions/*` pages
 *
 * Runtime access still flows through `planHasFeatureWithPackaging` and plan limits;
 * domain packages are applied at registration or by platform admin.
 *
 * @see docs/MODULAR_PACKAGING_AND_DASHBOARD_MATRIX.md
 * @see lib/subscription/effectivePlanAccess.js
 */

import { PLAN_TIERS, resolvePlanTier } from '@/lib/config/plans';
import { resolveDomainKey } from '@/lib/config/domainKeyAliases';
import { mergePackagingIntoBusinessSettings } from '@/lib/utils/businessPackagingSettings';
import { applyRegistrationVerticalPackaging } from '@/lib/onboarding/registrationVerticalPackaging.js';
import {
  CLOTHING_COMMERCE_FEATURE_OVERRIDES,
  PHARMACY_COMMERCE_FEATURE_OVERRIDES,
  AUTO_PARTS_COMMERCE_FEATURE_OVERRIDES,
  VEHICLE_SHOWROOM_FEATURE_OVERRIDES,
  FURNITURE_COMMERCE_FEATURE_OVERRIDES,
  FITNESS_COMMERCE_FEATURE_OVERRIDES,
  MILK_COMMERCE_FEATURE_OVERRIDES,
  WATER_COMMERCE_FEATURE_OVERRIDES,
  CONSTRUCTION_MANAGEMENT_FEATURE_OVERRIDES,
  EV_BIKES_COMMERCE_FEATURE_OVERRIDES,
  LUBRICANT_DISTRIBUTION_FEATURE_OVERRIDES,
  HARDWARE_COMMERCE_FEATURE_OVERRIDES,
} from '@/lib/config/domainPackageFeatures';

export {
  CLOTHING_COMMERCE_FEATURE_OVERRIDES,
  PHARMACY_COMMERCE_FEATURE_OVERRIDES,
  AUTO_PARTS_COMMERCE_FEATURE_OVERRIDES,
  VEHICLE_SHOWROOM_FEATURE_OVERRIDES,
  FURNITURE_COMMERCE_FEATURE_OVERRIDES,
  FITNESS_COMMERCE_FEATURE_OVERRIDES,
  MILK_COMMERCE_FEATURE_OVERRIDES,
  WATER_COMMERCE_FEATURE_OVERRIDES,
  CONSTRUCTION_MANAGEMENT_FEATURE_OVERRIDES,
  EV_BIKES_COMMERCE_FEATURE_OVERRIDES,
  LUBRICANT_DISTRIBUTION_FEATURE_OVERRIDES,
  HARDWARE_COMMERCE_FEATURE_OVERRIDES,
} from '@/lib/config/domainPackageFeatures';

/** @typedef {'shipped' | 'partial' | 'roadmap'} DomainCapabilityStatus */

/**
 * @typedef {Object} DomainPackageModule
 * @property {string} id
 * @property {string} title
 * @property {string} body
 * @property {string[]} bullets
 * @property {DomainCapabilityStatus} [status]
 */

/**
 * @typedef {Object} DomainPackageDefinition
 * @property {string} key
 * @property {string} slug
 * @property {string} name
 * @property {string} tagline
 * @property {string} summary
 * @property {string[]} verticals
 * @property {string} defaultVertical
 * @property {string} recommendedPlanTier
 * @property {Record<string, number>} [limitOverrides]
 * @property {{ mode: 'tier' | 'custom', feature_overrides?: Record<string, boolean> }} packaging
 * @property {{ price_pkr: number, price_usd: number, billing?: string, badge?: string | null }} pricing
 * @property {string} marketingPath
 * @property {string} [demoStoreDomain]
 * @property {DomainPackageModule[]} moduleGroups
 * @property {Array<{ question: string, answer: string }>} faqs
 * @property {string[]} competitorNotes
 * @property {Array<{ key: string, label: string, desc: string }>} [verticalPresets]
 */

/**
 * Canonical domain package catalog. Add new vertical SKUs here.
 * @type {Record<string, DomainPackageDefinition>}
 */
export const DOMAIN_PACKAGES = Object.freeze({
  'clothing-commerce': {
    key: 'clothing-commerce',
    slug: 'clothing-commerce',
    name: 'Clothing & Textile Commerce Suite',
    tagline: 'Online store, retail POS, and wholesale in one stock picture',
    summary:
      'Built for Pakistani and regional fashion operators running a branded storefront, showroom POS, and B2B wholesale desk from the same catalog. Size/color variants, seasonal collections, price lists, thaan-aware inventory, and campaign tools without stitching Shopify, Busy, and spreadsheets together.',
    verticals: ['garments', 'boutique-fashion', 'textile-wholesale', 'textile-mill'],
    defaultVertical: 'garments',
    recommendedPlanTier: 'business',
    limitOverrides: {
      max_users: 20,
      max_products: 25000,
      max_customers: 8000,
      max_vendors: 3000,
      max_warehouses: 12,
      max_invoices_per_month: 15000,
      max_pos_terminals: 8,
      max_storage_mb: 8000,
      max_branches: 8,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: { ...CLOTHING_COMMERCE_FEATURE_OVERRIDES },
    },
    pricing: {
      price_pkr: 12999,
      price_usd: 45,
      billing: 'monthly',
      badge: 'Vertical suite',
    },
    marketingPath: '/solutions/clothing-commerce',
    demoStoreDomain: 'demo-textile',
    moduleGroups: [
      {
        id: 'catalog',
        title: 'Fashion catalog & variants',
        body:
          'Size/color matrix, designer and season fields, fabric sourcing, and rich PK clothing seed catalogs on registration. Excel bulk import for large seasonal drops.',
        bullets: [
          'Variant-aware storefront and hub inventory',
          'Season, collection, and stitching status fields',
          'PK brand presets and demo boutique storefront',
        ],
        status: 'shipped',
      },
      {
        id: 'channels',
        title: 'Three channels, one ledger',
        body:
          'Public online store, in-store POS, and wholesale quotations share stock, tax, and customer credit without duplicate SKU lists.',
        bullets: [
          'Branded storefront with cart and checkout',
          'Retail POS with barcode lookup',
          'B2B quotes, sales orders, and delivery challans',
        ],
        status: 'shipped',
      },
      {
        id: 'wholesale',
        title: 'Wholesale & trade pricing',
        body:
          'Separate price lists for retail, dealer, and export buyers. Credit limits, broker-friendly customer fields, and thaan/meter units for fabric wholesalers.',
        bullets: [
          'Dynamic price lists (Professional+ base, enabled in suite)',
          'Customer credit limits and ledger (udhaar)',
          'Textile units: suit, thaan, meter, gaz',
        ],
        status: 'shipped',
      },
      {
        id: 'operations',
        title: 'Multi-location & seasonal stock',
        body:
          'Warehouse, showroom, and online fulfilment locations with transfers. Batch/thaan tracking for fabric rolls. Smart restock signals tuned for Eid and wedding peaks.',
        bullets: [
          'Multi-warehouse with inter-location transfers',
          'Batch tracking for fabric thaan (wholesale vertical)',
          'Industry Insights and AI restock (Business tier)',
        ],
        status: 'partial',
      },
      {
        id: 'growth',
        title: 'Campaigns & loyalty',
        body:
          'Run Eid previews, loyalty rewards, and segmented outreach from the same order history as your storefront and POS, not a disconnected email tool.',
        bullets: [
          'Loyalty and promotions hub',
          'Campaign segments on Business+',
          'Abandoned cart recovery (when configured)',
        ],
        status: 'partial',
      },
      {
        id: 'compliance',
        title: 'Tax & audit trail',
        body:
          'GST configuration, provincial tax fields, further-tax awareness for unregistered buyers, and fiscal periods for year-end close. FBR IRIS live filing remains roadmap.',
        bullets: [
          'Sales tax on invoices and POS receipts',
          'Fiscal periods and GL posting',
          'Approval workflows for high discounts',
        ],
        status: 'partial',
      },
    ],
    faqs: [
      {
        question: 'Which business types fit this suite?',
        answer:
          'Fashion retailers, designer boutiques, garment brands, and textile wholesalers who sell online and offline. Choose garments or boutique-fashion for ready-to-wear; textile-wholesale for fabric/than trade.',
      },
      {
        question: 'How is this different from generic Business tier?',
        answer:
          'Same core platform, but the suite pre-enables the module mix clothing operators need (POS + storefront + price lists + campaigns + multi-warehouse), bumps catalog limits, and registers you with the right vertical preset and demo storefront.',
      },
      {
        question: 'Do you support size/color matrices?',
        answer:
          'Yes. Product variants power storefront selection and hub inventory. Domain fields capture season, fabric, stitching status, and designer metadata.',
      },
      {
        question: 'Can I run wholesale-only without POS?',
        answer:
          'Yes. Platform admins or owners can adjust module packaging after signup. This suite defaults to full omni-channel because most large clothing groups run retail and wholesale together.',
      },
      {
        question: 'Is manufacturing included?',
        answer:
          'Basic BOM and production orders are enabled for in-house stitching units. Full shop-floor MES is roadmap; confirm scope on a demo if you run a large production floor.',
      },
    ],
    competitorNotes: [
      'Shopify excels at storefront apps but lacks native wholesale ledgers and Pakistani tax depth.',
      'Busy/Odoo-style ERPs handle wholesale but often need separate e-commerce and campaign tools.',
      'Tenvo keeps catalog, POS, B2B quotes, and campaigns in one hub with clothing-specific presets.',
    ],
  },

  'pharmacy-commerce': {
    key: 'pharmacy-commerce',
    slug: 'pharmacy-commerce',
    name: 'Pharmacy & Wellness Commerce Suite',
    tagline: 'Licensed counter, online OTC catalog, and expiry-aware stock',
    summary:
      'Built for licensed pharmacies and wellness retailers running counter POS, home delivery, and repeat-order outreach. Batch expiry tracking, pharmacist support workflows, GST-compliant receipts, and an elevated pharmacy storefront without juggling a separate e-commerce app and spreadsheet stock.',
    verticals: ['pharmacy'],
    defaultVertical: 'pharmacy',
    recommendedPlanTier: 'business',
    limitOverrides: {
      max_users: 18,
      max_products: 18000,
      max_customers: 12000,
      max_vendors: 1500,
      max_warehouses: 8,
      max_invoices_per_month: 20000,
      max_pos_terminals: 6,
      max_storage_mb: 6000,
      max_branches: 6,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: { ...PHARMACY_COMMERCE_FEATURE_OVERRIDES },
    },
    pricing: {
      price_pkr: 11999,
      price_usd: 42,
      billing: 'monthly',
      badge: 'Pharmacy suite',
    },
    marketingPath: '/solutions/pharmacy-commerce',
    demoStoreDomain: 'demo-pharmacy',
    moduleGroups: [
      {
        id: 'pharmacy',
        title: 'Pharmacy catalog & compliance',
        body:
          'OTC, wellness, and device categories with batch expiry, GST fields, and elevated pharmacy storefront templates on registration.',
        bullets: [
          'Expiry-aware batch tracking',
          'Pharmacy storefront with category rails',
          'Genuine-medicine trust copy and delivery thresholds',
        ],
        status: 'shipped',
      },
      {
        id: 'channels',
        title: 'Counter + online fulfilment',
        body:
          'Walk-in POS and web orders share one stock ledger. Hub fulfils delivery and pickup from the same locations.',
        bullets: [
          'Barcode POS with tax receipts',
          'Storefront cart and checkout',
          'Low-stock and expiry alerts in dashboard',
        ],
        status: 'shipped',
      },
      {
        id: 'service',
        title: 'Care, refills & support',
        body:
          'Appointment booking for pharmacist consults, helpdesk tickets, and feedback surveys, aligned with repeat-order campaigns.',
        bullets: [
          'Appointment booking module',
          'Helpdesk and feedback surveys',
          'Email campaigns and abandoned cart recovery',
        ],
        status: 'partial',
      },
      {
        id: 'compliance',
        title: 'Tax & audit',
        body:
          'Sales tax on invoices and POS, fiscal periods, and approval workflows for high discounts. Prescription verification APIs remain roadmap.',
        bullets: [
          'GST on hub invoices and POS',
          'Fiscal periods and GL posting',
          'Audit logs and approval workflows',
        ],
        status: 'partial',
      },
    ],
    faqs: [
      {
        question: 'Is this for licensed pharmacies only?',
        answer:
          'The suite is designed for licensed pharmacy and wellness retail operators. OTC and general wellness catalogs are supported today; prescription routing integrations are sales-assisted.',
      },
      {
        question: 'How do you handle expiry dates?',
        answer:
          'Batch tracking is enabled in the suite packaging so you can record expiry per batch and surface low-stock and industry insights in the hub.',
      },
      {
        question: 'Can I run delivery and counter together?',
        answer:
          'Yes. Storefront orders and POS sales decrement the same product and location stock. Configure free-delivery thresholds in storefront settings.',
      },
    ],
    competitorNotes: [
      'Standalone pharmacy e-commerce apps rarely include counter POS and wholesale purchase ledgers.',
      'Generic ERPs lack pharmacy storefront templates and refill-oriented campaigns.',
      'Tenvo wires elevated pharmacy UX, batch stock, and hub fulfilment in one tenant.',
    ],
  },

  'auto-parts-commerce': {
    key: 'auto-parts-commerce',
    slug: 'auto-parts-commerce',
    name: 'Auto Parts & Accessories Suite',
    tagline: 'Parts finder storefront, trade counter, and multi-branch stock',
    summary:
      'For auto parts retailers and distributors selling OEM and aftermarket SKUs online and over the counter. Vehicle-aware storefront search, serial and batch tracking, workshop price lists, and supplier quotes without a separate catalog CMS and POS.',
    verticals: ['auto-parts'],
    defaultVertical: 'auto-parts',
    recommendedPlanTier: 'business',
    limitOverrides: {
      max_users: 22,
      max_products: 35000,
      max_customers: 6000,
      max_vendors: 4000,
      max_warehouses: 15,
      max_invoices_per_month: 18000,
      max_pos_terminals: 10,
      max_storage_mb: 9000,
      max_branches: 10,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: { ...AUTO_PARTS_COMMERCE_FEATURE_OVERRIDES },
    },
    pricing: {
      price_pkr: 13999,
      price_usd: 48,
      billing: 'monthly',
      badge: 'Auto parts suite',
    },
    marketingPath: '/solutions/auto-parts-commerce',
    demoStoreDomain: 'demo-autoparts',
    moduleGroups: [
      {
        id: 'parts',
        title: 'Parts finder & catalog',
        body:
          'Immersive parts-finder hero, category rails, and vehicle-brand filters on the public storefront. Rich auto-parts seed catalog on registration.',
        bullets: [
          'Vehicle / part search hero',
          'OEM-style category and brand rails',
          'Semantic search for large SKU lists',
        ],
        status: 'shipped',
      },
      {
        id: 'channels',
        title: 'Counter & e-shop',
        body:
          'Workshop counter POS with barcode lookup plus online cart checkout, one stock picture across branches.',
        bullets: [
          'Trade counter POS and refunds',
          'Storefront orders in hub fulfilment',
          'Barcode scanning at POS',
        ],
        status: 'shipped',
      },
      {
        id: 'wholesale',
        title: 'Workshop & trade pricing',
        body:
          'Price lists for workshops, supplier quotes for imports, and delivery challans for bulk supply runs.',
        bullets: [
          'Dynamic price lists',
          'Supplier quote comparison',
          'Delivery challans for B2B runs',
        ],
        status: 'shipped',
      },
      {
        id: 'inventory',
        title: 'Serials, batches & branches',
        body:
          'Serial tracking for electronics, batch lots for oils and fluids, and multi-warehouse transfers between branches.',
        bullets: [
          'Serial and batch tracking',
          'Multi-warehouse transfers',
          'AI restock signals for fast movers',
        ],
        status: 'partial',
      },
    ],
    faqs: [
      {
        question: 'Does the storefront include a parts finder?',
        answer:
          'Yes. The auto-parts vertical uses an immersive finder hero (vehicle, VIN, and part search) on the public store, wired to the same catalog you manage in the hub.',
      },
      {
        question: 'Can workshops buy on credit?',
        answer:
          'Customer credit limits and ledger (udhaar) are supported. Trade price lists can be assigned per customer segment.',
      },
      {
        question: 'Is this the same as the vehicle dealership suite?',
        answer:
          'No. This suite targets parts and accessories retailers. Vehicle listings and test-drive booking are in the Vehicle Showroom suite.',
      },
    ],
    competitorNotes: [
      'Marketplace templates focus on listings, not parts SKUs and workshop counter workflows.',
      'Shopify auto apps add cost per site and rarely include native POS and purchase ledgers.',
      'Tenvo combines parts-finder storefront, POS, and wholesale supply in one hub.',
    ],
  },

  'vehicle-showroom': {
    key: 'vehicle-showroom',
    slug: 'vehicle-showroom',
    name: 'Vehicle Showroom & Dealership Suite',
    tagline: 'List vehicles, book test drives, and sell parts online',
    summary:
      'For single-brand dealerships and showrooms listing new and used vehicles, booking test drives, and running an accessories e-shop. Lead capture, live chat, and showroom POS share one CRM, not a separate DMS, website, and parts catalog.',
    verticals: ['vehicle-dealership'],
    defaultVertical: 'vehicle-dealership',
    recommendedPlanTier: 'business',
    limitOverrides: {
      max_users: 20,
      max_products: 8000,
      max_customers: 15000,
      max_vendors: 2000,
      max_warehouses: 8,
      max_invoices_per_month: 8000,
      max_pos_terminals: 6,
      max_storage_mb: 7000,
      max_branches: 6,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: { ...VEHICLE_SHOWROOM_FEATURE_OVERRIDES },
    },
    pricing: {
      price_pkr: 14999,
      price_usd: 52,
      billing: 'monthly',
      badge: 'Dealership suite',
    },
    marketingPath: '/solutions/vehicle-showroom',
    demoStoreDomain: 'demo-showroom',
    moduleGroups: [
      {
        id: 'showroom',
        title: 'Vehicle listings & showroom',
        body:
          'Dealership storefront with make, model, condition, and body-type filters. Demo showroom profile with booking CTAs and trust strip.',
        bullets: [
          'Vehicle listing catalog with domain fields',
          'Test-drive and contact booking CTAs',
          'Showroom hero and marketing banners',
        ],
        status: 'shipped',
      },
      {
        id: 'growth',
        title: 'Leads, chat & campaigns',
        body:
          'Capture web leads, nurture with email campaigns, and respond via live chat, tied to the same customer record as showroom walk-ins.',
        bullets: [
          'Lead capture forms and nurture',
          'Live chat on storefront',
          'Marketing automation and email campaigns',
        ],
        status: 'partial',
      },
      {
        id: 'channels',
        title: 'Parts shop & POS',
        body:
          'Accessories, car care, and PPF products on the same site as vehicle listings. Counter POS for showroom add-ons.',
        bullets: [
          'E-shop collections alongside listings',
          'POS for accessories and services',
          'Customer portal for order history',
        ],
        status: 'shipped',
      },
      {
        id: 'operations',
        title: 'Stock & governance',
        body:
          'Serial tracking for VIN-linked units, multi-location stock for parts, and approval workflows for high discounts on vehicle deals.',
        bullets: [
          'Serial tracking for vehicle units',
          'Multi-warehouse parts stock',
          'Approval workflows for discounts',
        ],
        status: 'partial',
      },
    ],
    faqs: [
      {
        question: 'Can I list both new and used vehicles?',
        answer:
          'Yes. Products use automotive domain fields (make, model, condition, fuel, body type) and appear in storefront filters.',
      },
      {
        question: 'How do test drives work?',
        answer:
          'Appointment booking is enabled in the suite. Owners configure meeting URLs in storefront settings; platform Calendly is never used for tenant booking.',
      },
      {
        question: 'Is this a multi-dealer marketplace?',
        answer:
          'This suite is for a single-brand dealership. Multi-seller portals use the separate auto-marketplace vertical.',
      },
    ],
    competitorNotes: [
      'Listing-only sites lack parts POS and purchase accounting.',
      'DMS tools rarely include branded e-commerce and campaign automation.',
      'Tenvo unifies listings, booking, parts shop, and hub ledgers for one dealership brand.',
    ],
  },

  'furniture-commerce': {
    key: 'furniture-commerce',
    slug: 'furniture-commerce',
    name: 'Furniture & Home Commerce Suite',
    tagline: 'Showroom collections, delivery challans, and large-ticket checkout',
    summary:
      'For furniture and home retailers selling sofas, beds, dining sets, and décor online and in showroom. Elevated furniture storefront, appointment booking for showroom visits, stock reservations for made-to-order lines, and delivery challans for bulky fulfilment.',
    verticals: ['furniture'],
    defaultVertical: 'furniture',
    recommendedPlanTier: 'business',
    limitOverrides: {
      max_users: 18,
      max_products: 12000,
      max_customers: 10000,
      max_vendors: 2000,
      max_warehouses: 10,
      max_invoices_per_month: 12000,
      max_pos_terminals: 6,
      max_storage_mb: 7000,
      max_branches: 6,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: { ...FURNITURE_COMMERCE_FEATURE_OVERRIDES },
    },
    pricing: {
      price_pkr: 12499,
      price_usd: 44,
      billing: 'monthly',
      badge: 'Home & furniture suite',
    },
    marketingPath: '/solutions/furniture-commerce',
    demoStoreDomain: 'demo-furniture',
    moduleGroups: [
      {
        id: 'furniture',
        title: 'Collections & elevated store',
        body:
          'Room-inspired catalog, collection pages, and furniture-elevated hero on registration. Demo furniture archive seed for day-one credibility.',
        bullets: [
          'Living, bedroom, and dining collections',
          'Elevated furniture storefront template',
          'High-ticket checkout and delivery thresholds',
        ],
        status: 'shipped',
      },
      {
        id: 'channels',
        title: 'Showroom & online',
        body:
          'Showroom POS for deposits and walk-in quotes plus web cart checkout, same availability across channels.',
        bullets: [
          'Showroom POS and quotations',
          'Storefront orders with assembly notes',
          'Abandoned cart recovery for big-ticket carts',
        ],
        status: 'shipped',
      },
      {
        id: 'delivery',
        title: 'Bulky fulfilment',
        body:
          'Delivery challans, stock reservations for custom orders, and multi-warehouse stock for display vs warehouse units.',
        bullets: [
          'Delivery challans for dispatch',
          'Stock reservations for made-to-order',
          'Multi-warehouse display vs stock rooms',
        ],
        status: 'shipped',
      },
      {
        id: 'growth',
        title: 'Appointments & campaigns',
        body:
          'Book showroom visits, run seasonal sales campaigns, and loyalty for repeat buyers of accessories and mattresses.',
        bullets: [
          'Appointment booking for showroom',
          'Campaigns and loyalty programs',
          'Price lists for interior trade',
        ],
        status: 'partial',
      },
    ],
    faqs: [
      {
        question: 'Do you support made-to-order furniture?',
        answer:
          'Stock reservations and quotations are enabled so you can hold capacity and collect deposits before fulfilment.',
      },
      {
        question: 'How does delivery work for large items?',
        answer:
          'Configure free-delivery thresholds and use delivery challans from the hub when dispatching bulky orders.',
      },
      {
        question: 'Is assembly scheduling included?',
        answer:
          'Appointment booking covers showroom visits today. Dedicated assembly crew scheduling is roadmap; confirm on a demo.',
      },
    ],
    competitorNotes: [
      'Furniture SaaS storefronts often lack native POS and purchase accounting.',
      'Generic ERPs miss room-collection UX and large-ticket cart recovery.',
      'Tenvo pairs elevated furniture storefronts with showroom POS and delivery workflows.',
    ],
  },

  'fitness-commerce': {
    key: 'fitness-commerce',
    slug: 'fitness-commerce',
    name: 'Gym & Fitness Commerce Suite',
    tagline: 'Supplements shop, memberships, and coach booking on one dark storefront',
    summary:
      'Built for gyms and fitness retailers selling supplements online, memberships at the front desk, and personal training by appointment. Elevated fitness storefront with archive supplement catalog, Wild Programs hero, membership SKUs, tenant meeting URL booking, and counter POS without a separate Shopify stack and paper sign-up sheets.',
    verticals: ['gym-fitness'],
    defaultVertical: 'gym-fitness',
    recommendedPlanTier: 'business',
    limitOverrides: {
      max_users: 20,
      max_products: 15000,
      max_customers: 12000,
      max_vendors: 2000,
      max_warehouses: 8,
      max_invoices_per_month: 15000,
      max_pos_terminals: 6,
      max_storage_mb: 7000,
      max_branches: 6,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: { ...FITNESS_COMMERCE_FEATURE_OVERRIDES },
    },
    pricing: {
      price_pkr: 11999,
      price_usd: 42,
      billing: 'monthly',
      badge: 'Gym & fitness suite',
    },
    marketingPath: '/solutions/fitness-commerce',
    demoStoreDomain: 'demo-fitness',
    moduleGroups: [
      {
        id: 'storefront',
        title: 'Elevated fitness storefront',
        body:
          'Dark immersive hero, supplement category rails, membership tiers, and coach profiles. Rich gym-fitness seed catalog from archive references on registration.',
        bullets: [
          'Supplements catalog with real product imagery',
          'Membership and class SKUs on day one',
          'Wild Programs hero with male/female poses',
        ],
        status: 'shipped',
      },
      {
        id: 'channels',
        title: 'Front desk & online shop',
        body:
          'Sell whey, pre-workout, and accessories at the counter and on the web from one stock ledger. Hub fulfils pickup and delivery orders.',
        bullets: [
          'Barcode POS for supplement counter',
          'Storefront cart and checkout',
          'Multi-warehouse stock for shop vs floor',
        ],
        status: 'shipped',
      },
      {
        id: 'coaching',
        title: 'Coaching & appointments',
        body:
          'Book personal training, nutrition consults, and trial passes via tenant meeting URLs and contact flows wired to hub operations.',
        bullets: [
          'Calendly-style booking strip on storefront',
          'PT packs and class passes as products',
          'Contact queue in domain operations hub',
        ],
        status: 'shipped',
      },
      {
        id: 'growth',
        title: 'Retention & campaigns',
        body:
          'Loyalty for repeat supplement buyers, seasonal challenge campaigns, and abandoned cart recovery for online stacks.',
        bullets: [
          'Loyalty programs and campaigns',
          'Abandoned cart recovery',
          'AI restock for fast-moving SKUs',
        ],
        status: 'partial',
      },
    ],
    faqs: [
      {
        question: 'Does this include a full gym access control system?',
        answer:
          'Membership products, POS, and booking are shipped today. Turnstile and biometric access integrations are sales-assisted roadmap items.',
      },
      {
        question: 'Can I sell supplements and memberships together?',
        answer:
          'Yes. The suite seeds supplement categories and membership SKUs. Member pricing and bundles can be configured in hub price lists.',
      },
      {
        question: 'How does coach booking work?',
        answer:
          'Owners set a tenant meeting URL in Store Settings. The fitness storefront surfaces Book CTAs and a booking strip without using the platform sales calendar.',
      },
    ],
    competitorNotes: [
      'Gym management apps often lack a credible supplement e-commerce storefront.',
      'Supplement-only Shopify themes miss membership POS and coach scheduling.',
      'Tenvo pairs elevated fitness UX, archive supplement catalog, and hub fulfilment in one tenant.',
    ],
  },

  'milk-commerce': {
    key: 'milk-commerce',
    slug: 'milk-commerce',
    name: 'Milk Shop Commerce Suite',
    tagline: 'kg counter, Route Hisab, chill FEFO, and credit collect in one doodh-shop OS',
    summary:
      'Built for Pakistani neighborhood milk shops: sell fresh milk by kg at the counter, run morning house routes on Route Hisab, keep chill dairy on FEFO, and collect weekly or monthly credit with bilingual thermal bills. Same stock and AR as your online milk store. No restaurant, warehouse chain, or membership chrome.',
    verticals: ['milk-shop'],
    defaultVertical: 'milk-shop',
    recommendedPlanTier: 'professional',
    limitOverrides: {
      max_users: 5,
      max_products: 2000,
      max_customers: 3000,
      max_vendors: 200,
      max_warehouses: 1,
      max_invoices_per_month: 2000,
      max_pos_terminals: 2,
      max_storage_mb: 1000,
      max_branches: 1,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: { ...MILK_COMMERCE_FEATURE_OVERRIDES },
    },
    pricing: {
      price_pkr: 10000,
      price_usd: 35,
      billing: 'monthly',
      badge: 'Milk shop suite',
    },
    marketingPath: '/solutions/milk-commerce',
    demoStoreDomain: 'demo-milk',
    moduleGroups: [
      {
        id: 'counter',
        title: 'Counter POS (kg)',
        body:
          'Walk-in fresh milk by kg, pack dairy by barcode, refunds, and offline queue for load-shedding. Same SuperStore shell used by grocery counters, tuned for weight lines.',
        bullets: [
          'Weight-aware SuperStore POS',
          'Barcode for packaged dairy',
          'Offline POS mode for power cuts',
        ],
        status: 'shipped',
      },
      {
        id: 'route-hisab',
        title: 'Route Hisab',
        body:
          'Morning doorstep sheet by house number and route. Week or month totals become invoices and receipts on the standard AR path, with 58mm thermal hisab bills and reminders.',
        bullets: [
          'Daily route grid for active houses',
          'Week/month invoice from delivery totals',
          'Thermal bills plus hub/email/WhatsApp reminders',
        ],
        status: 'shipped',
      },
      {
        id: 'chill',
        title: 'Chill inventory (FEFO)',
        body:
          'Best-before and batch tracking for chilled and packaged dairy. Dairy field presets (milk type, fat %, chilled) without farm livestock fields.',
        bullets: [
          'FEFO and near-expiry awareness',
          'Batch tracking on Professional packaging',
          'kg-first units and PK dairy brands',
        ],
        status: 'shipped',
      },
      {
        id: 'online',
        title: 'Online milk store',
        body:
          'Elevated supermarket-family storefront with milk overlays, category rails, and storefront orders fulfilled in the hub. Decimal kg qty on weight SKUs only.',
        bullets: [
          'demo-milk style public store',
          'Storefront orders in hub',
          'JazzCash, Easypaisa, COD friendly checkout',
        ],
        status: 'shipped',
      },
    ],
    faqs: [
      {
        question: 'Is this the same as a supermarket or dairy farm package?',
        answer:
          'No. milk-commerce is only for milk-shop (and aliases like doodh-shop). Supermarket grocery aisles and dairy-farm livestock stay on their own verticals.',
      },
      {
        question: 'How do I register this suite?',
        answer:
          'Open Industry Plans → Milk shop, or go to /solutions/milk-commerce and choose Get this suite. Registration uses package=milk-commerce, domain=milk-shop, and the Professional plan with lean packaging applied.',
      },
      {
        question: 'Does Route Hisab need a higher plan?',
        answer:
          'Route Hisab is domain-gated for milk-shop tenants. The suite recommends Professional so batch/FEFO and solid reports sit next to counter and hisab without Business bloat.',
      },
      {
        question: 'Can I add a second chill room later?',
        answer:
          'Default is one location. Owners can re-enable multi-location via Settings Industry overrides and packaging if the shop grows. Neighborhood doodh shops usually stay single-counter.',
      },
      {
        question: 'Will this help me sell more milk with the same staff?',
        answer:
          'Yes for throughput and collection: faster kg POS at the counter, one morning sheet instead of paper registers, bilingual bills that reduce credit disputes, and offline modes so load-shedding does not stop the day. Stock and AR stay unified so you spend less time reconciling and more time serving houses.',
      },
    ],
    competitorNotes: [
      'Generic ERPs bury route hisab under warehouse and CRM modules keepers never use.',
      'Delivery apps sell orders but not morning house registers and monthly credit collect.',
      'Tenvo keeps counter, hisab, chill stock, and online milk on one tenant without restaurant or gym chrome.',
    ],
  },

  'water-commerce': {
    key: 'water-commerce',
    slug: 'water-commerce',
    name: 'Water Delivery Commerce Suite',
    tagline: 'City & area routes, 19L refills, domestic & corporate collect, and expense tracking in one lean OS',
    summary:
      'Built for Pakistani bottled water suppliers: register houses and offices by city and area, run daily route sheets with weekday delivery days, track empty bottle deposits, record operational expenses, and collect weekly or monthly credit with clean thermal and A4 bills. Includes a simple retail dashboard, Route Hisab, expense tracking, and reporting. Stripped of online store, POS, and financial hub clutter.',
    verticals: ['water-delivery'],
    defaultVertical: 'water-delivery',
    recommendedPlanTier: 'professional',
    limitOverrides: {
      max_users: 8,
      max_products: 2000,
      max_customers: 5000,
      max_vendors: 200,
      max_warehouses: 1,
      max_invoices_per_month: 3000,
      max_pos_terminals: 0,
      max_storage_mb: 1000,
      max_branches: 1,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: { ...WATER_COMMERCE_FEATURE_OVERRIDES },
    },
    pricing: {
      price_pkr: 5000,
      price_usd: 18,
      billing: 'monthly',
      badge: 'Water delivery suite',
    },
    marketingPath: '/solutions/water-commerce',
    demoStoreDomain: 'demo-water',
    moduleGroups: [
      {
        id: 'routes',
        title: 'City and area routes',
        body:
          'Customer records with city, delivery area (Karachi DHA, Gulshan, Clifton, and other cities), rider/route, and weekday cadence for domestic and corporate accounts.',
        bullets: [
          'Pakistan city → area knowledge presets',
          'Domestic vs corporate customer types',
          'Delivery days (Daily, Mon Wed Fri, Weekdays…)',
        ],
        status: 'shipped',
      },
      {
        id: 'route-hisab',
        title: 'Daily Route Hisab',
        body:
          'Rider doorstep grid by house/office and route. Week or month totals become invoices and receipts on the standard AR path, with thermal bills and reminders.',
        bullets: [
          'Daily route grid filtered by delivery days',
          'Week/month invoice from delivery totals',
          'Thermal bills plus hub/email/WhatsApp reminders',
        ],
        status: 'shipped',
      },
      {
        id: 'bottles',
        title: 'Bottles and deposits',
        body:
          '19L refill, family packs, PET cases, and empty bottle deposit SKUs with bottle/case units and PK water brands.',
        bullets: [
          'Bottle and case inventory units',
          'Deposit amount on products and customers',
          'Nestlé Pure Life, Aquafina, and local plant brands',
        ],
        status: 'shipped',
      },
      {
        id: 'expenses',
        title: 'Expense tracking & reporting',
        body:
          'Record daily operational expenses (fuel, rider wages, vehicle maintenance, plant utility) with category breakdowns and lean reporting.',
        bullets: [
          'Fuel & vehicle maintenance category tracking',
          'Log Expense modal integrated in header/dashboard',
          'Clean revenue vs expense reporting',
        ],
        status: 'shipped',
      },
    ],
    faqs: [
      {
        question: 'Is this the same as milk-shop Route Hisab?',
        answer:
          'Same core principle (daily sheet + period bills), tailored for water. water-delivery has dedicated bottle fields, city/area presets, weekday delivery schedules, and expense tracking.',
      },
      {
        question: 'How do I register this suite?',
        answer:
          'Open Industry Plans → Water delivery, or go to /solutions/water-commerce and choose Get this suite. Registration uses package=water-commerce, domain=water-delivery, and the Professional plan (5,000 PKR/mo) with lean packaging applied.',
      },
      {
        question: 'Does Daily Route need a higher plan?',
        answer:
          'No. Daily Route Hisab is fully included in the Professional package (5,000 PKR/mo), along with expense tracking and customer management, stripped of unnecessary POS and store clutter.',
      },
    ],
    competitorNotes: [
      'Spreadsheet route books lose weekday schedules and deposit tracking.',
      'Generic ERPs clutter water distributors with unwanted POS, e-commerce, and financial hub menus.',
      'Tenvo keeps rider sheet, bottle stock, expenses, and collections on one focused tenant at 5,000 PKR/mo.',
    ],
  },
  'construction-management': {
    key: 'construction-management',
    slug: 'construction-management',
    name: 'Construction & Infrastructure EPC Suite',
    tagline: 'Public & private projects, BOQ estimation, IPC running bills, material rates, and site machinery tracking',
    summary:
      'Built for general contractors, civil engineers, and EPC firms managing public sector tenders (PEC / PPRA / NHA / CWD) and private multi-million projects (buildings, highways, bridges, dams). Integrated BOQ cost analysis, Running Bill / IPC calculations, escalation formulas, retention money tracking, machinery fuel logs, and subcontractor ledgers.',
    verticals: [
      'construction-contractor',
      'civil-engineering',
      'building-construction',
      'road-infrastructure',
    ],
    defaultVertical: 'construction-contractor',
    recommendedPlanTier: 'enterprise',
    limitOverrides: {
      max_users: 50,
      max_products: 50000,
      max_customers: 2000,
      max_vendors: 2000,
      max_warehouses: 25,
      max_invoices_per_month: 20000,
      max_storage_mb: 20000,
      max_branches: 15,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: {
        boq_tracking: true,
        ipc_billing: true,
        machinery_logbook: true,
        subcontractor_ledger: true,
        site_operations: true,
        price_escalation_clause: true,
        multi_warehouse: true,
        expense_tracking: true,
        project_costing: true,
        approval_workflows: true,
        tax_compliance: true,
        advanced_reports: true,
        supplier_quotes: true,
        // Strip retail/F&B modules
        restaurant_pos: false,
        loyalty_programs: false,
        membership_management: false,
        storefront_orders: false,
        abandoned_cart_recovery: false,
      },
    },
    pricing: {
      price_pkr: 24999,
      price_usd: 89,
      billing: 'monthly',
      badge: 'EPC Enterprise',
    },
    marketingPath: '/solutions/construction-management',
    demoStoreDomain: 'demo-construction',
    moduleGroups: [
      {
        id: 'boq_estimation',
        title: 'BOQ & Schedule of Rates (MRS / CSR)',
        body:
          'Bill of Quantities breakdown with Composite Rate Analysis (Material, Labor, Equipment), Punjab MRS / NHA CSR integration, and cost variance tracking.',
        bullets: [
          'Itemized BOQ breakdown with unit rate analysis',
          'Punjab MRS, Federal CSR, SPPRA DRC rate schedules',
          'Real-time material rate benchmarks (Grade 60 Rebar, Cement, Bitumen)',
        ],
        status: 'shipped',
      },
      {
        id: 'ipc_billing',
        title: 'IPC Running Bills & Contract Ledgers',
        body:
          'Interim Payment Certificate (IPC #1, #2... Final), Mobilization Advance recovery, Retention Money (5-10%), and PEC Clause 70 Price Adjustment formulas.',
        bullets: [
          'Running bill IPC workflow with net payable calculation',
          'Retention Money ledger and Defects Liability Period tracking',
          'FBR Sec 153(1)(c) WHT & Provincial Tax (PRA/SRB/KPRA/BRA) deduction',
        ],
        status: 'shipped',
      },
      {
        id: 'site_machinery',
        title: 'Plant, Machinery & Material Inventory',
        body:
          'Site inventory logs (steel, cement, aggregate, bitumen), batch quality certification, plant machinery hour-meter logbooks, diesel fuel consumption, and subcontractor running accounts.',
        bullets: [
          'Site warehouse material transfer & Gate Pass tracking',
          'Heavy equipment (Excavators, Graders, Pavers, Mixers) logbooks',
          'Subcontractor Work Orders and milestone retainage',
        ],
        status: 'shipped',
      },
    ],
    faqs: [
      {
        question: 'Does TENVO support PEC contractor categories and PPRA rules?',
        answer:
          'Yes. The suite is preconfigured with Pakistan Engineering Council (PEC) contractor categories (C-A down to C-6), specialization codes (CE01, CE02, BC01), and PPRA standard bidding document workflows.',
      },
      {
        question: 'How are Interim Payment Certificates (IPCs) calculated?',
        answer:
          'IPCs measure cumulative work done against BOQ items, deduct Mobilization Advance amortization, retain 5-10% Retention Money, apply PEC Clause 70 Price Escalation, and compute net tax-adjusted payments.',
      },
      {
        question: 'Can I track heavy machinery fuel and rental costs?',
        answer:
          'Yes. The site machinery module logs hour-meters, diesel consumption in Liters/Hour, maintenance schedules, and equipment rental vs owned asset yields.',
      },
    ],
    competitorNotes: [
      'Generic accounting software lacks IPC running bills, retention money ledgers, and BOQ composite rate breakdowns.',
      'Complex legacy desktop construction software requires months of setup and cannot manage mobile site gate passes or live fuel logs.',
      'Tenvo unifies BOQ estimation, IPC running bills, MRS rate schedules, site machinery logs, and FBR/Provincial WHT compliance on one platform.',
    ],
  },
  'ev-bikes': {
    key: 'ev-bikes',
    slug: 'ev-bikes',
    name: 'EV & Electric Bike Commerce Suite',
    tagline: 'Showroom sales, lithium battery tracking, test-ride bookings, and retail POS',
    summary:
      'Built for electric bike manufacturers, dealerships, and EV scooter showrooms across Pakistan and global markets. Manage motor & battery serial tracking, lithium battery warranty ledgers, customer test-ride bookings, trade-in valuations, showroom POS, and branded online store from a single catalog.',
    verticals: ['ev-bikes', 'vehicle-dealership', 'auto-parts'],
    defaultVertical: 'ev-bikes',
    recommendedPlanTier: 'business',
    limitOverrides: {
      max_users: 20,
      max_products: 25000,
      max_customers: 8000,
      max_vendors: 3000,
      max_warehouses: 12,
      max_invoices_per_month: 15000,
      max_pos_terminals: 8,
      max_storage_mb: 8000,
      max_branches: 8,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: { ...EV_BIKES_COMMERCE_FEATURE_OVERRIDES },
    },
    pricing: {
      price_pkr: 14999,
      price_usd: 49,
      billing: 'monthly',
      badge: 'Vertical suite',
    },
    marketingPath: '/solutions/ev-bikes',
    demoStoreDomain: 'demo-ev',
    moduleGroups: [
      {
        id: 'ev_catalog',
        title: 'EV Motor & Battery Specifications',
        body:
          'Track lithium vs graphene battery types, motor power (W), range (km), top speed (km/h), charging time, and voltage ratings alongside VIN frame & battery serial numbers.',
        bullets: [
          'Battery type, range, top speed, and voltage specifications',
          'Frame VIN and battery pack serial number tracking',
          'Vlektra, Metro EV, and Ramza Aima catalog presets',
        ],
        status: 'shipped',
      },
      {
        id: 'showroom_pos',
        title: 'Showroom POS & Test-Ride Bookings',
        body:
          'In-store point of sale for instant vehicle deposits, accessories, fast chargers, and spare parts. Integrated online test-ride scheduling for prospective buyers.',
        bullets: [
          'Showroom POS with barcode scanner and invoice generation',
          'Customer test-ride booking workflow',
          'Quotation, booking deposit, and sales contract management',
        ],
        status: 'shipped',
      },
      {
        id: 'warranty_battery',
        title: 'Lithium Battery Warranty & Service Ledger',
        body:
          'Monitor battery warranty periods (1-3 years), charge cycles, health logs, and replacement battery inventory with full serial traceability.',
        bullets: [
          'Serial-based battery warranty activation and history',
          'After-sales service, inspection, and maintenance tracking',
          'Multi-location spare parts & fast-charger stock management',
        ],
        status: 'shipped',
      },
      {
        id: 'tradein_finance',
        title: 'Trade-In Valuations & Installment Ledger (Udhaar)',
        body:
          'Accept petrol bike trade-ins towards new EV purchases. Customer ledger for monthly payment installments (udhaar) and dealer network wholesale credit.',
        bullets: [
          'Trade-in valuation and inventory acquisition',
          'Customer credit ledger (udhaar / installment plans)',
          'Dealer network price lists and bulk quotations',
        ],
        status: 'shipped',
      },
    ],
    faqs: [
      {
        question: 'Does TENVO support electric bike & scooter specifications?',
        answer:
          'Yes. TENVO includes native support for EV technical specs including battery type (Lithium-ion / Graphene), voltage, range (km), top speed, motor power (W), and charging time.',
      },
      {
        question: 'Can I track individual motor and battery serial numbers?',
        answer:
          'Absolutely. Built-in serial tracking lets you associate frame VIN numbers and battery pack serial numbers with every sales invoice and warranty claim.',
      },
      {
        question: 'How do test-ride bookings work for online visitors?',
        answer:
          'Public storefronts generated with TENVO EV include a seamless "Book Test Ride" button allowing visitors to schedule a showroom visit.',
      },
    ],
    competitorNotes: [
      'Generic retail POS software cannot track VIN numbers, battery serials, or EV performance specs.',
      'Legacy software lacks customer test-ride booking and trade-in valuation workflows.',
      'Tenvo unifies EV specs, battery warranty ledgers, showroom POS, test-ride bookings, and online storefronts in one platform.',
    ],
    verticalPresets: [
      { key: 'vlektra', label: 'Vlektra Electric Motorcycles', desc: 'High-performance café racer & naked sport EV bikes' },
      { key: 'metroev', label: 'Metro EV Scooters', desc: 'Best-selling urban smart scooters & long-range commuters' },
      { key: 'ramza', label: 'Ramza Aima E-Bikes', desc: 'Fashion-forward urban mobility with Azure Controllers' },
      { key: 'tenvo-ev', label: 'Tenvo EV Showroom', desc: 'Multi-brand EV dealership & battery service station' },
    ],
  },
  'lubricant-distribution': {
    key: 'lubricant-distribution',
    slug: 'lubricant-distribution',
    name: 'Lubricant & Engine Oil Distribution Suite (LDMS)',
    tagline: 'Complete operating system for lubricant distributors, wholesalers & mobilers in Pakistan',
    summary:
      'Engineered for engine oil distributors and wholesalers running multi-unit carton/bottle inventory, mobile van sales, territory beat routes, customer credit limits, and principal target tracking.',
    verticals: ['lubricant-distribution', 'industrial'],
    defaultVertical: 'lubricant-distribution',
    recommendedPlanTier: 'business',
    limitOverrides: {
      max_users: 25,
      max_products: 30000,
      max_customers: 10000,
      max_vendors: 4000,
      max_warehouses: 15,
      max_invoices_per_month: 20000,
      max_pos_terminals: 10,
      max_storage_mb: 10000,
      max_branches: 10,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: { ...LUBRICANT_DISTRIBUTION_FEATURE_OVERRIDES },
    },
    pricing: {
      price_pkr: 14999,
      price_usd: 55,
      billing: 'monthly',
      badge: 'Distribution suite',
    },
    marketingPath: '/solutions/lubricant-distribution',
    demoStoreDomain: 'demo-oil',
    moduleGroups: [
      {
        id: 'catalog',
        title: 'Technical Lubricant Master & Multi-Unit Packaging',
        body:
          'Manage viscosity grades, API/ACEA specs, OEM approvals, and automatic Carton-to-Bottle unit conversions.',
        bullets: [
          'Viscosity grade and API spec fields',
          'Carton -> Bottle -> Liter unit conversion engine',
          'Pakistani oil brand presets and seed catalog',
        ],
        status: 'shipped',
      },
      {
        id: 'distribution',
        title: 'Van Sales & Route Management',
        body:
          'Mobile van inventory, beat routes, doorstep invoicing, and daily end-of-day van stock reconciliation.',
        bullets: [
          'Van stock transfers and mobile inventory',
          'Daily variance reconciliation (Expected vs Physical)',
          'Territory beat route tracking',
        ],
        status: 'shipped',
      },
      {
        id: 'credit',
        title: 'Credit Limit Enforcement & Aging',
        body:
          'Real-time projected credit checking, aging buckets, Post-Dated Cheque (PDC) tracking, and manager approvals.',
        bullets: [
          'Projected credit limit enforcement',
          'Customer aging breakdown (30/60/90+ days)',
          'Cheque and PDC status ledger',
        ],
        status: 'shipped',
      },
      {
        id: 'schemes',
        title: 'Principal Schemes & Rebate Targets',
        body:
          'Track monthly oil company sales targets, distributor margins, volume schemes, and rebate claims.',
        bullets: [
          'Principal sales target vs achievement tracking',
          'Quantity & rebate schemes (e.g. Buy 10 Get 1 Free)',
          'Distributor margin & claim settlement ledger',
        ],
        status: 'shipped',
      },
    ],
    faqs: [
      {
        question: 'Does this suit engine oil distributors and wholesalers in Pakistan?',
        answer:
          'Yes. Built specifically for Pakistani distributors representing principals like Shell, Caltex, TotalEnergies, ZIC, and PSO with multi-unit packaging and van sales support.',
      },
      {
        question: 'How does carton-to-bottle conversion work?',
        answer:
          'Products track bottles-per-carton (e.g. 12 bottles of 1L). Stock can be received in cartons, sold in bottles or cartons, and reported in total liters automatically.',
      },
      {
        question: 'Can salesmen manage doorstep sales and van stock?',
        answer:
          'Yes. Salesmen can carry van inventory, place orders on customer visits, accept payments/cheques, and reconcile stock variances at the end of the day.',
      },
    ],
    competitorNotes: [
      'Generic accounting software fails at multi-unit carton-to-liter conversions and van stock reconciliation.',
      'Legacy Pakistani software lacks real-time credit limit enforcement and beat route tracking.',
      'Tenvo unifies technical oil master, multi-unit stock, van reconciliation, credit control, and GST invoicing.',
    ],
    verticalPresets: [
      { key: 'shell-distributor', label: 'Shell Lubricants Distributor', desc: 'Commercial & passenger motor oils distributor' },
      { key: 'caltex-distributor', label: 'Caltex Havoline Distributor', desc: 'Wholesaler & mobiler distribution network' },
      { key: 'zic-distributor', label: 'ZIC SK Lubricants Distributor', desc: 'Synthetic motor oils & gear lubricants' },
      { key: 'pso-distributor', label: 'PSO Lubricants Distributor', desc: 'Carient & Deo diesel engine oil distribution' },
    ],
  },
  'hardware-commerce': {
    key: 'hardware-commerce',
    slug: 'hardware-commerce',
    name: 'Hardware & Architectural Fittings Suite',
    tagline: 'Architectural door handles, digital locks, hydraulic hinges, and contractor project billing',
    summary:
      'Complete solution for Pakistani hardware stores, architectural handle importers, cabinet fitting specialists, and building material suppliers. Hardware-aware invoicing with finish/size/material specs, box-pack conversion, trade discount schedules, and contractor project ledgers.',
    verticals: ['hardware-sanitary', 'hardware-store', 'hardware', 'building-hardware', 'hardware-tools'],
    defaultVertical: 'hardware-sanitary',
    recommendedPlanTier: 'business',
    limitOverrides: {
      max_users: 15,
      max_products: 15000,
      max_customers: 5000,
      max_vendors: 1500,
    },
    packaging: {
      mode: 'custom',
      feature_overrides: HARDWARE_COMMERCE_FEATURE_OVERRIDES,
    },
    pricing: {
      price_pkr: 14500,
      price_usd: 55,
      billing: 'monthly',
      badge: 'Popular for Hardware',
    },
    marketingPath: '/solutions/hardware-commerce',
    demoStoreDomain: 'demo-hardware',
    moduleGroups: [
      {
        id: 'hardware-catalog',
        title: 'Architectural Hardware & Technical Master',
        body: 'Manage handles, locks, hinges, drawer slides, and tools with technical specs (Item Code, Finish, Material, Size, Gauge, Box Pack).',
        bullets: [
          'Item code, finish (Matt Black, Antique Brass, SS 304), and dimension tracking',
          'Pcs per box and carton conversion breakdown for bulk inventory',
          'Finish guarantee and mechanism warranty tracking',
        ],
        status: 'shipped',
      },
      {
        id: 'hardware-invoicing',
        title: 'Hardware-Aware Invoicing & Contractor Billing',
        body: 'Generate professional A4 PDF invoices displaying item codes, finish/material specs, box counts, and trade discounts.',
        bullets: [
          'Item code, finish, and size printed on line items',
          'Contractor project reference and trade discount support',
          'Box & Pcs dual breakdown on invoice printouts',
        ],
        status: 'shipped',
      },
      {
        id: 'contractor-ledger',
        title: 'Contractor & Architect Project Receivables',
        body: 'Track project accounts, staged payments, and trade credit limits for builders and contractors.',
        bullets: [
          'Project account ledger with running balance',
          'Staged billings and site delivery challans',
          'Credit limit enforcement with aging reports',
        ],
        status: 'shipped',
      },
      {
        id: 'hardware-storefront',
        title: 'Modern Hardware Storefront & B2B Trade Quotes',
        body: 'Showcase architectural collections, digital locks, and cabinet fittings online with B2B trade quote requests.',
        bullets: [
          'Elevated hero carousel and category spotlights',
          'Finish filter badges and box pack specifications',
          'Online retail checkout & contractor trade quote inquiries',
        ],
        status: 'shipped',
      },
    ],
    faqs: [
      {
        question: 'Does this support architectural door handles, digital locks, and cabinet fittings?',
        answer:
          'Yes. Built specifically for Pakistani hardware importers and retailers selling items like DTC hydraulic hinges, Philips smart locks, Blum Legrabox, and solid brass handles.',
      },
      {
        question: 'How does box-pack quantity work?',
        answer:
          'Each product can specify Pcs per Box (e.g. 24 Pcs/Box or 100 Pcs/Ctn). Inventory and invoices show both primary piece count and box equivalent.',
      },
      {
        question: 'Can contractors and builders get trade pricing?',
        answer:
          'Yes. Set up trade pricing and credit limits for contractors with project ledgers and delivery challans.',
      },
    ],
    competitorNotes: [
      'Generic POS software lacks item code, finish, and size specifications on invoices.',
      'Manual bill books lead to inventory errors when selling in both boxes and pieces.',
      'Tenvo unifies technical hardware master, hardware-aware invoicing, contractor accounts, and modern storefront.',
    ],
    verticalPresets: [
      { key: 'architectural-hardware', label: 'Architectural Door Handles & Pulls', desc: 'Solid brass rose handles, mortise locks, & luxury pulls' },
      { key: 'digital-locks', label: 'Biometric Digital Smart Locks', desc: 'Palm vein, fingerprint, & deadbolt smart locks' },
      { key: 'cabinet-fittings', label: 'Kitchen & Cabinet Fittings', desc: 'DTC tandemboxes, hydraulic hinges, & lift-up systems' },
      { key: 'hardware-tools', label: 'Power Tools & Fasteners', desc: 'Drills, saws, hand tools, & stainless steel fasteners' },
    ],
  },
});

/** @returns {DomainPackageDefinition[]} */
export function listDomainPackages() {
  return Object.values(DOMAIN_PACKAGES);
}

/**
 * @param {string | null | undefined} key
 * @returns {DomainPackageDefinition | null}
 */
export function getDomainPackage(key) {
  if (!key || typeof key !== 'string') return null;
  return DOMAIN_PACKAGES[key.trim()] || null;
}

/**
 * Resolve package by marketing slug (`/solutions/[slug]`).
 * @param {string | null | undefined} slug
 */
export function getDomainPackageBySlug(slug) {
  if (!slug || typeof slug !== 'string') return null;
  const normalized = slug.trim();
  return (
    DOMAIN_PACKAGES[normalized] ||
    listDomainPackages().find((p) => p.slug === normalized) ||
    null
  );
}

/**
 * Best package for a registration vertical (first match wins).
 * @param {string | null | undefined} verticalKey
 * @returns {DomainPackageDefinition | null}
 */
export function resolveDomainPackageForVertical(verticalKey) {
  if (!verticalKey) return null;
  const canonical = resolveDomainKey(String(verticalKey).trim());
  return (
    listDomainPackages().find((pkg) =>
      pkg.verticals.some((v) => resolveDomainKey(v) === canonical)
    ) || null
  );
}

/**
 * Merge limit overrides onto a plan tier's defaults (only known limit keys).
 * @param {string} planTier
 * @param {Record<string, number> | undefined} limitOverrides
 */
export function mergePlanLimits(planTier, limitOverrides) {
  const tier = resolvePlanTier(planTier || 'free');
  const base = PLAN_TIERS[tier]?.limits || PLAN_TIERS.free.limits;
  if (!limitOverrides || typeof limitOverrides !== 'object') {
    return { ...base };
  }
  const merged = { ...base };
  for (const [key, value] of Object.entries(limitOverrides)) {
    if (key in merged && typeof value === 'number') {
      merged[key] = value;
    }
  }
  return merged;
}

/**
 * Build registration payload fragments from a domain package.
 *
 * @param {string} packageKey
 * @param {{ planTier?: string, verticalKey?: string }} [options]
 * @returns {{
 *   package: DomainPackageDefinition | null,
 *   planTier: string,
 *   limits: Record<string, number>,
 *   settingsPatch: Record<string, unknown>,
 *   category: string | null,
 * }}
 */
export function buildRegistrationFromDomainPackage(packageKey, options = {}) {
  const pkg = getDomainPackage(packageKey);
  if (!pkg) {
    return {
      package: null,
      planTier: resolvePlanTier(options.planTier || 'free'),
      limits: mergePlanLimits(options.planTier || 'free'),
      settingsPatch: {},
      category: options.verticalKey || null,
    };
  }

  const incomingTier = resolvePlanTier(options.planTier || 'free');
  const recommended = resolvePlanTier(pkg.recommendedPlanTier);
  const planTier =
    (PLAN_TIERS[incomingTier] && incomingTier !== 'free' ? incomingTier : recommended) || recommended;

  const limits = mergePlanLimits(planTier, pkg.limitOverrides);

  const verticalKey = (() => {
    if (!options.verticalKey) return pkg.defaultVertical;
    const incoming = resolveDomainKey(String(options.verticalKey).trim());
    const matched = pkg.verticals.find((v) => resolveDomainKey(v) === incoming);
    return matched || pkg.defaultVertical;
  })();

  let settingsPatch = {
    domain_package: {
      key: pkg.key,
      name: pkg.name,
      applied_at: new Date().toISOString(),
      vertical: verticalKey,
    },
  };

  if (pkg.packaging?.mode === 'custom' && pkg.packaging.feature_overrides) {
    const { nextSettings } = mergePackagingIntoBusinessSettings(settingsPatch, {
      mode: 'custom',
      featureOverrides: pkg.packaging.feature_overrides,
    });
    settingsPatch = nextSettings;
  }

  // Align packaging with vertical knowledge (e.g. textile-wholesale: batch on, manufacturing off).
  settingsPatch = applyRegistrationVerticalPackaging(settingsPatch, verticalKey);

  return {
    package: pkg,
    planTier,
    limits,
    settingsPatch,
    category: verticalKey,
  };
}

/**
 * Registration URL with package + vertical preselection.
 * @param {string} packageKey
 * @param {{ vertical?: string, plan?: string }} [query]
 */
export function getDomainPackageRegisterHref(packageKey, query = {}) {
  const pkg = getDomainPackage(packageKey);
  const params = new URLSearchParams();
  params.set('package', packageKey);
  if (query.vertical) {
    params.set('domain', query.vertical);
  } else if (pkg?.defaultVertical) {
    params.set('domain', pkg.defaultVertical);
  }
  if (query.plan) {
    params.set('plan', query.plan);
  } else if (pkg?.recommendedPlanTier) {
    params.set('plan', pkg.recommendedPlanTier);
  }
  return `/register?${params.toString()}`;
}
