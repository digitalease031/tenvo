...
export const BUSINESS_DOMAINS = {
    ...
    'ev-bikes': {
        ...
        name_ur: 'ای وی بائیکس اور اسکوٹرز',
        icon: 'Zap',
        description: 'Electric motorcycle & scooter showroom, lithium battery tracking, test-ride bookings, and retail POS',
        tax_config: {
            default_tax_rate: 18,
            tax_label: 'GST',
            withholding_enabled: false,
        ...
        default_units: ['unit', 'set', 'pcs', 'pack'],
        default_categories: [
            'Electric Motorcycles', 'Urban EV Scooters', 'Long-Range Commuters',
            'High-Performance EV', 'Batteries & Chargers', 'Accessories & Spares',
        ],
        required_modules: ['invoicing', 'purchases', 'customers', 'vendors', 'basic_accounting'],
        recommended_modules: ['pos', 'serial_tracking', 'batch_tracking', 'quotations', 'expense_tracking', 'multi_warehouse'],
        label_overrides: {
            product: 'EV Bike / Scooter',
            sku: 'VIN / Frame No',
            serial_number: 'Battery / Motor Serial',
            invoice: 'Sales Invoice',
        },
    },
    ...
    'construction-contractor': {
        ...
        name_ur: 'کنسٹرکشن اینڈ انجینئرنگ',
        icon: 'Building2',
        description: 'General contractor, civil engineering, cGMP cleanrooms, BOQ items, PEC/PPRA compliance, IPC running bills',
        tax_config: {
            default_tax_rate: 17,
            tax_label: 'GST / WHT',
            withholding_enabled: true,
        ...
        default_units: ['Ton', 'Cu.M', 'Sq.Ft', 'Bag', 'Rft', 'L.S', 'Liters', 'Hours'],
        default_categories: [
            'Civil & Structural',
            'Finishing & Masonry',
            'Road & Bitumen',
        ...
        recommended_modules: ['multi_warehouse', 'batch_tracking', 'expense_tracking', 'delivery_challans', 'quotations'],
        label_overrides: {
            product: 'BOQ Line Item',
            invoice: 'IPC Running Bill',
            purchase: 'Material Purchase Order',
            customer: 'Client / Employer',
            vendor: 'Subcontractor / Supplier',
            warehouse: 'Site Yard / Godown',
        },
    },
...
function getInstallmentSet() {
    if (!_installmentSet) _installmentSet = new Set(_INSTALLMENT_LIST);
    return _installmentSet;
}
...
export const INSTALLMENT_RELEVANT_DOMAINS = { has: (v) => getInstallmentSet().has(v) };
 ...
 * @property {boolean} [installmentMode]
 * @property {boolean} [installmentPlansEnabled]
 */
 ...
 * @param {DomainKnowledgeClassification|null|undefined} [domainKnowledge]
 */
export function isInstallmentRelevant(category, domainKnowledge = null) {
    const key = resolveDomainKey(category);
    if (key === 'lubricant-distribution' || key === 'milk-shop' || key === 'water-delivery' || key === 'restaurant-cafe' || key === 'pharmacy-retail' || key === 'supermarket') {
        if (!domainKnowledge?.installmentPlansEnabled && !domainKnowledge?.installmentMode) {
            return false;
        }
    }
    if (getInstallmentSet().has(key) || getInstallmentSet().has(category)) return true;
    return !!(domainKnowledge?.installmentMode || domainKnowledge?.installmentPlansEnabled);
}
