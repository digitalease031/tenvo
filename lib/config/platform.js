...
// Built-in founder/owner account. Always recognized as platform owner even when
// no env is configured, so the control panel is reachable out of the box.
// Additional owners can be added via PLATFORM_OWNER_EMAILS (comma-separated).
// NOTE: kept as a literal (not env-only) so client-side detection works too —
// Next.js does not expose non-`NEXT_PUBLIC_` env vars to the browser bundle.
const DEFAULT_OWNER_EMAILS = ['zeeshan.keerio@mindscapeanalytics.com'];
...
function collectOwnerEmails() {
    ...
    const fromEnv = parts
        ...
        .filter(Boolean);
    return new Set([
        ...DEFAULT_OWNER_EMAILS.map((e) => e.toLowerCase()),
        ...fromEnv,
    ]);
}
...
/**
 * Check if a user is an affiliate partner / demo presenter.
 * Partner users have user.role === 'partner' or user.role === 'affiliate'.
 * They have access to the Partner Showcase (/partner) and all registered Demo Stores (is_demo = true),
 * but are strictly blocked from super-admin controls (/admin) and private client businesses.
 * 
...
export function isPartnerUser(user) {
    ...
    return role === 'partner' || role === 'affiliate';
}

/**
 * Inboxes that receive new registration approval emails.
 * Prefer REGISTRATION_APPROVAL_NOTIFY_EMAILS so tenant owner logins are not flooded
 * when the same address is also listed in PLATFORM_OWNER_EMAILS.
 * @returns {string[]}
 */
export function getRegistrationApprovalNotifyEmails() {
    const dedicated = (process.env.REGISTRATION_APPROVAL_NOTIFY_EMAILS || '')
        .split(',')
        .map((s) => s.trim().toLowerCase())
        .filter(Boolean);
