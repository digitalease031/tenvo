...
export function DataProvider({ children }) {
    ...
    const applyHubShellPayload = useCallback((payload, { markReady = true, paintedKey = null, businessId: payloadBusinessId = null } = {}) => {
        ...
        setInvoices((prev) => {
            const shellInvoices = Array.isArray(payload.invoices) ? payload.invoices : [];
            if (!Array.isArray(prev) || prev.length === 0) {
                return shellInvoices;
            }
            const map = new Map(prev.map((inv) => [inv.id, { ...inv }]));
            for (const shellInv of shellInvoices) {
                if (!shellInv?.id) continue;
                if (map.has(shellInv.id)) {
                    const existing = map.get(shellInv.id);
                    map.set(shellInv.id, {
                        ...shellInv,
                        ...existing,
                        // Update basic header properties from shell
                        invoice_number: shellInv.invoice_number || existing.invoice_number,
                        grand_total: shellInv.grand_total ?? existing.grand_total,
                        // Preserve locally updated balance & payment status if present
                        balance: existing.balance !== undefined ? existing.balance : shellInv.balance,
                        payment_status: existing.payment_status || shellInv.payment_status,
                        status: existing.status || shellInv.status,
                        items: (existing.items && existing.items.length > 0) ? existing.items : (shellInv.items || []),
                        customer_name: existing.customer_name || shellInv.customer_name,
                    });
                } else {
                    map.set(shellInv.id, shellInv);
                }
            }
        ...
        if (salesListDepthRef.current !== 'invoices' && salesListDepthRef.current !== 'full') {
    ...
    const fetchPurchases = useCallback(async ({ force = false } = {}) => {
        ...
        try {
            const [vendResult, poResult] = await Promise.allSettled([
                vendorAPI.getAll(businessId),
                purchaseAPI.getAll(businessId),
            ]);
            if (isStale()) return;

            if (vendResult.status === 'fulfilled') {
                setVendors(vendResult.value || []);
            } else {
                console.error('Fetch Vendors Error:', vendResult.reason);
                setVendors([]);
            }
            ...
            if (poResult.status === 'fulfilled') {
                ...
                console.error('Fetch PurchaseOrders Error:', poResult.reason);
                setPurchaseOrders([]);
            }
    ...
    const upsertInvoiceInState = useCallback((invoice) => {
        ...
        if (businessId) {
            ...
            queryClient.setQueriesData({ queryKey: ['hubShell', businessId] }, (oldData) => {
                if (!oldData || !Array.isArray(oldData.invoices)) return oldData;
                const idx = oldData.invoices.findIndex((inv) => inv?.id === invoice.id);
                const clean = {};
                for (const [k, v] of Object.entries(invoice)) {
                    if (v !== undefined) clean[k] = v;
                }
                const nextInvoices = oldData.invoices.slice();
                if (idx === -1) {
                    nextInvoices.unshift(clean);
                } else {
                    nextInvoices[idx] = { ...nextInvoices[idx], ...clean };
                }
                return { ...oldData, invoices: nextInvoices };
            });
    ...
    const removeInvoiceFromState = useCallback((invoiceId) => {
        ...
        if (businessId) {
            ...
            queryClient.setQueriesData({ queryKey: ['hubShell', businessId] }, (oldData) => {
                ...
                return {
                    ...oldData,
                    invoices: oldData.invoices.filter((inv) => inv?.id !== invoiceId),
                };
            });
            void queryClient.invalidateQueries({ queryKey: ['hubAnalytics', businessId] });
            void queryClient.invalidateQueries({ queryKey: ['hubSalesPerformance', businessId] });
