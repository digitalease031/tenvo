...
/** @typedef {'pharmacy_desk'|'clinical_ops'|'showroom'|'hospitality'|'online_store'|'parts_desk'|'manufacturing'|'construction_ops'|'operations'} OperationsMode */
...
const CATEGORY_MODE = {
  ...
  'construction-contractor': 'construction_ops',
  contractor: 'construction_ops',
  construction: 'construction_ops',
  'civil-engineering': 'construction_ops',
  infrastructure: 'construction_ops',
  epc: 'construction_ops',
};
...
const CATEGORY_TAB_LABELS = {
  ...
  'construction-contractor': 'Site ops',
  contractor: 'Site ops',
  construction: 'Site ops',
  'civil-engineering': 'Site ops',
  infrastructure: 'Site ops',
  epc: 'Site ops',
};
...
const MODE_GUIDANCE = {
  ...
  construction_ops:
    'Site material stock (rebar, cement, aggregate), heavy equipment hour-logs, IPC running bills, and PEC/PPRA compliance.',
  operations:
...
function buildRegionalComplianceHint(countryIso, mode, category) {
  ...
  if (mode === 'construction_ops' && iso === 'PK') {
    return 'Pakistan construction: verify PEC license category, FBR Sec 153(1)(c) WHT, and PRA/SRB/KPRA/BRA tax on IPC running bills.';
  }
...
export function resolveOperationsProfile(category, domainKnowledge, business) {
  ...
  return {
    ...
    showConstruction: mode === 'construction_ops' || key === 'construction-contractor',
...
export function buildOperationsKpiTiles(profile, snapshot, formatters) {
  ...
  if (profile.showConstruction) {
    const construction = /** @type {Record<string, unknown>} */ (snapshot.constructionOps || {});
    tiles.push({
      id: 'active_projects',
      label: 'Active Projects',
      value: (Number(construction.activeProjects || 1)) + ' EPC Project',
      hint: 'PEC C-A / C-1 registered execution',
      actionTab: 'projects',
      tone: 'text-[#a71930]',
    });
    tiles.push({
      id: 'ipc_running_bills',
      label: 'Pending IPC Bills',
      value: (Number(construction.pendingIpcCount || 1)) + ' IPC Bill',
      hint: 'FIDIC running bill & WHT tax recovery',
      actionTab: 'invoices',
      tone: 'text-amber-700',
    });
    tiles.push({
      id: 'machinery_logbook',
      label: 'Fleet & Fuel Log',
      value: (Number(construction.machineryFleetCount || 8)) + ' Machines',
      hint: 'Hours & fuel consumption tracking',
      actionTab: 'inventory',
    });
    tiles.push({
      id: 'boq_steel_cement',
      label: 'BOQ Steel & Cement',
      value: 'Grade 60 ASTM',
      hint: 'Rebar, OPC cement & RMC C25/C30',
      actionTab: 'inventory',
    });
  }
  ...
  if (profile.showConstruction) {
    const constOps = /** @type {Record<string, unknown>} */ (snapshot.constructionOps || {});
    tiles.push({
      id: 'active_projects',
      label: 'Active Projects',
      value: Number(constOps.activeProjects || 1),
      hint: 'PEC registered contract works',
      actionTab: 'projects',
    });
    tiles.push({
      id: 'ipc_running_bills',
      label: 'IPC Certified Billing',
      value: formatCurrency(Number(constOps.ipcCertifiedTotal || sf.revenue || 0)),
      hint: 'Cumulative certified work done',
      actionTab: 'invoices',
    });
    tiles.push({
      id: 'retention_held',
      label: 'Retention Money Held',
      value: formatCurrency(Number(constOps.retentionHeld || 0)),
      hint: '5-10% holdback balance',
      actionTab: 'invoices',
      tone: 'text-amber-700',
    });
    tiles.push({
      id: 'machinery_logbook',
      label: 'Equipment Fuel Log',
      value: `${Number(constOps.totalMachineryHours || 0)} hrs`,
      hint: 'Heavy plant fuel & hour-meter',
      actionTab: 'manufacturing',
    });
  }
