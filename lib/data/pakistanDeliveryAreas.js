...
export function waterDeliveryCadenceCoversDate(cadence, date = new Date()) {
  ...
  if (/^custom$/i.test(raw)) return true; // Manual tracking - show on all days
  ...
  // Weekend preset (Sat-Sun)
  if (/^(sat[\s-]*sun|sat\s*sun|weekends?)$/i.test(raw)) return isWeekend;
  ...
  if (lower.includes('only')) {
    // "Monday only", "Saturday only", etc. - check if today matches
    return lower.includes(todayName);
  }
  
  // Legacy "Alternate Days" pattern - default to Mon/Wed/Fri for backward compatibility
  if (/^alternate\s*days?$/i.test(raw)) {
    return day === 1 || day === 3 || day === 5; // Mon=1, Wed=3, Fri=5
  }
  
  // Parse space/comma/dash-separated day list: "Mon Wed Fri", "Tue-Thu-Sat", "Mon-Wed-Fri"
  const tokens = raw
    .toLowerCase()
    .replace(/[,\-]/g, ' ')
    .split(/\s+/)
  ...
  if (!wanted.size) return true; // Fallback: no valid days parsed, show on all days
