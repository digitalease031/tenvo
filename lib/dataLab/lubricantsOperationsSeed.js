 ...
 * @param {any} clientOrBusinessId
 * @param {string} [businessIdArg]
 */
export async function seedLubricantsOperations(clientOrBusinessId, businessIdArg) {
  let dbClient = typeof clientOrBusinessId === 'object' && clientOrBusinessId ? clientOrBusinessId : null;
  let businessId = typeof clientOrBusinessId === 'string' ? clientOrBusinessId : businessIdArg;
  let ownPool = null;
  ...
  try {
    if (!dbClient) {
      ownPool = createPool();
      dbClient = await ownPool.connect();
    }

    // 1. Ensure Warehouses / Mobile Vans
    const whRes = await dbClient.query(
      `SELECT id, name FROM warehouse_locations WHERE business_id = $1::uuid`,
      [businessId]
    );
    ...
    if (!mainWhId) {
      const insWh = await dbClient.query(
        `INSERT INTO warehouse_locations (business_id, name, code, is_primary)
    ...
    const van1 = await dbClient.query(
      `INSERT INTO warehouse_locations (business_id, name, code, is_primary)
    ...
    const van2 = await dbClient.query(
      `INSERT INTO warehouse_locations (business_id, name, code, is_primary)
    ...
    for (const v of LUBRICANT_SEED_VENDORS) {
      ...
      if (existing.rows[0]) {
        ...
        const ins = await dbClient.query(
          `INSERT INTO vendors (business_id, name, email, phone, address, ntn, payment_terms, contact_person)
           VALUES ($1::uuid, $2, $3, $4, $5, $6, $7, $8)
           RETURNING id`,
          [businessId, v.name, v.email, v.phone, v.address, v.ntn, v.terms, v.contact_person]
        );
    ...
    const prodRes = await dbClient.query(
      `SELECT id, name, price, cost_price, sku FROM products WHERE business_id = $1::uuid LIMIT 25`,
      [businessId]
    );
    ...
    if (shellVendorId) {
      await dbClient.query(
        `INSERT INTO purchases (business_id, vendor_id, purchase_number, date, status, total_amount, notes, created_at)
         VALUES ($1::uuid, $2::uuid, 'PO-OIL-2026-001', CURRENT_DATE - INTERVAL '15 days', 'received', 5240000, 'Monthly Shell Helix & Rimula bulk consignment', NOW() - INTERVAL '15 days')
    ...
    if (caltexVendorId) {
      await dbClient.query(
        `INSERT INTO purchases (business_id, vendor_id, purchase_number, date, status, total_amount, notes, created_at)
         VALUES ($1::uuid, $2::uuid, 'PO-OIL-2026-002', CURRENT_DATE - INTERVAL '10 days', 'received', 3850000, 'Caltex Havoline Formula & Delo 400 shipment', NOW() - INTERVAL '10 days')
         ON CONFLICT DO NOTHING`,
    ...
    if (guardVendorId) {
      await dbClient.query(
        `INSERT INTO purchases (business_id, vendor_id, purchase_number, date, status, total_amount, notes, created_at)
         VALUES ($1::uuid, $2::uuid, 'PO-FLT-2026-003', CURRENT_DATE - INTERVAL '5 days', 'received', 1250000, 'Guard Oil, Air & Fuel Filters wholesale drop', NOW() - INTERVAL '5 days')
         ON CONFLICT DO NOTHING`,
    ...
    if (khanCustId && p1 && p4) {
      const invRes = await dbClient.query(
        `INSERT INTO invoices (business_id, customer_id, invoice_number, status, payment_status, subtotal, tax_total, grand_total, total_amount, date, due_date)
         VALUES ($1::uuid, $2::uuid, 'INV-OIL-2026-0101', 'partially_paid', 'partially_paid', 850000, 153000, 1003000, 1003000, CURRENT_DATE - INTERVAL '12 days', CURRENT_DATE + INTERVAL '18 days')
         ON CONFLICT DO NOTHING RETURNING id`,
      ...
      if (invRes.rows[0]) {
        ...
        await dbClient.query(
          `INSERT INTO invoice_items (business_id, invoice_id, product_id, name, description, quantity, unit_price, total_amount)
           VALUES ($1::uuid, $2::uuid, $3::uuid, $4, $4, 20, $5, $6)`,
          [businessId, invId, p1.id, p1.name, p1.price || 28800, (p1.price || 28800) * 20]
        );
        await dbClient.query(
    ...
    if (madinaCustId && p2 && p3) {
      const invRes = await dbClient.query(
        `INSERT INTO invoices (business_id, customer_id, invoice_number, status, payment_status, subtotal, tax_total, grand_total, total_amount, date, due_date)
         VALUES ($1::uuid, $2::uuid, 'INV-OIL-2026-0102', 'paid', 'paid', 420000, 75600, 495600, 495600, CURRENT_DATE - INTERVAL '8 days', CURRENT_DATE + INTERVAL '7 days')
         ON CONFLICT DO NOTHING RETURNING id`,
      ...
      if (invRes.rows[0]) {
        ...
        await dbClient.query(
          `INSERT INTO invoice_items (business_id, invoice_id, product_id, name, description, quantity, unit_price, total_amount)
           VALUES ($1::uuid, $2::uuid, $3::uuid, $4, $4, 10, $5, $6)`,
          [businessId, invId, p2.id, p2.name, p2.price || 25200, (p2.price || 25200) * 10]
        );
        await dbClient.query(
          `INSERT INTO invoice_payments (business_id, invoice_id, amount, payment_method, notes, payment_date)
    ...
    if (fleetCustId && products.length > 5) {
      ...
      const invRes = await dbClient.query(
        `INSERT INTO invoices (business_id, customer_id, invoice_number, status, payment_status, subtotal, tax_total, grand_total, total_amount, date, due_date)
         VALUES ($1::uuid, $2::uuid, 'INV-OIL-2026-0103', 'unpaid', 'unpaid', 1820000, 327600, 2147600, 2147600, CURRENT_DATE - INTERVAL '20 days', CURRENT_DATE + INTERVAL '10 days')
         ON CONFLICT DO NOTHING RETURNING id`,
      ...
      if (invRes.rows[0]) {
        ...
        await dbClient.query(
          `INSERT INTO invoice_items (business_id, invoice_id, product_id, name, description, quantity, unit_price, total_amount)
           VALUES ($1::uuid, $2::uuid, $3::uuid, $4, $4, 50, $5, $6)`,
          [businessId, invId, pFleet.id, pFleet.name, pFleet.price || 31200, (pFleet.price || 31200) * 50]
        );
      }
    }

    // 8. Create Operating Expenses
    const glRes = await dbClient.query(
      `SELECT id FROM gl_accounts WHERE business_id = $1::uuid LIMIT 1`,
      [businessId]
    );
    const accountId = glRes.rows[0]?.id;
    if (accountId) {
      for (const exp of LUBRICANT_EXPENSES_SEED) {
        await dbClient.query(
          `INSERT INTO expenses (business_id, account_id, category, description, amount, date)
           VALUES ($1::uuid, $2::uuid, $3, $4, $5, CURRENT_DATE - INTERVAL '4 days')
           ON CONFLICT DO NOTHING`,
          [businessId, accountId, exp.category, exp.description, exp.amount]
        );
      }
    }
  } catch (err) {
