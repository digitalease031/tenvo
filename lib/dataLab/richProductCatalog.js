...
import { GARMENTS_SEED_PRODUCTS, TEXTILE_MILL_SEED_PRODUCTS, TEXTILE_WHOLESALE_IMPORTED_SUPPLEMENT } from './pakistanClothingSeedCatalog.js';
import { CONSTRUCTION_CONTRACTOR_SEED_PRODUCTS } from './constructionContractorCatalog.js';
import { EV_BIKES_SEED_PRODUCTS } from './evBikesDemoCatalog.js';
import { LUBRICANT_DISTRIBUTION_SEED_PRODUCTS } from './catalogs/lubricantsSeedCatalog.js';
import { HARDWARE_SEED_PRODUCTS } from './catalogs/hardwareDemoCatalog.js';
...
export const RICH_PRODUCT_CATALOG = {
  ...
  'hardware-sanitary': HARDWARE_SEED_PRODUCTS,
  ...
  'construction-contractor': CONSTRUCTION_CONTRACTOR_SEED_PRODUCTS,
  'ev-bikes': EV_BIKES_SEED_PRODUCTS,
  'auto-parts': AUTO_PARTS_SEED_CATALOG,
  'lubricant-distribution': LUBRICANT_DISTRIBUTION_SEED_PRODUCTS,
};
...
export function buildRichSeedItems({ businessId, domainKey, countryIso, taxRate, brands = [] }) {
  ...
  return rows.map((item, index) => {
    const price =
      ...
      item.retail_price ??
      item.display_price ??
      item.domain_data?.price ??
      item.domain_data?.trade_price ??
      0;
    const compare_price =
