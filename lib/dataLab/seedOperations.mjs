...
import { seedConstructionOperations } from './constructionOperationsSeed.js';
import { seedLubricantsOperations } from './lubricantsOperationsSeed.js';
...
export async function seedOperationalData({ businessId, userId, sampleBatchId = null, replaceSampleOps = false }) {
  ...
  try {
    return await withTransaction(client, async (tx) => {
      ...
      // Seed construction operations for construction domains
      let constructionProjects = 0;
      let constructionBOQ = 0;
      let constructionIPCs = 0;
      let constructionMachinery = 0;
      let constructionSiteOps = 0;
      let constructionSubcontractors = 0;

      const isConstructionDomain = ['construction-contractor', 'construction', 'civil-engineering'].includes(
        String(category).toLowerCase()
      );

      if (isConstructionDomain) {
        try {
          const constructionResult = await seedConstructionOperations(businessId);
          if (constructionResult?.success) {
            // Count seeded records
            const projectsRes = await tx.query(
              `SELECT COUNT(*) as count FROM construction_projects WHERE business_id = $1::uuid`,
              [businessId]
            );
            constructionProjects = parseInt(projectsRes.rows[0]?.count || 0);

            const boqRes = await tx.query(
              `SELECT COUNT(*) as count FROM construction_boq_items WHERE business_id = $1::uuid`,
              [businessId]
            );
            constructionBOQ = parseInt(boqRes.rows[0]?.count || 0);

            const ipcRes = await tx.query(
              `SELECT COUNT(*) as count FROM construction_ipcs WHERE business_id = $1::uuid`,
              [businessId]
            );
            constructionIPCs = parseInt(ipcRes.rows[0]?.count || 0);

            const machineryRes = await tx.query(
              `SELECT COUNT(*) as count FROM construction_machinery WHERE business_id = $1::uuid`,
              [businessId]
            );
            constructionMachinery = parseInt(machineryRes.rows[0]?.count || 0);

            const siteOpsRes = await tx.query(
              `SELECT COUNT(*) as count FROM construction_site_operations WHERE business_id = $1::uuid`,
              [businessId]
            );
            constructionSiteOps = parseInt(siteOpsRes.rows[0]?.count || 0);

            const subcontractorsRes = await tx.query(
              `SELECT COUNT(*) as count FROM construction_subcontractors WHERE business_id = $1::uuid`,
              [businessId]
            );
            constructionSubcontractors = parseInt(subcontractorsRes.rows[0]?.count || 0);

            console.log(
              `  ✓ Construction: ${constructionProjects} projects, ${constructionBOQ} BOQ items, ${constructionIPCs} IPCs`
            );
          }
        } catch (constructionError) {
          console.warn(`  ⚠️  Construction seeding skipped:`, constructionError.message);
        }
      }

      if (resolveDomainKey(category) === 'lubricant-distribution') {
        try {
          await seedLubricantsOperations(tx, businessId);
          console.log(`  ✓ Lubricant Distribution: Seeded vendors, customers, POs, invoices, PDCs, and expenses`);
        } catch (lubricantError) {
          console.warn(`  ⚠️  Lubricant operations seeding skipped:`, lubricantError.message);
        }
      }
