...
import { isCampaignRelevant, isHospitality, isMembershipRelevant, isPosRelevant, isInstallmentRelevant } from '@/lib/config/domains';
...
/**
 * Lean primary nav for water-delivery and milk-shop domains on mobile.
 * Starter+: Route (Daily Route) becomes the primary action.
 * Finance/Reports/Settings move to More.
 */
...
const OVERFLOW_CANDIDATES = [
  ...
  { key: 'installments', label: 'Installments', icon: Calculator, domainRule: 'installmentRelevant' },
...
function resolveNavVisibility(item, ctx) {
  ...
  if (item.domainRule === 'installmentRelevant' && !ctx.installmentRelevant) {
    return { visible: false, locked: false };
  }
...
export function useHubMobileNav() {
  ...
  const primaryItems = useMemo(() => {
    ...
    // This gives them a clean, app-like experience with Route as the primary action
  ...
  const overflowItems = useMemo(() => {
    // Determine which primary keys to exclude from overflow
    let primaryKeys;
    if (isWaterOrMilkDomain) {
      primaryKeys = new Set(
        WATER_MILK_LEAN_PRIMARY.map((i) => i.key).filter((k) => k !== '__more__')
      );
    } else {
      primaryKeys = new Set(
    ...
    return OVERFLOW_CANDIDATES.map(filterItem).filter(
      (i) => i.visible && !i.locked && !primaryKeys.has(i.key)
    );
  }, [isEasyMode, isWaterOrMilkDomain, ctx, safeIsPlatformAdmin]);
