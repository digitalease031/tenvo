...
import {
  isConstructionStore,
  buildDefaultConstructionStorefrontSettings,
  getDefaultConstructionBusinessMedia,
  CONSTRUCTION_REGISTRATION_METADATA,
} from '../storefront/constructionStorefront.js';
...
export function resolveRegistrationStorefrontDefaults({
  ...
  if (isConstructionStore(canonical)) {
    const vertical = buildDefaultConstructionStorefrontSettings(businessName);
    const description = trimmedDescription || CONSTRUCTION_REGISTRATION_METADATA.description;
    return withMediaAndSeo(pickBusinessMediaAndSeo(getDefaultConstructionBusinessMedia()), {
      businessDescription: description,
      heroSubtitle: description,
      paymentCodInstructions: null,
      storefrontExtras: {
  ...
  if (canonical === 'auto-parts') {    const vertical = buildDefaultAutoPartsStorefrontSettings(businessName);
  ...
  if (canonical === 'marine-parts') {    const vertical = buildDefaultMarinePartsStorefrontSettings(businessName);
