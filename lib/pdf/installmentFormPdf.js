...
export async function generateInstallmentFormPdf({
  ...
  const w1Rel = applicant.witness1Relation || '____________';
  const w1City = applicant.witness1City || '____________';

  const w2Name = applicant.witness2Name || '';
  const w2Phone = applicant.witness2Phone || '';
  const w2Cnic = applicant.witness2Cnic || '';
  const w2Rel = applicant.witness2Relation || '____________';
  const w2City = applicant.witness2City || '____________';
  ...
  doc.text(`Relation: ${w1Rel}  City: ${w1City}`, margin + 3, y + 23);
  ...
  doc.text(`Relation: ${w2Rel}  City: ${w2City}`, margin + colW + 7, y + 23);
