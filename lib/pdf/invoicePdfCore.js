...
export function buildInvoicePDFDoc(doc, isPakistaniDomain = true) {
  ...
  // ── Dispatch & Transport (Bilti) Section ──────────────────────────────────
  const bilti = doc.biltiDetails;
  if (bilti && typeof bilti === 'object') {
    currentY += 6;
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(9);
    pdf.setTextColor(...brand);
    pdf.text('DISPATCH & TRANSPORT DETAILS (BILTI)', PAGE_MARGIN, currentY);
    currentY += 4;
    ...
    if (bilti.baleCount) biltiParts.push(`Bales / Packages: ${bilti.baleCount}`);
  ...
  if (vehicleAgreement && typeof vehicleAgreement === 'object') {
    ...
    const sellerPhoneVal = vehicleAgreement.sellerPhone || business?.phone || '-';
    ...
    const vehicleSpecText = [
      ...
      `Buyer: Phone ${buyerPhoneVal}  |  CNIC ${buyerCnicVal}`,
      `Seller: Phone ${sellerPhoneVal}  |  CNIC ${sellerCnicVal}   |   Token Tax: ${vehicleAgreement.tokenTaxStatus || 'Paid'}`,
    ];
