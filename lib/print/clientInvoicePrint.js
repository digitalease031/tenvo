...
export async function downloadInstallmentFormPdfFromRow(
  ...
  const agreement =
    full.vehicleAgreement ||
    full.vehicle_meta ||
    full.taxDetails?.vehicleAgreement ||
    full.tax_details?.vehicleAgreement ||
    full.domain_data?.vehicleAgreement ||
    {};
  ...
  const pdfDoc = await generateInstallmentFormPdf({
    ...
    applicant: {
      ...
      address: agreement.buyerAddress || customer.address || '',
      city: customer.city || '',
