...
/**
 * Human-readable label for fabric/textile measurement units.
 * Keeps raw keys (meter, gaz, thaan, guth, suit) out of printed receipts.
 */
const RECEIPT_UNIT_LABELS = {
  meter: 'Mtr',
  metre: 'Mtr',
  gaz: 'Gaz',
  yard: 'Yrd',
...
/** Format a unit key into a compact receipt label (e.g. "gaz" → "Gaz"). */
function formatReceiptUnit(unit) {
  if (!unit) return null;
  const key = String(unit).trim().toLowerCase();
  return RECEIPT_UNIT_LABELS[key] || (key.charAt(0).toUpperCase() + key.slice(1));
}
...
export function normalizeReceiptData({
  ...
  const rows = (lineItems || []).map((item) => ({
    ...
    unit: String(item.unit || '').trim().toLowerCase() || null,
...
function getReceiptStyles(paperSize = '58mm', pageHeightMm = 120) {
  ...
  const PW_PX = paperSize === '80mm' ? 302 : 219;
  const pageH = Math.min(Math.max(Number(pageHeightMm) || 120, 50), 320);
  return `
  @page { size: ${pageW}mm ${pageH}mm; margin: 0 !important; }
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media print {
    @page { size: ${pageW}mm ${pageH}mm; margin: 0 !important; }
    html { width: ${pageW}mm !important; max-width: ${pageW}mm !important; }
    body {
      width: ${PW_PX}px !important;
      max-width: ${PW_PX}px !important;
      margin: 0 !important;
  ...
  .muted { color: #666; }
  .meta { margin: 2mm 0; }
  .meta-row { display: flex; justify-content: space-between; gap: 1mm; font-size: 8.5px; }
  ...
  .items th.col-item, .items td.col-item { text-align: left; width: 54%; }
  .items th.col-qty, .items td.col-qty { text-align: right; width: 16%; }
  .items th.col-amt, .items td.col-amt { text-align: right; width: 30%; }
  .items tbody td {
    vertical-align: top;
    padding: 1.5mm 0;
    font-size: 8.5px;
  ...
  .unit-label { font-size: 7.5px; color: #555; font-weight: 400; }
  .totals { margin-top: 2mm; font-size: 8.5px; }
  .totals td { padding: 0.8mm 0; vertical-align: top; }
  .totals td.label { text-align: left; color: #444; width: 50%; }
  .totals td.value { text-align: right; width: 50%; white-space: nowrap; font-variant-numeric: tabular-nums; }
...
export function buildThermalReceiptHtml(opts) {
  ...
  const itemRows = d.rows
    ...
    <tr>
      ...
      <td class="col-qty">${esc(String(row.quantity))}${row.unit ? `<span class="unit-label"> ${esc(formatReceiptUnit(row.unit))}</span>` : ''}</td>
...
function renderThermalReceiptOntoPdf(jsPDF, autoTable, d, pageW, pageH) {
  ...
  autoTable(doc, {
    ...
    body: d.rows.map((row) => [
      row.sku ? `${row.name}\n${row.sku}` : row.name,
      row.unit ? `${row.quantity} ${formatReceiptUnit(row.unit)}` : String(row.quantity),
      moneyNum(row.amount),
    ]),
    theme: 'plain',
    styles: {
