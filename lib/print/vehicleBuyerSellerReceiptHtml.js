...
export function resolveVehicleAgreementDetails(invoice = {}, business = {}) {
  ...
  const agreement = invoice.vehicleAgreement || invoice.vehicle_meta || invoice.taxDetails?.vehicleAgreement || {};
  ...
  const sellerAddress = formatBusinessAddressBlock(business).join(', ') || String(business.address || '').trim();
  ...
  const buyerAddress = String(customer.address || '').trim();
