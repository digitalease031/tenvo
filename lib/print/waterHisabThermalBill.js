...
export function buildWaterPeriodBillHtml(model) {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .row { display: flex; justify-content: space-between; align-items: baseline; gap: 1.5mm; font-size: 8px; padding: 0.55mm 0; }
...
export function buildWater58mmAllCustomersSummaryHtml({
  ...
  // Column widths for 58mm thermal (32 chars usable):
  // CUST(12) DEL(3) REC(3) BAL(3) CASH(6) = 27 chars + spacing
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body {
    width: ${pageW}; max-width: ${pageW}; margin: 0 auto;
    padding: 2.5mm 2mm 4mm;
    font-family: ${fontStack};
    font-size: 8px; line-height: 1.28; color: #111;
    -webkit-print-color-adjust: exact; print-color-adjust: exact;
  }
  .c { text-align: center; }
  .b { font-weight: 700; }
  ...
  .grid {
    ...
    line-height: 1.42; font-variant-numeric: tabular-nums;
  }
  .grid .hdr { 
    font-weight: 700; border-bottom: 1px solid #333; 
    padding-bottom: 0.6mm; margin-bottom: 0.6mm; 
  }
  .grid .row { padding: 0.4mm 0; }
  .totals {
    font-family: ${fontStack};
    font-size: 7.5px; white-space: pre; letter-spacing: 0;
    font-variant-numeric: tabular-nums; margin-top: 1.5mm;
    border-top: 1px solid #333; padding-top: 1mm;
  }
  .totals .row { padding: 0.5mm 0; }
  .totals .grand { font-weight: 800; font-size: 8.5px; margin-top: 1mm; }
  ...
  // Header line
  const headerLine = `${'CUSTOMER'.padEnd(18)} ${'DEL'.padStart(3)} ${'REC'.padStart(3)} ${'BAL'.padStart(3)}${'CASH'.padStart(6)}`;

  // Totals section
  const totalsLine1 = `${'Total Delivered:'.padEnd(22)}${formatNum(totalDel, 8)}`;
  const totalsLine2 = `${'Total Received:'.padEnd(22)}${formatNum(totalRec, 8)}`;
  const totalsLine3 = `${'Cash Collected:'.padEnd(22)}${formatMoney(totalCash)}`;
  const grandLine = `${'GRAND TOTAL:'.padEnd(22)}${formatMoney(totalAmount)}`;
...
<style>${styles}</style>
  ...
  <hr/>
  <div class="c b" style="font-size:9px;margin-bottom:0.5mm;">${esc(kind === 'week' ? 'WEEKLY' : 'MONTHLY')} BILLS SUMMARY</div>
  <div class="c s b">${esc(periodLabel)}</div>
  <div class="c s" style="color:#666;margin-top:1mm;">${sortedRows.length} customers</div>
  <hr/>
  <div class="grid">
  ...
  <div class="foot">
    ...
    <div class="s" style="color:#666;margin-top:1mm;">Del = Delivered · Rec = Received</div>
