 ...
 */
export function buildWaterAllCustomersBillSummaryHtml(args) {
  const {
    business,
    rows = [],
    productColumns = [],
...
<head>
  ...
  <title>${esc(business.business_name)} - ${esc(periodLabel)} Bills Summary</title>
  <style>
    * {
      margin: 0;
      padding: 0;
  ...
  <table>
    <thead>
      <tr>
        ...
        ${productColumns.map(p => `<th class="text-center">${esc(shortWaterHisabProductLabel(p, 10))}</th>`).join('')}
    ...
    <tbody>
      ${sortedRows.map(row => {
        ...
        return `<tr>
          <td class="col-house">${esc(row.houseNo || '-')}</td>
    ...
    <tfoot>
      <tr class="totals-row">
        <td colspan="${3 + productColumns.length}" class="totals-label">TOTAL (${totalCustomers} customers)</td>
        <td class="totals-amount text-right">${money(totalAmount, currency, locale)}</td>
        <td></td>
      </tr>
    </tfoot>
  </table>
...
export async function printWaterAllCustomersBillSummary(args, mode = 'print') {
  try {
    ...
    if (mode === 'pdf') {
      // Generate PDF using jsPDF and html2canvas for better formatting
