...
async function createWaterPeriodPdf(model, existingDoc = null) {
  ...
  const renderOnDoc = (targetDoc) => {
    ...
    const cleanHouse = d.houseNo && String(d.houseNo).trim() !== '?' && String(d.houseNo).trim() !== 'null' ? String(d.houseNo).trim() : '';
    const houseStr = [cleanHouse, d.floorFlat].filter(Boolean).join(' / ');
...
export function buildWater58mmAllCustomersSummaryHtml({
  ...
  const resolveRowDel = (r) => {
    let d = Number(r.delTotal);
    if ((!Number.isFinite(d) || d === 0) && r.qtyByProduct) {
      d = Object.values(r.qtyByProduct).reduce((a, b) => a + (Number(b) || 0), 0);
    }
    return Math.round(d || 0);
  };
  const resolveRowRec = (r) => Math.round(Number(r.recTotal) || 0);

  sortedRows.forEach(row => {
    totalDel += resolveRowDel(row);
    totalRec += resolveRowRec(row);
    totalAmount += Number(row.amount) || 0;
    totalCash += Number(row.cashCollected) || 0;
  });

  // Dynamic page height: header ~28mm + rows ~2.6mm each + totals ~14mm + footer ~10mm
  const pageH = Math.min(Math.max(Math.ceil(28 + sortedRows.length * 2.6 + 14 + 10), 60), 400);
  const PW_PX = 219;
  const fontStack = `ui-monospace, SFMono-Regular, Menlo, Consolas, monospace`;

  // Column widths for 58mm thermal — 16+1+3+1+3+1+3+1+5 = 34 chars:
  const formatCustomerName = (name, house) => {
    let base = String(name || 'Customer').trim();
    if (house && String(house).trim() !== '?' && String(house).trim() !== 'null') {
      base = `${String(house).trim()} ${base}`;
    }
    return base.slice(0, 16).padEnd(16, ' ');
  };
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body {
    ...
    font-family: ${fontStack};
    font-size: 7.5px; line-height: 1.15; color: #111;
    -webkit-print-color-adjust: exact; print-color-adjust: exact;
  }
  .c { text-align: center; }
  .b { font-weight: 700; }
  ...
  .s { font-size: 6.5px; color: #444; line-height: 1.2; }
  hr { border: none; border-top: 1px dashed #888; margin: 1mm 0; }
  .grid {
    font-family: ${fontStack};
  ...
  .grid .hdr { 
    ...
    padding-bottom: 0.2mm; margin-bottom: 0.3mm; 
  }
  .grid .row { padding: 0; margin: 0; line-height: 1.15; }
  .totals {
    font-family: ${fontStack};
    font-size: 6.5px; white-space: pre; letter-spacing: 0;
    font-variant-numeric: tabular-nums; margin-top: 0.8mm;
    border-top: 1px solid #222; padding-top: 0.6mm;
  }
  .totals .row { padding: 0.2mm 0; line-height: 1.2; }
  .totals .grand { font-weight: 800; font-size: 7.5px; margin-top: 0.5mm; }
  .foot { margin-top: 1.2mm; text-align: center; font-size: 6.5px; line-height: 1.25; }
  `;

  const customerRows = sortedRows.map(row => {
    const cust = formatCustomerName(row.customerName, row.houseNo);
    const del = formatNum(resolveRowDel(row), 3);
    const rec = formatNum(resolveRowRec(row), 3);
    const bal = formatNum(row.bottleBalance, 3);
    const cash = formatMoney(row.cashCollected);
    return `${cust} ${del} ${rec} ${bal} ${cash}`;
  }).join('\n');

  const headerLine = `${'CUSTOMER'.padEnd(16)} ${'DEL'.padStart(3)} ${'REC'.padStart(3)} ${'BAL'.padStart(3)} ${'CASH'.padStart(5)}`;

  const totalsLine1 = `${'Del:'.padEnd(16)} ${formatNum(totalDel, 8)}`;
  const totalsLine2 = `${'Rec:'.padEnd(16)} ${formatNum(totalRec, 8)}`;
  const totalsLine3 = `${'Cash:'.padEnd(16)} ${formatMoney(totalCash).trim().padStart(8)}`;
  const grandLine = `${'GRAND TOTAL:'.padEnd(16)} ${formatMoney(totalAmount).trim().padStart(8)}`;
...
<style>${styles}</style>
  ...
  <div class="foot">
    ...
    <div class="s" style="color:#666;margin-top:0.6mm;">Del = Delivered · Rec = Received</div>
    <div class="s" style="color:#666;">BAL = Customer balance · Cash = Collected</div>
  </div>
</body></html>`;
}

export async function createWater58mmAllCustomersSummaryPdf(args, existingDoc = null) {
  const { default: jsPDF } = await import('jspdf');
  const pageW = 58;
  const margin = 1.8;
  const contentW = pageW - margin * 2;
  const { business = {}, rows = [], periodLabel = '', kind = 'month' } = args || {};
  ...
  const resolvePdfRowRec = (r) => Math.round(Number(r.recTotal) || 0);
  ...
  sortedRows.forEach((row) => {
    totalDel += resolvePdfRowDel(row);
    totalRec += resolvePdfRowRec(row);
    totalAmount += Number(row.amount) || 0;
    totalCash += Number(row.cashCollected) || 0;
  });
  ...
  const renderOnDoc = (targetDoc) => {
    let y = margin + 2;
    const write = (text, opts = {}) => {
      const { size = 7, bold = false, align = 'center', advance } = opts;
      targetDoc.setFont('courier', bold ? 'bold' : 'normal');
      targetDoc.setFontSize(size);
      const x = align === 'center' ? pageW / 2 : align === 'right' ? pageW - margin : margin;
      targetDoc.text(String(text ?? ''), x, y, { align, maxWidth: contentW });
      y += advance != null ? advance : size * 0.38 + 0.9;
    };
    const rule = () => {
      y += 0.5;
      targetDoc.setDrawColor(150);
    ...
    write(business?.business_name || business?.name || 'Water Supply', { size: 9, bold: true });
    if (business?.address) write(business.address, { size: 6.5 });
    if (business?.phone) write(business.phone, { size: 6.5 });
    rule();

    write(`${kind === 'week' ? 'WEEKLY' : 'MONTHLY'} BILLS SUMMARY`, { size: 8, bold: true });
    if (periodLabel) write(periodLabel, { size: 7, bold: true });
    write(`${sortedRows.length} customers`, { size: 6.5 });
    rule();

    const formatCust = (name, house) => {
      let b = String(name || 'Customer').trim();
      if (house && String(house).trim() !== '?' && String(house).trim() !== 'null') {
        b = `${String(house).trim()} ${b}`;
      }
      return b.slice(0, 16).padEnd(16, ' ');
    };
    const formatN = (v, w) => String(Math.round(v || 0)).padStart(w, ' ');
    const formatM = (v) => String(Math.round(v || 0)).trim().padStart(5, ' ');

    const hdr = `${'CUSTOMER'.padEnd(16)} ${'DEL'.padStart(3)} ${'REC'.padStart(3)} ${'BAL'.padStart(3)} ${'CASH'.padStart(5)}`;
    write(hdr, { size: 5.8, bold: true, advance: 2.7 });

    for (const r of sortedRows) {
      const cust = formatCust(r.customerName, r.houseNo);
      const del = formatN(resolvePdfRowDel(r), 3);
      const rec = formatN(resolvePdfRowRec(r), 3);
      const bal = formatN(r.bottleBalance, 3);
      const cash = formatM(r.cashCollected);
      write(`${cust} ${del} ${rec} ${bal} ${cash}`, { size: 5.8, advance: 2.6 });
    }
    rule();

    write(`${'Del:'.padEnd(16)} ${formatN(totalDel, 8)}`, { size: 6.5 });
    write(`${'Rec:'.padEnd(16)} ${formatN(totalRec, 8)}`, { size: 6.5 });
    write(`${'Cash:'.padEnd(16)} ${formatM(totalCash).padStart(8)}`, { size: 6.5 });
    write(`${'GRAND TOTAL:'.padEnd(16)} ${formatM(totalAmount).padStart(8)}`, { size: 7, bold: true });
    rule();

    write('Shukriya · Thank you', { size: 7, bold: true });
    write('Del = Delivered · Rec = Received', { size: 5.8 });
    write('BAL = Customer balance · Cash = Collected', { size: 5.8 });

    return { doc: targetDoc, finalY: y };
  };

  const probeH = Math.max(30 + sortedRows.length * 2.6 + 30, 80);
  const probeDoc = new jsPDF({ unit: 'mm', format: [pageW, probeH], orientation: 'portrait', compress: true });
  const { finalY } = renderOnDoc(probeDoc);
  const pageH = Math.min(Math.max(Math.ceil(finalY + margin + 3), 60), 400);
  ...
  renderOnDoc(doc);
  const slug = String(periodLabel || 'summary').replace(/[^\w-]+/g, '-').slice(0, 24);
  return { doc, pageW, pageH, filename: `water-${kind}-summary-${slug}.pdf` };
}

export async function printWater58mmAllCustomersSummary(args, mode = 'print') {
  if (mode === 'pdf') {
    try {
      const { doc, filename } = await createWater58mmAllCustomersSummaryPdf(args);
      doc.save(filename);
      return true;
    } catch (err) {
  ...
  try {
    const { doc, pageW, pageH } = await createWater58mmAllCustomersSummaryPdf(args);
    const ok = await printJsPdfDocument(doc, { delayMs: 450, pageW, pageH, title: `Summary ${pageW}x${pageH}mm` });
    if (ok) return true;
  } catch (err) {
    console.warn('[waterHisab] 58mm summary PDF print failed, HTML fallback', err);
  }
...
export function buildWaterDailySummaryTableHtml({
  ...
  // Column formatting helpers — 16 chars name fits 58mm at 6px Courier
  const formatCustomerName = (name, house) => {
    let base = String(name || 'Customer').trim();
    if (house && String(house).trim() !== '?' && String(house).trim() !== 'null') {
      base = `${String(house).trim()} ${base}`;
  ...
  // Dynamic page height: header ~22mm + rows ~2.6mm each + totals ~12mm + footer ~8mm
  const pageH = Math.min(Math.max(Math.ceil(22 + customerData.length * 2.6 + 12 + 8), 55), 400);
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .grid .row { display: block; padding: 0; margin: 0; line-height: 1.15; }
  .totals {
    font-family: ${fontStack};
    font-size: 6.5px; white-space: pre; letter-spacing: 0;
    font-variant-numeric: tabular-nums; margin-top: 0.8mm;
  ...
  const totalsLine2 = `${'Rec:'.padEnd(16)} ${formatNum(totalRec, 8)}`;
  const totalsLine3 = `${'Cash:'.padEnd(16)} ${formatMoney(totalCash).trim().padStart(8)}`;
  const grandLine   = `${'TOTAL:'.padEnd(16)} ${formatMoney(totalAmount).trim().padStart(8)}`;
...
export async function createWaterDailySummaryTablePdf(args, existingDoc = null) {
  const { default: jsPDF } = await import('jspdf');
  const pageW = 58;
  const margin = 1.8;
  const contentW = pageW - margin * 2;
  const { business = {}, rows = [], products = [], deliveryDate = '' } = args || {};
  ...
  const renderOnDoc = (targetDoc) => {
    ...
    write('DAILY DELIVERY SUMMARY', { size: 8, bold: true });
    write(deliveryDate || new Date().toISOString().slice(0, 10), { size: 7, bold: true });
    write(`${sortedRows.length} customers`, { size: 6.5 });
    rule();
    ...
    for (const c of customerData) {
      const line = `${formatCust(c.customerName, c.houseNo)} ${formatN(c.del, 3)} ${formatN(c.rec, 3)} ${formatN(c.bal, 3)} ${formatM(c.cash)}`;
      write(line, { size: 5.8, advance: 2.6 });
    }
    rule();
    ...
    write(`${'Rec:'.padEnd(16)} ${formatN(totalRec, 8)}`, { size: 6.5 });
    write(`${'Cash:'.padEnd(16)} ${formatM(totalCash).padStart(8)}`, { size: 6.5 });
    write(`${'TOTAL:'.padEnd(16)} ${formatM(totalAmount).padStart(8)}`, { size: 7, bold: true });
    rule();

    write('Shukriya · Thank you', { size: 7, bold: true });
    write('Del=Delivered Rec=Received BAL=Balance', { size: 5.8 });
    return { doc: targetDoc, finalY: y };
  };

  const probeH = Math.max(28 + customerData.length * 2.6 + 30, 80);
  const probeDoc = new jsPDF({ unit: 'mm', format: [pageW, probeH], orientation: 'portrait', compress: true });
  const { finalY } = renderOnDoc(probeDoc);
  const pageH = Math.min(Math.max(Math.ceil(finalY + margin + 3), 60), 400);
  ...
  renderOnDoc(doc);
  return { doc, pageW, pageH, filename: `water-daily-summary-${deliveryDate || 'today'}.pdf` };
}

export async function printWaterDailySummaryTable(args) {
  try {
    const { doc, pageW, pageH } = await createWaterDailySummaryTablePdf(args);
    const ok = await printJsPdfDocument(doc, { delayMs: 450, pageW, pageH, title: `Daily Summary ${pageW}x${pageH}mm` });
    if (ok) return true;
  } catch (err) {
    console.warn('[waterHisab] daily summary PDF print failed, HTML fallback', err);
  }
...
export async function createWaterDeliveryChecklistPdf(args, existingDoc = null) {
  ...
  const renderOnDoc = (doc) => {
    ...
    //   0      7          20       25         32         39        44
    const col = is80
      ? { h: 0, cu: 7, ph: 31, tg: 47, del: 53, rec: 60, cash: 67, bal: 74 }
      : { h: 0, cu: 6, tg: 27, del: 33, rec: 40, bal: 47 };
    ...
    if (is80) {
      ...
      doc.text('BAL',      margin + 75.5,                y, { align: 'right' });
    } else {
      doc.text('H#',       margin + col.h,              y, { align: 'left' });
      doc.text('CUSTOMER', margin + col.cu,              y, { align: 'left' });
      ...
      doc.text('DEL',      margin + col.del + boxW / 2,  y, { align: 'center' });
      doc.text('REC',      margin + col.rec + boxW / 2,  y, { align: 'center' });
      doc.text('BAL',      margin + 54.5,                y, { align: 'right' });
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const balX = is80 ? margin + 75.5 : margin + 54.5;
