...
export async function createWaterDeliveryChecklistPdf(args, existingDoc = null) {
  ...
  const renderOnDoc = (doc) => {
    ...
    // Heavy separator after business info
    rule('heavy', 'solid');

    // Document title - uppercase, bold, well-spaced
    write('ROUTE DELIVERY CHECKLIST', { size: is80 ? 9.0 : 8.0, bold: true, spacing: 1.5 });
    
    // Date and rider info - bold, medium size
    const dateRiderText = riderName 
      ? `Date: ${deliveryDate}  |  Rider: ${riderName}`
      : `Date: ${deliveryDate}`;
    write(dateRiderText, { size: is80 ? 7.5 : 7.0, bold: true, spacing: 1.2 });
    
    // Stops and target summary
    const summaryText = `Stops: ${rows.length}  |  Target Load: ${totalTargetBottles} Pcs`;
    write(summaryText, { size: is80 ? 7.0 : 6.5, bold: false, spacing: 1.5 });
    
    // Heavy separator before data table
    rule('heavy', 'solid');
    ...
    const rowH   = boxH + 3.0;   // increased from 2.2 → 3.0 to prevent text/box overlap
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // rowTop: align tick-box top with text baseline minus box height
      const rowTop = y - boxH + 0.6;   // top of the tick-box area

      // H# bold
      doc.setFont('courier', 'bold');
      doc.text(house, margin + col.h + 0.5, y, { align: 'left' });
      doc.setFont('courier', 'normal');
      // Customer name — maxWidth prevents bleed into adjacent column
      const nameMaxW = is80 ? (col.ph - col.cu - 1) : (col.tg - col.cu - 1);
      doc.text(name, margin + col.cu + 0.5, y, { align: 'left', maxWidth: nameMaxW });
      // Phone (small, grey) - ensure no overlap with TGT
      if (is80 && ph) {
        doc.setFontSize(5.2);
        doc.setTextColor(100, 100, 100);
        doc.text(ph, margin + col.ph + 0.5, y, { align: 'left', maxWidth: col.tg - col.ph - 1 });
        doc.setTextColor(0, 0, 0);
        doc.setFontSize(fontSize);
      }
      // TGT bold - centered in its column space
      doc.setFont('courier', 'bold');
      const tgtX = is80 ? margin + col.tg + 2 : margin + col.tg + 1.5;
      doc.text(String(rowTgt), tgtX, y, { align: 'left' });
      doc.setFont('courier', 'normal');
      // BAL - positioned within its column, not at the edge
      ...
      // Light dotted row separator (subtle, doesn't overwhelm the content)
      doc.setDrawColor(200);
