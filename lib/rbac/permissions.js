...
import { planHasFeature, FEATURE_MIN_PLAN, resolvePlanTier } from '@/lib/config/plans';
...
import { isWholesaleEnterpriseNavKey } from '@/lib/utils/wholesaleDomain';
...
export const NAV_PERMISSION_MAP = {
    ...
    installments: { permission: 'sales.view', feature: null },
    ...
    // Full Finance Hub (GL, Day Book, Trial Balance) - Professional+
    finance: { permission: 'finance.view_reports', feature: 'finance_hub' },
    ...
    refunds: { permission: 'pos.process_refund', feature: 'standalone_refunds' },
    ...
    'view-storefront': { permission: 'dashboard.view', feature: 'storefront_orders' },
    'store-settings': { permission: 'settings.view', feature: 'storefront_orders' },
...
/**
 * Determine navigation item visibility & lock status based on role + subscription tier.
 *
 * @param {string} navKey - Navigation item key (maps to NAV_PERMISSION_MAP)
 * @param {string} role - User's role in the business
 ...
 * @param {Record<string, boolean> | null | undefined} [moduleAccess] - Optional module overrides; null/undefined = role-only
 * @param {string} [category] - Optional business domain category
 * @returns {{ visible: boolean, locked: boolean, requiredPlan: string | null }}
 */
export function getNavItemAccess(navKey, role, planTier, businessSettings, platformOverrides, moduleAccess, category) {
    ...
    // 2. Wholesale Enterprise domain gating
    if (category && isWholesaleEnterpriseNavKey(navKey, category)) {
        const resolvedTier = resolvePlanTier(planTier || 'free');
    ...
    // 3. Feature flag (subscription) lock — support featuresAny (OR) or single feature
 ...
 */
export function getRequiredPlan(navKey, category) {
    if (category && isWholesaleEnterpriseNavKey(navKey, category)) {
        return 'enterprise';
    }
    const mapping = NAV_PERMISSION_MAP[navKey];
...
/**
 * Standard priority tab order for default landing tab resolution
 */
export const DEFAULT_TAB_PRIORITY = [
    'dashboard',
    'pos',
    'inventory',
    'customers',
...
/**
 * Determine the first allowed and accessible tab for a user based on their active permissions.
 *
 * @param {object} params
 * @param {string} [params.role]
 * @param {string} [params.planTier]
 * @param {object} [params.businessSettings]
 * @param {object} [params.platformOverrides]
 * @param {Record<string, boolean> | null} [params.moduleAccess]
 * @param {string} [params.category]
 * @returns {string} Allowed tab key
 */
export function getFirstAllowedTab({
    ...
    const effectiveRole = (role || 'viewer').toLowerCase().trim();
    if (effectiveRole === 'owner') return 'dashboard';

    // 1. Check priority tab candidates
    for (const tabKey of DEFAULT_TAB_PRIORITY) {
        const access = getNavItemAccess(tabKey, effectiveRole, planTier, businessSettings, platformOverrides, moduleAccess);
        if (access.visible && !access.locked) {
            return tabKey;
        }
    }

    // 2. Fallback: check any visible/unlocked tab in NAV_PERMISSION_MAP
    for (const tabKey of Object.keys(NAV_PERMISSION_MAP)) {
        const access = getNavItemAccess(tabKey, effectiveRole, planTier, businessSettings, platformOverrides, moduleAccess);
        if (access.visible && !access.locked) {
            return tabKey;
        }
