...
const getCachedSession = cache(async () => {
    try {
        ...
        console.error('[serverGuard] Session lookup error:', error);
        return null;
    }
});
    ...
    try {
        ...
        if (memberRes.rows.length === 0 || memberRes.rows[0].status !== 'active') {
            const ownerCheck = await client.query(
                `SELECT id, user_id FROM businesses WHERE id = $1`,
                [businessId]
            );
            if (ownerCheck.rows.length > 0) {
                const biz = ownerCheck.rows[0];
                if (!biz.user_id || biz.user_id === userId || platformAdmin) {
                    await client.query(
                    ...
                    if (!biz.user_id) {
                        await client.query(`UPDATE businesses SET user_id = $1 WHERE id = $2`, [userId, businessId]);
                    }
                    role = 'owner';
                    moduleAccess = null;
                } else {
                ...
                const err = new Error('Unauthorized: Business not found.');
