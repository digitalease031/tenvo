...
export const AccountingService = {
    ...
    async createJournalEntry(params, txClient = null) {
        ...
        try {
            ...
            const status = params.status === 'draft' ? 'draft' : 'posted';
            ...
            `, [businessId, journalNumber, date, description, referenceType, referenceId || null, referenceNumber || null, userId, status]);
    ...
    /**
     * Reverse a posted Journal Entry (GAAP-Compliant Offsetting Posting)
     */
    async reverseJournalEntry(params, txClient = null) {
        const client = await this.getClient(txClient);
        const shouldManageTransaction = !txClient;
        ...
        try {
            ...
            const { journalId, businessId, userId, reason } = params;
            if (!journalId || !businessId) {
                throw new Error('Journal ID and Business ID are required for reversal');
            }

            // 1. Fetch original journal entry
            const origRes = await client.query(
                `SELECT * FROM journal_entries WHERE id = $1 AND business_id = $2`,
                [journalId, businessId]
            );
            if (origRes.rows.length === 0) {
                throw new Error('Original journal entry not found');
            }
            const origJournal = origRes.rows[0];

            if (origJournal.is_reversed) {
                throw new Error(`Journal entry ${origJournal.journal_number} has already been reversed`);
            }

            // 2. Fetch original GL lines
            const linesRes = await client.query(
                `SELECT * FROM gl_entries WHERE journal_id = $1 ORDER BY debit DESC`,
                [journalId]
            );
            if (linesRes.rows.length === 0) {
                throw new Error('Original GL lines not found');
            }

            // 3. Create inverted GL entries (Debits <-> Credits swapped)
            const reversedEntries = linesRes.rows.map(line => ({
                accountId: line.account_id,
                debit: Number(line.credit || 0),
                credit: Number(line.debit || 0),
                description: `Reversal of line in ${origJournal.journal_number}`
            }));

            const reversalDescription = `Reversal of ${origJournal.journal_number}${reason ? `: ${reason}` : ''}`;
            const reversalResult = await this.createJournalEntry({
                ...
                referenceNumber: origJournal.journal_number,
                userId,
                status: 'posted',
            ...
            await client.query(
                ...
                [userId || null, journalId, businessId]
            );
            ...
            return {
                ...
                originalJournalNumber: origJournal.journal_number
            };
        } catch (error) {
            if (shouldManageTransaction) await client.query('ROLLBACK');
            throw error;
     ...
     */
    async postDraftJournalEntry(params, txClient = null) {
        const client = await this.getClient(txClient);
        const shouldManageTransaction = !txClient;
        ...
        try {
            ...
            const { journalId, businessId } = params;
            ...
            if (res.rows.length === 0) {
                throw new Error('Draft journal entry not found or already posted');
            }

            if (shouldManageTransaction) await client.query('COMMIT');
            return { success: true, journal: res.rows[0] };
        } catch (error) {
            if (shouldManageTransaction) await client.query('ROLLBACK');
            throw error;
        } finally {
        ...
        try {
            ...
            switch (type) {
                ...
                case 'payment': {
                    ...
                    // Receipt Void: DR Accounts Receivable, CR Cash/Bank
                    // Payment: DR Accounts Payable, CR Cash/Bank
                    const isReceipt = data.paymentType === 'receipt';
                    const isReceiptVoid = data.paymentType === 'receipt_void';
                    const isCash = (data.paymentMode || 'cash') === 'cash';
                    const accountTypes = (isReceipt || isReceiptVoid)
                        ? [isCash ? 'cash' : 'bank', 'ar']
                        : ['ap', isCash ? 'cash' : 'bank'];
                    ...
                    } else if (isReceiptVoid) {
                        const fundAcc = isCash ? accounts.cash : accounts.bank;
                        entries.push({ accountId: accounts.ar.id, debit: data.amount, credit: 0 });
                ...
                case 'expense': {
                    // Expense: DR Expense Account (net), DR Input Tax (if present), CR Cash/Bank/AP (gross)
                    ...
                    const expenseTax = Number(data.taxAmount || 0);
                    let netExpense = totalAmount;
                    if (expenseTax > 0 && Math.abs(totalAmount - expenseTax) > 0.009) {
                        netExpense = Math.round((totalAmount - expenseTax) * 100) / 100;
                    }

                    // Debit expense account
                    entries.push({ accountId: expenseAccountId, debit: netExpense, credit: 0 });

                    if (expenseTax > 0) {
                        const taxAccounts = await this.getGLAccountsByTypes(businessId, ['input_tax'], client);
                        entries.push({ accountId: taxAccounts.input_tax.id, debit: expenseTax, credit: 0 });
                    }
                    ...
                    if (isPaid) {
                        ...
                        entries.push({ accountId: accounts.ap.id, debit: 0, credit: totalAmount });
                    }
                    break;
                }
                default:
