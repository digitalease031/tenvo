...
import { convertFabricQuantity } from '@/lib/utils/fabricUnitConversions';
...
export const InventoryService = {
        ...
        try {
            ...
            if (params.unit && params.unit !== product.unit) {
                ...
                if (factor) {
                    ...
                    console.warn(
                        `[InventoryService.addStock] No unit_conversions factor for unit "${params.unit}" ` +
                        `on product ${productId} (base unit: "${product.unit}"). ` +
                        `Using 1:1 — add unit_conversions to this product to fix.`
                    );
                }
            }
            ...
            if (batchNumber) {
                const bRes = await client.query(`
                    ...
                    RETURNING id
                `, [businessId, productId, warehouseId, batchNumber, manufacturingDate, expiryDate, finalQuantity, finalCostPrice, notes, safeJsonParam(domainData)]);
            ...
            `, [businessId, productId, warehouseId, batchId, referenceType, quantity, costPrice, referenceType, referenceId, notes, safeJsonParam(domainData)]);
    ...
    async removeStock(params, userId, txClient = null) {
        ...
        try {
            ...
            if (params.unit && params.unit !== product.unit) {
                const { convertedQty } = convertFabricQuantity(
                    qty,
                    params.unit,
                    product.unit,
    ...
    async getProductById(businessId, productId) {
        ...
        try {
            ...
            // Fetch related data
            const locations = await client.query('SELECT sl.*, w.name as warehouse_name FROM product_stock_locations sl JOIN warehouse_locations w ON sl.warehouse_id = w.id WHERE sl.product_id = $1 AND sl.business_id = $2', [productId, businessId]);
            const batches = await client.query(
    ...
    async getInventorySummary(businessId) {
        const client = await this.getClient();
        try {
            const stockVal = await this.getStockValuation(businessId);
            const alerts = await this.getLowStockAlerts(businessId);
