...
export const InvoicePaymentService = {
    ...
    async recordPayment(params, txClient = null) {
        ...
        try {
            ...
            // Calculate new balance directly from validated payment amount
            const newBalance = Math.max(0, Math.round((currentBalance - Number(amount)) * 100) / 100);
            ...
            // Dispatch notification asynchronously outside the transaction lock
            void (async () => {
                try {
    ...
    async voidPayment(businessId, paymentId, userId, reason = '', txClient = null) {
        ...
        try {
            ...
            // Recalculate remaining balance & update invoice payment_status / status
            const newBalanceRes = await client.query(
                'SELECT calculate_invoice_balance($1) as balance',
                [payment.invoice_id]
            );
            const newBalance = Number(newBalanceRes.rows[0]?.balance ?? 0);
            const grandTotal = Number(invoice.grand_total || 0);

            let newPaymentStatus = 'unpaid';
            if (newBalance <= 0.009 && grandTotal > 0) {
                newPaymentStatus = 'paid';
            } else if (newBalance < grandTotal - 0.009) {
            ...
            await client.query(`
                ...
                SET payment_status = $1,
                    status = CASE 
                        WHEN $1 != 'paid' AND status = 'paid' THEN 'pending'
                        ELSE status 
                    END,
                    updated_at = NOW()
                WHERE id = $2 AND business_id = $3
            `, [newPaymentStatus, payment.invoice_id, businessId]);

            // Reverse accounting journal entry
            const mode = String(payment.payment_method || 'cash').toLowerCase();
            const isCash = mode === 'cash' || mode === 'cod';
            await AccountingService.recordBusinessTransaction(
                'payment',
                {
                    ...
                    paymentType: 'receipt_void',
                    paymentMode: isCash ? 'cash' : 'bank',
                    description: `Voided payment for invoice ${invoice.invoice_number}`,
                    userId,
                },
                client
            );
