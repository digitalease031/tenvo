...
export const ProductService = {
    ...
    async getProducts(businessId, options = {}, tx = db) {
        ...
        let products, total;
        try {
            const queryClient = (tx && typeof tx.products?.findMany === 'function') ? tx : db;
            [products, total] = await Promise.all([
                queryClient.products.findMany({
                    where,
                    include,
                    orderBy: { name: 'asc' },
                    ...(usePagination ? { skip: parseInt(effectiveOffset, 10), take: parseInt(effectiveLimit, 10) } : {})
                }),
                usePagination ? queryClient.products.count({ where }) : Promise.resolve(0)
            ]);
        } catch (err) {
            console.warn('[ProductService.getProducts] Prisma query failed, falling back to pg pool:', err.message);
            return this.getProductsViaPg(businessId, options, pool);
        }
    ...
    /**
     * Query products list via pg pool/client fallback when Prisma connection times out
     */
    async getProductsViaPg(businessId, options = {}, clientOrPool = pool) {
        const queryClient = (clientOrPool && typeof clientOrPool.query === 'function') ? clientOrPool : pool;
        const {
            offset = 0,
            limit = 500,
            search = '',
        ...
        const effectiveLimit = Math.min(Math.max(1, parseInt(limit, 10) || 500), 500);
        const effectiveOffset = Math.max(0, parseInt(offset, 10) || 0);

        let whereClause = `p.business_id = $1 AND COALESCE(p.is_deleted, false) = false`;
        const queryParams = [businessId];

        if (search && search.trim()) {
            queryParams.push(`%${search.trim().toLowerCase()}%`);
            whereClause += ` AND (LOWER(p.name) LIKE $${queryParams.length} OR LOWER(COALESCE(p.sku, '')) LIKE $${queryParams.length} OR LOWER(COALESCE(p.barcode, '')) LIKE $${queryParams.length})`;
        }
        ...
        if (isActive !== undefined && isActive !== null) {
            ...
            whereClause += ` AND COALESCE(p.is_active, true) = $${queryParams.length}`;
        }
        ...
        const res = await queryClient.query(
            `SELECT p.*,
                COALESCE((
                    SELECT json_agg(json_build_object('warehouse_id', l.warehouse_id, 'quantity', l.quantity, 'state', l.state)) 
                    FROM product_stock_locations l 
                    WHERE l.product_id = p.id AND l.business_id = p.business_id
                ), '[]'::json) AS product_stock_locations,
                COALESCE((
                    SELECT json_agg(json_build_object(
                        'id', v.id, 'variant_sku', v.variant_sku, 'variant_name', v.variant_name, 
                        'size', v.size, 'color', v.color, 'price', v.price, 'cost_price', v.cost_price, 
                        'mrp', v.mrp, 'stock', v.stock, 'is_default', v.is_default
                    )) FROM product_variants v 
                    WHERE v.product_id = p.id AND v.business_id = p.business_id 
                      AND COALESCE(v.is_deleted, false) = false
                ), '[]'::json) AS product_variants,
                COALESCE((
                    SELECT json_agg(b.*) FROM product_batches b 
                    WHERE b.product_id = p.id AND b.business_id = p.business_id 
                      AND COALESCE(b.is_active, true) = true 
                      AND COALESCE(b.is_deleted, false) = false
                ), '[]'::json) AS product_batches
             FROM products p
             WHERE ${whereClause}
             ORDER BY p.name ASC
        ...
        const mappedProducts = res.rows.map(p => {
            const mapped = {
                ...p,
    ...
    /**
     * Query single product via pg pool/client (used as primary for pg clients or fallback on Prisma connection error)
     */
    async getProductViaPg(productId, businessId, clientOrPool = pool) {
        const queryClient = (clientOrPool && typeof clientOrPool.query === 'function') ? clientOrPool : pool;
        const res = await queryClient.query(
            `SELECT p.*,
                COALESCE((
                ...
                COALESCE((
                    SELECT json_agg(s.*) FROM product_serials s 
                    WHERE s.product_id = p.id AND s.business_id = p.business_id 
                      AND LOWER(COALESCE(s.status, '')) IN ('in_stock', 'available') 
                      AND COALESCE(s.is_deleted, false) = false
                ), '[]'::json) AS product_serials,
                COALESCE((
                    SELECT json_agg(json_build_object('warehouse_id', l.warehouse_id, 'quantity', l.quantity, 'state', l.state)) 
                    FROM product_stock_locations l 
                    WHERE l.product_id = p.id AND l.business_id = p.business_id
             ...
             FROM products p
             WHERE p.id = $1 AND p.business_id = $2 AND COALESCE(p.is_deleted, false) = false`,
    ...
    async getProduct(productId, businessId, tx = db) {
        if (tx && typeof tx.query === 'function' && typeof tx.products?.findFirst !== 'function') {
            return this.getProductViaPg(productId, businessId, tx);
        }
        try {
            const client = (tx && typeof tx.products?.findFirst === 'function') ? tx : db;
            ...
            console.warn('[ProductService.getProduct] Fallback to pg pool:', err.message);
            return this.getProductViaPg(productId, businessId, pool);
        }
    },
