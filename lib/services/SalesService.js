...
export const SalesService = {
    ...
    async createQuotation(data, userId, txClient = null) {
        ...
        try {
            ...
            for (const item of (items || [])) {
                ...
                const lineBase = Math.round(qty * price * 100) / 100;
                calculatedSubtotal += lineBase;
                const itemTaxPct = Number(item.tax_percent || 0);
                const itemTaxAmt = item.tax_amount != null && Number.isFinite(Number(item.tax_amount))
                    ? Number(item.tax_amount)
                    : Math.round((lineBase * itemTaxPct) / 100 * 100) / 100;
                calculatedTax += itemTaxAmt;
            }

            const headerSubtotal = Number(header.subtotal) || calculatedSubtotal;
            const headerTax = Number(header.tax_total) || 0;
            const finalTax = (header.tax_total !== undefined && header.tax_total !== null && header.tax_total !== '')
                ? headerTax
                : calculatedTax;
            const expectedGrandTotal = Math.round((headerSubtotal + finalTax) * 100) / 100;
            const receivedGrandTotal = Math.round(Number(header.total_amount) * 100) / 100;

            if (receivedGrandTotal > 0 && Math.abs(expectedGrandTotal - receivedGrandTotal) > 0.5) {
                console.warn(`Quotation math discrepancy: Expected ${expectedGrandTotal}, received ${receivedGrandTotal}. Accepting explicit total.`);
            }
            ...
            `, [businessId, header.customer_id, qNumber, header.date || new Date(), header.valid_until, headerSubtotal, finalTax, header.total_amount || expectedGrandTotal, header.notes, header.status || 'draft']);
    ...
    async createSalesOrder(data, userId, txClient = null) {
        ...
        try {
            ...
            if (receivedGrandTotal > 0 && Math.abs(expectedGrandTotal - receivedGrandTotal) > 0.5) {
                console.warn(`Sales Order math discrepancy: Expected ${expectedGrandTotal}, received ${receivedGrandTotal}. Accepting explicit total.`);
            }
            ...
            `, [businessId, header.customer_id, header.quotation_id || null, soNumber, header.date || new Date(), header.delivery_date, headerSubtotal, finalTax, header.total_amount || expectedGrandTotal, header.notes, header.status || 'pending']);
