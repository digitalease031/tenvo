...
export const AIOrderForecaster = {
    ...
    async forecastDemand(domainKey, product, salesHistory, options = {}) {
        // Fast-path: if no OPENAI_API_KEY is configured, or offline/statistical engine is requested, instantly return statistical WMA forecast
        if (!process.env.OPENAI_API_KEY || options.useApi === false) {
        ...
        try {
            ...
            if (process.env.NODE_ENV !== 'production') {
                console.info('[AI Forecasting] Operating on statistical WMA forecast fallback.');
            }
            return this.fallbackForecast(product, salesHistory);
        }
    },
