...
/**
 * Parse quantity or calculator style expression (e.g. "5*100" or "5 x 100" or "5").
 * @param {string | number | null | undefined} rawInput
 * @param {number} defaultPrice
 */
export function parseQtyCalculatorInput(rawInput, defaultPrice = 0) {
  if (rawInput === null || rawInput === undefined || rawInput === '') {
    return { qty: 0, price: Number(defaultPrice) || 0, lineTotal: 0, isCustomPrice: false, raw: '' };
  }

  const str = String(rawInput).trim();
  const defP = Number(defaultPrice) || 0;
  ...
  if (str.includes('*') || str.includes('x') || str.includes('X')) {
    ...
    const customPrice = Number.isFinite(p) && p >= 0 ? p : defP;
    const lineTotal = Math.round(qty * customPrice * 100) / 100;
    const isCustom = Number.isFinite(p) && Math.abs(customPrice - defP) > 0.01;
  ...
  const qty = Number.isFinite(q) && q > 0 ? q : 0;
  const lineTotal = Math.round(qty * defP * 100) / 100;
...
export function resolveMilkHisabProducts(products = [], settings = {}) {
  ...
  const withShortLabel = (p, label) => ({
    ...p,
    id: String(p.id),
    hisabShortLabel: label || resolveMilkHisabHintLabel(p) || p.name || null,
  });

  if (Array.isArray(rawIds) && rawIds.length) {
    const byId = new Map(active.map((p) => [String(p.id), p]));
    const picked = rawIds.map((id) => byId.get(String(id))).filter(Boolean);
    if (picked.length) return picked.slice(0, 8).map((p) => withShortLabel(p, p.name));
  }
  ...
  for (const p of active) {
    ...
    out.push(withShortLabel(p, p.name));
  }

  const finalOut = out.slice(0, 8);
  // When multiple products share the same generic hint (e.g. Dawn Bread vs Bake Parlor Bread),
  // preserve their distinct product names so column headers are unambiguous.
  const hintCounts = new Map();
  for (const p of finalOut) {
    const label = p.hisabShortLabel || resolveMilkHisabHintLabel(p);
    if (label) hintCounts.set(label, (hintCounts.get(label) || 0) + 1);
  }
  return finalOut.map((p) => {
    const hint = resolveMilkHisabHintLabel(p);
    if (hint && hintCounts.get(hint) > 1 && p.name) {
      return { ...p, hisabShortLabel: p.name };
    }
    return p;
  });
}
...
export function readMilkCustomerPrefs(customer = {}) {
  ...
  const customMilkRate = Number(dd.custommilkrate ?? dd.custom_milk_rate ?? dd.milkrate ?? 0);
  const productPrices =
    dd.productPrices && typeof dd.productPrices === 'object'
      ? dd.productPrices
      : dd.product_prices && typeof dd.product_prices === 'object'
        ? dd.product_prices
        : {};
  ...
  return {
    ...
    customMilkRate: Number.isFinite(customMilkRate) && customMilkRate > 0 ? customMilkRate : 0,
    productPrices,
    deliveryActive,
