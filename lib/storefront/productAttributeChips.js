...
/**
 * Vehicle (dealership & marketplace) attribute rows for product detail, specs, and cards.
 * @param {object} product
 * @returns {Array<{ key: string, label: string, value: string, filterKey?: string, filterValue?: string }>}
 */
export function buildVehicleAttributeRows(product) {
  const dd = product?.domain_data || {};
  /** @type {Array<{ key: string, label: string, value: string, filterKey?: string, filterValue?: string }>} */
  const rows = [];

  const make = domainField(dd, 'vehiclemake') || domainField(dd, 'make') || product?.brand?.trim();
  if (make) {
    rows.push({ key: 'vehiclemake', label: 'Make', value: make, filterValue: make, filterKey: 'brand' });
  }

  const model = domainField(dd, 'vehiclemodel') || domainField(dd, 'model');
  if (model) {
    rows.push({ key: 'vehiclemodel', label: 'Model', value: model });
  }

  const year = domainField(dd, 'modelyear') || domainField(dd, 'year');
  if (year) {
    rows.push({ key: 'modelyear', label: 'Year', value: String(year) });
  }

  const mileage = domainField(dd, 'mileage');
  if (mileage != null && mileage !== '') {
    const num = Number(mileage);
    const val = !isNaN(num) && num > 0 ? `${num.toLocaleString()} km` : String(mileage);
    rows.push({ key: 'mileage', label: 'Mileage', value: val });
  }

  const fuel = domainField(dd, 'fueltype') || domainField(dd, 'fuel');
  if (fuel) {
    rows.push({ key: 'fueltype', label: 'Fuel Type', value: fuel });
  }

  const transmission = domainField(dd, 'transmission');
  if (transmission) {
    rows.push({ key: 'transmission', label: 'Transmission', value: transmission });
  }

  const engine = domainField(dd, 'enginesize') || domainField(dd, 'engine_cc') || domainField(dd, 'engine');
  if (engine) {
    rows.push({ key: 'enginesize', label: 'Engine', value: engine });
  }

  const body = domainField(dd, 'bodytype');
  if (body) {
    rows.push({ key: 'bodytype', label: 'Body Type', value: body });
  }
  ...
  if (condition) {
    const label = String(condition)
      .replace(/_/g, ' ')
      .replace(/\b\w/g, (c) => c.toUpperCase());
    rows.push({ key: 'condition', label: 'Condition', value: label });
  }

  const regCity = domainField(dd, 'registrationcity') || domainField(dd, 'reg_city') || domainField(dd, 'registration_city');
  if (regCity) {
    rows.push({ key: 'registrationcity', label: 'Registration City', value: regCity });
  }

  const color = domainField(dd, 'exterior_color') || domainField(dd, 'color');
  if (color) {
    rows.push({ key: 'exterior_color', label: 'Exterior Color', value: color });
  }

  const intColor = domainField(dd, 'interior_color');
  if (intColor) {
    rows.push({ key: 'interior_color', label: 'Interior Color', value: intColor });
  }

  const vin = domainField(dd, 'vin') || domainField(dd, 'chassis_no');
  if (vin) {
    rows.push({ key: 'vin', label: 'VIN / Chassis', value: vin });
  }

  const assembly = domainField(dd, 'assembly');
  if (assembly) {
    rows.push({ key: 'assembly', label: 'Assembly', value: assembly });
  }
...
/**
 * Format a compact single-line specification summary for vehicle items in cart or cards.
 * Example: "2021 · 92,000 km · Diesel · Automatic · Islamabad"
 * @param {object} item
 * @returns {string}
 */
export function formatVehicleSummaryLine(item) {
  const dd = item?.domain_data || {};
  const year = domainField(dd, 'modelyear') || domainField(dd, 'year');
  const mileageRaw = domainField(dd, 'mileage');
  const mileageNum = mileageRaw != null && mileageRaw !== '' ? Number(mileageRaw) : NaN;
  const mileage = !isNaN(mileageNum) && mileageNum > 0 ? `${mileageNum.toLocaleString()} km` : mileageRaw || null;
  const fuel = domainField(dd, 'fueltype') || domainField(dd, 'fuel');
  const transmission = domainField(dd, 'transmission');
  const city = domainField(dd, 'registrationcity') || domainField(dd, 'reg_city') || domainField(dd, 'registration_city');
  const condition = domainField(dd, 'condition');

  const parts = [year, mileage, fuel, transmission, city].filter(Boolean);
  if (parts.length > 0) {
    return parts.join(' · ');
  }
  if (condition) {
    return String(condition).replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
  }
  return '';
}
