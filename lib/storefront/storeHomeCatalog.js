    ...
    const [featured, newest, onSale, popularity, categories] = await Promise.all([
      plan.needFeatured
        ? fetchStorefrontProductsOnClient(pool, businessId, {
            ...
            console.error('[storeHomeCatalog] Featured fetch error:', err?.message);
      ...
      plan.needNewest
        ? fetchStorefrontProductsOnClient(pool, businessId, {
            ...
            console.error('[storeHomeCatalog] Newest fetch error:', err?.message);
            return EMPTY_PRODUCTS;
          })
        : Promise.resolve(EMPTY_PRODUCTS),
      plan.needOnSale
        ? fetchStorefrontProductsOnClient(pool, businessId, {
            ...
            console.error('[storeHomeCatalog] OnSale fetch error:', err?.message);
            return EMPTY_PRODUCTS;
          })
        : Promise.resolve(EMPTY_PRODUCTS),
      plan.needPopularity
        ? fetchStorefrontProductsOnClient(pool, businessId, {
            ...
            console.error('[storeHomeCatalog] Popularity fetch error:', err?.message);
            return EMPTY_PRODUCTS;
          })
        : Promise.resolve(EMPTY_PRODUCTS),
      plan.needCategories
        ? fetchStorefrontCategoriesOnClient(pool, businessId).catch((err) => {
            console.error('[storeHomeCatalog] Categories fetch error:', err?.message);
