...
/**
 * Build water-specific monthly bill grid with Day / Bottles / Balance columns.
 * Unlike the milk Y/N grid, this shows actual delivered bottle counts per day
 * and a running bottle balance.
 *
 * @param {{
 *   stops?: Array<{ delivery_date?: Date|string, lines?: Array<{ product_id: string, quantity: unknown, received_quantity?: unknown }> }>,
...
export function buildWaterMonthlyBillGrid({
  ...
  // Index delivered + received quantities by date
  /** @type {Map<string, { del: number, rec: number }>} */
  ...
  for (const stop of stops || []) {
    ...
    if (!byDate.has(dateKey)) byDate.set(dateKey, { del: 0, rec: 0 });
    const entry = byDate.get(dateKey);
    for (const line of stop.lines || []) {
      const q = Number(line.quantity) || 0;
      const r = Number(line.received_quantity) || 0;
      entry.del += q;
      entry.rec += r;
    }
  }

  // Build full calendar span
  const allDates = [];
  if (startIso && endIso) {
    ...
    allDates.push(...[...byDate.keys()].sort());
  }
  ...
  const days = allDates.map((dateKey) => {
    ...
    const rec = entry?.rec || 0;
    const hasDelivery = del > 0;
    if (hasDelivery) activeDays += 1;
    totalDel += del;
    totalRec += rec;
    runningBal = Math.round((runningBal + del - rec) * 1000) / 1000;
    return {
      dateKey,
      dayNum,
      delivered: del,
  ...
  // Also build the original Y/N grid for backward compat (columns, marks, qtys)
  const cols = (columns || [])
    .map((c) => ({
      id: String(c.id),
  ...
  const totalsByProduct = cols.map((col) => {
    ...
    for (const stop of stops || []) {
      ...
      if (endIso && dateKey > endIso) continue;
      for (const line of stop.lines || []) {
        if (String(line.product_id) === col.id) {
  ...
  return {
    ...
    days,
    activeDays,
    totalDel: Math.round(totalDel * 1000) / 1000,
    totalRec: Math.round(totalRec * 1000) / 1000,
...
/**
 * Header line for water monthly bill day grid (58mm).
 * Fixed-width monospace columns: "DD   Y   Bottles  Bal"
 * DD = 2 chars, Y = 1 char (delivered Y/N), Bottles = right-aligned count, Bal = right-aligned
 */
export function formatWaterMonthlyBillHeaderLine() {
  // 32-char usable width on 58mm thermal
  // DD  Y  Bottles  Balance
  return 'DD  Y  Bottles  Balance';
}

/**
 * One line for a day in the water monthly bill grid.
 * @param {{ dayNum: number, delivered: number, balance: number, hasDelivery: boolean }} day
 */
export function formatWaterMonthlyBillDayLine(day) {
  const dd = String(day.dayNum || 0).padStart(2, '0');
  const mark = day.hasDelivery ? 'Y' : 'N';
  const bottles = day.hasDelivery
    ? String(day.delivered).padStart(7, ' ')
    : '      -';
  const bal = String(day.balance).padStart(8, ' ');
  return `${dd}  ${mark}  ${bottles}  ${bal}`;
}
