...
export async function loadPlatformFeatureOverridesForBusiness(businessId, client = null, userId = null) {
  if (!businessId || typeof businessId !== 'string' || !UUID_REGEX.test(businessId)) return {};
  const validUserId = userId && typeof userId === 'string' && UUID_REGEX.test(userId) ? userId : null;
  ...
  try {
    ...
    const params = validUserId ? [businessId, validUserId] : [businessId];
