 ...
 */
export function convertFabricQuantity(quantity, fromUnit, baseUnit, unitConversions, options = {}) {
  ...
  // Handle variable Thaan length breakdown or custom Thaan length
  if (from === 'thaan' && (base === 'meter' || base === 'metre')) {
    const breakdown = options.thaan_breakdown || options.thaanBreakdown;
    let breakdownArray = [];
    ...
    if (breakdownArray.length > 0) {
      const totalM = Math.round(breakdownArray.reduce((sum, len) => sum + len, 0) * 10000) / 10000;
      return { convertedQty: totalM, factor: totalM / Number(quantity || 1), warned: false };
    }

    const thaanLen = Number(options.thaan_length || options.thaanLength || 40);
    const convertedQty = Math.round(Number(quantity) * thaanLen * 10000) / 10000;
    return { convertedQty, factor: thaanLen, warned: false };
  }

  if (from === 'suit' && (base === 'meter' || base === 'metre')) {
    const suitLen = Number(options.suit_cutting || options.suitCutting || 2.25);
    const convertedQty = Math.round(Number(quantity) * suitLen * 10000) / 10000;
    return { convertedQty, factor: suitLen, warned: false };
  }
...
export function formatFabricQtyDisplay(lineItem, lang = 'en') {
  ...
  // Handle variable Thaan breakdown if specified (e.g., [30, 12] or "30, 12")
  const breakdown = lineItem?.thaan_breakdown || lineItem?.thaanBreakdown;
  let breakdownArray = [];
  if (Array.isArray(breakdown)) {
    breakdownArray = breakdown.map(Number).filter(n => !isNaN(n) && n > 0);
  } else if (typeof breakdown === 'string' && breakdown.trim()) {
    breakdownArray = breakdown.split(',').map(s => Number(s.trim())).filter(n => !isNaN(n) && n > 0);
  }

  if (unit === 'thaan') {
    if (breakdownArray.length > 0) {
      const totalM = Math.round(breakdownArray.reduce((sum, len) => sum + len, 0) * 100) / 100;
      const rollsText = breakdownArray.map(n => `${n}m`).join(', ');
      return lang === 'ur'
        ? `${qty} ${unitLabel} (${rollsText} = ${totalM}م)`
        : `${qty} ${unitLabel} (${rollsText} = ${totalM}m)`;
    }
