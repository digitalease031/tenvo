...
/** Canonical Pakistani fabric type list — single source of truth for all clothing verticals. */
...
/** Canonical Pakistani textile color/shade options. */
export const PK_COLOR_OPTIONS = [
...
/** Quick-add name hints for construction & civil contractors (PK). */
export const CONSTRUCTION_NAME_SUGGESTIONS = [
  'Deformed Steel Rebar Grade 60 (12mm - 25mm)',
  'Ordinary Portland Cement OPC 50kg Bags',
  'Ready-Mix Concrete C25 (3000 PSI @ 28 Days)',
  'Ready-Mix Concrete C30 (4000 PSI @ 28 Days)',
  'Bitumen Asphalt Binder Grade 60/70',
  'Crushed Stone Aggregate 19mm (Margalla)',
  'Sand (Lawrencepur / Chenab)',
  'Earthwork Excavation & Compaction',
  'Excavator Hydraulic (CAT 320 Log)',
  'Steel Fixer & Masonry Subcontractor Crew',
];

/** Quick-add name hints for oil & lubricant distributors (PK). */
export const LUBRICANT_NAME_SUGGESTIONS = [
  'Shell Helix HX7 10W-40 (4L Carton)',
  'Shell Rimula R4 X 15W-40 (20L Pail)',
  'Caltex Havoline Formula 20W-50 (1L Bottle)',
  'ZIC X7 10W-40 Synthetic (4L Pack)',
  'PSO Carient Ultra 20W-50 (4L)',
  'Kixx Ultra 4T 20W-50 (0.7L Motorcycle Oil)',
  'Total Quartz 9000 5W-30 (4L Synthetic)',
  'Guard Oil Filter (GFO-101 Toyota)',
  'Vic Oil Filter (C-110 Japan)',
  'Hydraulic Oil HD-68 (208L Drum)',
];
...
export function getInventoryFieldSuggestions(accessorKey, ctx = {}) {
  ...
  if (scalarKey === 'name') {
    ...
    const canonicalDomain = resolveDomainKey(category);
    if (canonicalDomain === 'milk-shop') {
      return uniqueStrings([...MILK_SHOP_NAME_SUGGESTIONS, ...fromCatalog]);
    }
    if (canonicalDomain === 'construction-contractor') {
      return uniqueStrings([...CONSTRUCTION_NAME_SUGGESTIONS, ...fromCatalog]);
    }
    if (canonicalDomain === 'lubricant-distribution') {
      return uniqueStrings([...LUBRICANT_NAME_SUGGESTIONS, ...fromCatalog]);
    }
    return fromCatalog;
  }
  ...
  if (canonical === 'colorshade' || canonical === 'color' || canonical === 'shade' || canonical === 'colour') {
    const pkColors = PK_COLOR_OPTIONS;
    return uniqueStrings([...pkColors, ...configOptions, ...fromRows]);
  }
  ...
  if (canonical === 'season') {
    return uniqueStrings(['Summer', 'Winter', 'Eid', 'Spring', 'Ramadan', 'Wedding', 'Bridal', ...configOptions, ...fromRows]);
  }
