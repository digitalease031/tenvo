...
export function normalizeInvoiceForDocument(invoice, items = [], business = {}) {
  ...
  const biltiDetails =
    invoice.bilti_details ||
    invoice.biltiDetails ||
    taxDetails.bilti_details ||
    taxDetails.biltiDetails ||
    null;
  const previousBalance =
    invoice.previous_balance != null
      ? Number(invoice.previous_balance)
      : invoice.previousBalance != null
        ? Number(invoice.previousBalance)
        : invoice.customer?.outstanding_balance != null
          ? Number(invoice.customer.outstanding_balance)
          : null;
  const brokerDetails =
    invoice.broker_details ||
    invoice.brokerDetails ||
    taxDetails.broker_details ||
    taxDetails.brokerDetails ||
    null;
