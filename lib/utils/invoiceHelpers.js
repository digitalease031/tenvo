...
export function getDomainInvoiceColumns(category) {
    ...
    if (category === 'textile-wholesale' || category === 'textile') {
        columns.push(
            ...
            { header: 'Article #', field: 'article_no', width: 'w-24', placeholder: 'Art #' },
            { header: 'Design #', field: 'design_no', width: 'w-24', placeholder: 'Des #' },
            { header: 'Thaan Len (m)', field: 'thaan_length', width: 'w-24', placeholder: 'e.g. 40', type: 'number' }
        );
        return columns;
    }
...
export function resolveTextileLineQty(lineItem) {
    ...
    if (unit === 'thaan' && qty > 0 && thaanLen > 0) {
    ...
    if ((unit === 'gaz' || unit === 'yard') && qty > 0) {
        ...
        return {
            ...
            totalMeters,
            conversionNote: `${qty} ${unit === 'gaz' ? 'Gaz' : 'Yard'} × 0.9144 = ${totalMeters}m`,
        };
    }
    ...
    return {
        ...
        totalMeters: unit === 'meter' || unit === 'metre' ? qty : null,
        conversionNote: '',
    };
}
