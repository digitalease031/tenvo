...
export function normalizeProductImageUrls(product = {}) {
  const urls = [];
  const push = (raw) => {
    let u = typeof raw === 'string' ? raw.trim() : typeof raw?.url === 'string' ? raw.url.trim() : typeof raw?.src === 'string' ? raw.src.trim() : '';
    if (u.includes('–')) {
      u = u.replace(/–/g, '%E2%80%93');
    }
    if (u.startsWith('http://revoo-ev.com.pk/')) {
      u = u.replace('http://', 'https://');
    }
    if (u.startsWith('http://metroev.pk/')) {
      u = u.replace('http://', 'https://');
    }
    if (u && u !== '[object Object]' && !urls.includes(u)) urls.push(u);
  };
