...
/**
 * Check if a business category is an oil / lubricant distribution domain
 * @param {string} category
 * @returns {boolean}
 */
export function isOilDistributionDomain(category) {
  if (!category) return false;
  const key = String(category).toLowerCase().trim();
  return (
    key === 'lubricant-distribution' ||
    key === 'oil-distribution' ||
    key === 'oil-wholesale' ||
    key === 'lubricants' ||
    key.includes('lubricant')
  );
}
