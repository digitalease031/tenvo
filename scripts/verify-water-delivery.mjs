...
assert.equal(
  openingWaterBottleBalance({ storedBalance: 4, delivered: 5, received: 3 }),
  2,
  'opening reverses day DEL/REC for idempotent re-save'
);
assert.equal(
  computeWaterBottleBalance({
    previous: openingWaterBottleBalance({ storedBalance: 4, delivered: 5, received: 3 }),
    delivered: 5,
    received: 3,
  }),
  4,
  're-save same DEL/REC keeps BAL stable'
);
...
// ---------- Reminder copy must say water, not milk ----------
const waterReminder = buildMilkHisabReminderMessage({
  businessName: 'Aqua Co',
  customerName: 'Ali',
  amount: 500,
...
assert.ok(waterReminder.includes('water delivery bill'), 'water reminder noun');
assert.ok(!waterReminder.includes('milk delivery bill'), 'water reminder must not say milk');
assert.ok(
  waterHisabSrc.includes("deliveryNoun: 'water delivery'"),
  'water hisab reminder passes water deliveryNoun'
);

// ---------- Daily print model includes town code ----------
{
  const { buildWaterDailySalePrintModel } = await import('../lib/print/waterHisabThermalBill.js');
  const dailyModel = buildWaterDailySalePrintModel({
    business: { business_name: 'Aqua', currency: 'PKR', country: 'Pakistan' },
    row: {
      customerName: 'Ali',
      townCode: 'T12',
      accountNo: 'A1',
  ...
  assert.equal(dailyModel.townCode, 'T12', 'daily sale model carries townCode');
}

// ---------- Bulk period PDF helper accepts existingDoc ----------
{
  const printSrc = (await import('node:fs')).readFileSync('lib/print/waterHisabThermalBill.js', 'utf8');
  assert.ok(
    /async function createWaterPeriodPdf\(model,\s*existingDoc/.test(printSrc),
    'createWaterPeriodPdf accepts existingDoc for bulk multi-page PDF'
  );
  assert.ok(
    printSrc.includes('createMilkHisabDayBreakdownPdf(enriched, existingDoc)'),
    'createWaterPeriodPdf forwards existingDoc to milk PDF builder'
  );
}
